...
spec =
    describe "P5 clear-boundary retained-child probes" $ do
        ...
        it "sameLaneQuadrupleAliasFrameClearBoundaryExpr stays recursive as the next explicit milestone-3 representative broader-positive clear-boundary packet after the merged triple-alias anchor while the quantified boundary stays clear" $ do
        ...
        it "sameLaneSextupleAliasFrameClearBoundaryExpr stays recursive as the next explicit milestone-3 representative broader-positive clear-boundary packet after the merged quintuple-alias anchor while the quantified boundary stays clear" $ do
            fallbackTy <- sameLaneSextupleAliasFrameClearBoundaryFallbackType
            containsMu fallbackTy `shouldBe` True

        it "sameLaneSeptupleAliasFrameClearBoundaryExpr stays recursive as the next explicit milestone-3 representative broader-positive clear-boundary packet after the merged sextuple-alias anchor while the quantified boundary stays clear" $ do
            fallbackTy <- sameLaneSeptupleAliasFrameClearBoundaryFallbackType
            containsMu fallbackTy `shouldBe` True

        it "sameLaneOctupleAliasFrameClearBoundaryExpr stays recursive as the next explicit milestone-3 representative broader-positive clear-boundary packet after the merged septuple-alias anchor while the quantified boundary stays clear" $ do
            fallbackTy <- sameLaneOctupleAliasFrameClearBoundaryFallbackType
            containsMu fallbackTy `shouldBe` True

        it "sameLaneNonupleAliasFrameClearBoundaryExpr stays recursive as the next explicit milestone-3 representative broader-positive clear-boundary packet after the merged octuple-alias anchor while the quantified boundary stays clear" $ do
...
sameLaneSextupleAliasFrameClearBoundaryFallbackType :: IO ElabType
sameLaneSextupleAliasFrameClearBoundaryFallbackType = do
...
sameLaneSeptupleAliasFrameClearBoundaryFallbackType :: IO ElabType
sameLaneSeptupleAliasFrameClearBoundaryFallbackType = do
...
sameLaneOctupleAliasFrameClearBoundaryFallbackType :: IO ElabType
sameLaneOctupleAliasFrameClearBoundaryFallbackType = do
...
sameLaneNonupleAliasFrameClearBoundaryFallbackType :: IO ElabType
sameLaneNonupleAliasFrameClearBoundaryFallbackType = do
