...
emlfSurfaceParityMatrix =
    ...
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                [ "module Main export (main) {"
                , "  def main : Bool = let id = λx x in let keepInt = id 1 in id true;"
                , "}"
                ]
        )
        (ExpectRunValue "true")
...
emlfBoundaryMatrix =
    ...
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , "    deriving Eq;"
                , ""
                , "  def main : Bool = eq ((λx x) Zero) Zero;"
                , "}"
                ]
        )
        (ExpectRunValue "true")
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , "    deriving Eq;"
                , ""
                , "  def main : Bool = let id = λx x in eq (id Zero) Zero;"
                , "}"
                ]
        )
        (ExpectRunValue "true")
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , "    deriving Eq;"
                , ""
                , "  def main : Bool = eq (((λx x) Zero) : Nat) Zero;"
                , "}"
                ]
        )
        (ExpectRunValue "true")
    ...
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , "    | Succ : Nat -> Nat;"
                , ""
                , "  def main : Bool = case ((λx x) Zero) of {"
                , "    Zero -> true;"
                , "    Succ _ -> false"
                , "  };"
                , "}"
    ...
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , "    | Succ : Nat -> Nat;"
                , ""
                , "  def main : Bool = let id = λx x in case (id Zero) of {"
                , "    Zero -> true;"
                , "    Succ _ -> false"
                , "  };"
                , "}"
    ...
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , ""
                , "  instance Eq (Box Nat) {"
                , "    eq = λ(left : Box Nat) λ(right : Box Nat) true;"
                , "  }"
                , ""
                , "  def main : Bool = eq ((λx x) (Box Zero)) (Box Zero);"
                , "}"
                ]
        )
        (ExpectRunValue "true")
    ...
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , "    | Some : a -> Option a;"
                , ""
                , "  def isNone : Option Bool -> Bool = λopt true;"
        ...
        (ExpectRunValue "true")
    , ProgramMatrixCase
        "rejects constructor-local ∀ with no recoverable evidence"
        ( InlineProgram $
            unlines
                ...
                , "      Hidden : ∀ a. Hidden;"
                , ""
                , "  def main : Bool = let ignore = λx true in ignore Hidden;"
                , "}"
                ]
        )
    ...
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , "  }"
                , ""
                , "  def sameBool : Eq Bool => Bool -> Bool -> Bool = λx λy eq x y;"
    ...
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , ""
                , "  def same : Eq a => a -> a -> Bool = λx λy eq x y;"
                , "  def main : Bool = let f = λx same x x in f true;"
                , "}"
                ]
        )
        (ExpectRunValue "true")
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , "      Box : a -> Box a;"
                , ""
                , "  def f : C Int => Box Int -> Bool = λx case x of {"
                , "    Box n -> c n"
                , "  };"
                , ""
                , "  def main : Bool = true;"
    ...
    , ProgramMatrixCase
        ...
        (ExpectRunValue "true")
    , ProgramMatrixCase
        "runs deferred method when only a later ∀ binder is inferred"
        ( InlineProgram $
            unlines
    ...
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , "  }"
                , ""
                , "  def needsMarker : Marker Bool => Bool -> Bool = λx x;"
    ...
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , "  }"
                , ""
                , "  def needsMarker : Marker Nat => Bool -> Bool = λx x;"
                , "  def main : Bool = needsMarker true;"
                , "}"
                ]
        )
    ...
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , "  import Core as C;"
                , ""
                , "  def needsMarker : C.Marker Bool => Bool -> Bool = λx x;"
                , "  def main : Bool = needsMarker true;"
                , "}"
                ]
        )
    ...
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , "  }"
                , ""
                , "  def main : Bool = showEq ((λx x) Zero) Zero;"
                , "}"
                ]
        )
        (ExpectRunValue "true")
    , ProgramMatrixCase
        ...
        ( InlineProgram $
            unlines
                ...
                , "  }"
                , ""
                , "  def sameShow : ShowEq a => a -> a -> Bool = λx λy showEq x y;"
    ...
    , ProgramRuntimeCase
        ...
        ( InlineProgram $
            unlines
                ...
                , ""
                , "  def main : Holder = Holder (λ(x : Bool) x);"
                , "}"
                ]
        )
