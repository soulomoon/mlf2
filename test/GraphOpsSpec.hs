...
attachRootGen rootNode c =
    ...
    in c
        { cGenNodes = IntMap.singleton (getGenNodeId rootGenId) (rootGenNode rootNode)
        , cBindParents = bp'
        }
...
genFlexChain n
    | n <= 0 = return emptyConstraint { cGenNodes = IntMap.singleton (getGenNodeId rootGenId) (GenNode rootGenId []) }
