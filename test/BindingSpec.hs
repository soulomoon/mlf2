...
mkOrderedBinderConstraint n =
    let count = max 1 n
        ...
        bindParents =
            bindParentsFromPairs
                [ (NodeId i, binder, BindFlex) | i <- varIds ]
        constraint =
            rootedConstraint emptyConstraint
                { cNodes = nodes
                , cBindParents = bindParents
                }
    in (constraint, binder, bodyRoot)
  where
...
bindingTreeSpec = describe "MLF.Binding.Tree" $ do
    describe "Basic operations" $ do
        it "lookupBindParent returns Nothing for root nodes" $ do
            ...
            lookupBindParent c (genRef (GenNodeId 0)) `shouldBe` Nothing
        ...
        it "setBindParent adds a binding parent" $ do
            let c0 = rootedConstraint emptyConstraint
                ...
                c1 = setBindParent (typeRef (NodeId 1)) (typeRef (NodeId 0), BindRigid) c0
            lookupBindParent c1 (typeRef (NodeId 1)) `shouldBe` Just (typeRef (NodeId 0), BindRigid)
        ...
        it "removeBindParent removes a binding parent" $ do
            let c0 = rootedConstraint emptyConstraint
                ...
                c1 = removeBindParent (typeRef (NodeId 1)) c0
            lookupBindParent c1 (typeRef (NodeId 1)) `shouldBe` Nothing
    ...
    describe "Root detection" $ do
        it "bindingRoots returns all nodes without parents" $ do
            ...
            roots `shouldBe` [genRef (GenNodeId 0)]
        ...
        it "isBindingRoot correctly identifies roots" $ do
            ...
            isBindingRoot c (genRef (GenNodeId 0)) `shouldBe` True
            isBindingRoot c (typeRef (NodeId 0)) `shouldBe` False
            isBindingRoot c (typeRef (NodeId 1)) `shouldBe` False
    ...
    describe "Binder enumeration (Q(n))" $ do
        it "boundFlexChildren returns flex TyVar { tnId = children, tnBound = Nothing }" $ do
            ...
            boundFlexChildren c (typeRef (NodeId 0)) `shouldBe` Right [NodeId 1]
        ...
        it "orderedBinders filters unreachable binders and sorts by order keys" $ do
            ...
            orderedBinders id c (typeRef (NodeId 0)) `shouldBe` Right [NodeId 1, NodeId 2]
    ...
    describe "Path operations" $ do
        it "bindingPathToRoot returns path from node to root" $ do
            ...
            bindingPathToRoot c (typeRef (NodeId 2))
                `shouldBe`
                    Right
                        [ typeRef (NodeId 2)
                        , typeRef (NodeId 1)
            ...
            bindingPathToRoot c (typeRef (NodeId 1))
                `shouldBe` Right [typeRef (NodeId 1), typeRef (NodeId 0), genRef (GenNodeId 0)]
            bindingPathToRoot c (typeRef (NodeId 0))
                `shouldBe` Right [typeRef (NodeId 0), genRef (GenNodeId 0)]
        ...
        it "bindingPathToRoot detects cycles" $ do
            let c = rootedConstraint emptyConstraint
                    ...
                    , cBindParents = bindParentsFromPairs
                        [ (NodeId 0, NodeId 1, BindFlex)
                        , (NodeId 1, NodeId 0, BindFlex)
                        ]
                    }
            case bindingPathToRoot c (typeRef (NodeId 0)) of
        ...
        it "bindingLCA finds lowest common ancestor" $ do
            ...
            bindingLCA c (typeRef (NodeId 3)) (typeRef (NodeId 2)) `shouldBe` Right (typeRef (NodeId 0))
            bindingLCA c (typeRef (NodeId 3)) (typeRef (NodeId 1)) `shouldBe` Right (typeRef (NodeId 1))
            bindingLCA c (typeRef (NodeId 1)) (typeRef (NodeId 2)) `shouldBe` Right (typeRef (NodeId 0))
        ...
        it "bindingLCA uses the gen root for disconnected components" $ do
            ...
            bindingLCA c (typeRef (NodeId 1)) (typeRef (NodeId 3))
                `shouldBe` Right (genRef (GenNodeId 0))
    ...
    describe "Node kind classification (paper §3.1)" $ do
        it "distinguishes Restricted vs Locked (and strict under-rigid)" $ do
            ...
            nodeKind c (typeRef (NodeId 0)) `shouldBe` Right NodeInstantiable
            nodeKind c (typeRef (NodeId 1)) `shouldBe` Right NodeRestricted
            nodeKind c (typeRef (NodeId 2)) `shouldBe` Right NodeLocked
            nodeKind c (typeRef (NodeId 3)) `shouldBe` Right NodeInstantiable
        ...
        it "interiorOf returns all nodes bound to root" $ do
            ...
            case interiorOf c (typeRef (NodeId 0)) of
                Right interior -> do
                    ...
                    IntSet.member (nodeRefKey (typeRef (NodeId 0))) interior `shouldBe` True
                    IntSet.member (nodeRefKey (typeRef (NodeId 1))) interior `shouldBe` True
                    IntSet.member (nodeRefKey (typeRef (NodeId 2))) interior `shouldBe` True
                    IntSet.member (nodeRefKey (typeRef (NodeId 3))) interior `shouldBe` True
                Left err -> expectationFailure $ "Expected success, got: " ++ show err

            -- Interior of node 1 should be {1, 3}
            case interiorOf c (typeRef (NodeId 1)) of
                Right interior -> do
    ...
    describe "Interior computation (quotient)" $ do
        it "interiorOfUnder collapses ids under canonicalization (Binding.interiorOfUnder)" $ do
            ...
            interiorOfUnder canonical c (typeRef (NodeId 0))
                `shouldBe`
                    Right
                        (IntSet.fromList
        ...
        it "interiorOfUnder ignores self-edges induced by canonicalization (Binding.interiorOfUnder)" $ do
            ...
            interiorOfUnder canonical c (typeRef (NodeId 0))
                `shouldBe` Right (IntSet.fromList [nodeRefKey (typeRef (NodeId 0))])

    describe "Invariant checking" $ do
        it "checkBindingTree succeeds for empty constraint" $ do
            checkBindingTree (rootedConstraint emptyConstraint) `shouldBe` Right ()
        ...
        it "checkBindingTree succeeds for valid chain" $ do
            ...
            let c = rootedConstraint emptyConstraint
                    ...
                    , cBindParents = bindParentsFromPairs
                        [ (NodeId 1, NodeId 0, BindFlex)  -- 1's parent is 0 (0 can reach 1)
                        , (NodeId 2, NodeId 1, BindFlex)  -- 2's parent is 1 (1 can reach 2)
                        ]
                    }
            checkBindingTree c `shouldBe` Right ()
    ...
    describe "isUpper predicate" $ do
        ...
        it "isUpper returns True when parent can reach child via TyArrow" $ do
            ...
            isUpper c (typeRef (NodeId 0)) (typeRef (NodeId 1)) `shouldBe` True
            isUpper c (typeRef (NodeId 0)) (typeRef (NodeId 2)) `shouldBe` True
        ...
        it "isUpper returns False when parent cannot reach child" $ do
            ...
            isUpper c (typeRef (NodeId 1)) (typeRef (NodeId 2)) `shouldBe` False
            isUpper c (typeRef (NodeId 2)) (typeRef (NodeId 1)) `shouldBe` False
        ...
        it "isUpper returns True for transitive reachability via TyForall" $ do
            ...
            isUpper c (typeRef (NodeId 0)) (typeRef (NodeId 2)) `shouldBe` True
            isUpper c (typeRef (NodeId 0)) (typeRef (NodeId 3)) `shouldBe` True
            -- Node 1 can reach node 2 directly
            isUpper c (typeRef (NodeId 1)) (typeRef (NodeId 2)) `shouldBe` True
        ...
        it "isUpper handles shared subgraphs correctly" $ do
            ...
            isUpper c (typeRef (NodeId 0)) (typeRef (NodeId 4)) `shouldBe` False
            isUpper c (typeRef (NodeId 1)) (typeRef (NodeId 3)) `shouldBe` False
    ...
    describe "Property tests" $ do
        ...
        it "binding roots are exactly nodes without parents" $ property $
            forAll (choose (1, 20)) $ \n -> do
                ...
                roots `shouldBe` [genRef (GenNodeId 0)]
                -- The gen root has no parent.
                forM_ roots $ \r -> lookupBindParent c r `shouldBe` Nothing
                -- Every type node should have a binding parent.
                forM_ allNodes $ \nid ->
                    lookupBindParent c (typeRef nid) `shouldSatisfy` isJust
        ...
        it "path to root terminates for all nodes in valid trees" $ property $
            forAll (choose (1, 20)) $ \n -> do
                ...
                forM_ allNodes $ \nid -> do
                    case bindingPathToRoot c (typeRef nid) of
                        Right path -> do
                            -- Path should start with the node itself
                            case path of
                                (first:_) -> first `shouldBe` typeRef nid
            ...
            forM_ testExprs $ \(name, expr) -> do
                case generateConstraintsDefault expr of
                    ...
                    Right result -> do
                        ...
                        forM_ nonRoots $ \nid -> do
                            case lookupBindParent c (typeRef nid) of
            ...
            forAll (choose (2, 15)) $ \n -> do
                ...
                forM_ allNodes $ \nid -> do
                    case lookupBindParent c (typeRef nid) of
                        ...
                        Just (parentRef, _) -> do
                            case (interiorOf c (typeRef nid), interiorOf c parentRef) of
        ...
        it "interiorOfUnder matches quotient binding-path characterization (interiorOfUnder property)" $ property $
            forAll (choose (2, 20)) $ \n -> do
                ...
                let rootC = canonical (NodeId r0)
                    ...
                    quotientParents =
                        foldl'
                            (\m (childKey, (parent0, _flag)) ->
                                case nodeRefFromKey childKey of
                                    GenRef _ -> m
                                    TypeRef childId ->
                                        let childC = nodeRefKey (typeRef (canonical childId))
                                            parentC = nodeRefKey $ case parent0 of
                                                TypeRef pid -> typeRef (canonical pid)
                    ...
                    manualInterior = IntSet.filter hasRootOnPath canonTypeKeys

                case interiorOfUnder canonical c (typeRef rootC) of
...
genConstraintWithBinderChain n
    ...
    | otherwise =
        ...
        in return $ rootedConstraint emptyConstraint
            { cNodes = nodes
            , cBindParents = bindParents
            }
                                ...
                                case replayRaises c trace of
                                    Right cReplayed -> do
                                        ...
                                        forM_ nodesToCheck $ \nidInt -> do
                                            ...
                                            let replayedParent = lookupBindParent cReplayed (typeRef nid)
                                                harmonizedParent = lookupBindParent c' (typeRef nid)
