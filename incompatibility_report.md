...
**Implementation**: `Presolution.hs` (Phase 4) failed to propagate unification constraints derived from instantiation edges in certain cases:
...
- **Fix**: Implemented `unifyStructure` in `Presolution.hs` to recursively unify children of merged structural nodes.
...
**Implementation**: `Normalize.hs` (Phase 2) updated the Union-Find structure but did not "graft" the structure onto the variable node in the graph itself.
...
- **Fix**: Modified `applyUnionFindToConstraint` in `Normalize.hs` to destructive update `TyVar` nodes to become copies of their canonical structural representatives.
...
- **Fix**: Updated `runPipelineElab` to explicitly generalize the root node at the top level (GNode 0), consistent with the paper's observation that xMLF generalizes more freely.
...
**Implementation**: `generalizeAt` in `Elab.hs` blindly created new `Forall` wrappers even if the target node was already a `TyForall` with the correct level (structural forall).
...
- **Phase 4 witness recording**:
  - `src/MLF/Types.hs`: `InstanceOp`, `InstanceWitness`, `EdgeWitness`
  - `src/MLF/Presolution.hs`: records `prEdgeWitnesses :: IntMap EdgeWitness` (canonicalized during `rewriteConstraint`)
- **Phase 6 translation + checking**:
  - `src/MLF/Elab.hs`: `phiFromEdgeWitness` (Φ), `sigmaReorder` (Σ), and `applyInstantiation` (xmlf Fig. 3) for applying/checking instantiations against xMLF types.
...
- Witnesses are currently derived conservatively from the presolution lattice (`Expansion`) and therefore cover the operations the solver currently produces (`Raise`, `Graft`+`Weaken`). `Merge`/`RaiseMerge` are represented in the witness datatype but are not yet emitted by presolution, and instantiation-context translation \(C_{nr}\) is not exercised by current witnesses.
