...
- **item-1**
  - Verify the predecessor authority chain cites the completed April 1 family
    plus the accepted April 2 settlement and decision artifacts explicitly.
  - Verify one exact next representative-gap packet is frozen concretely.
  - Verify the inherited explicit-only / iso-recursive / non-equi-recursive /
    non-cyclic-graph / no-fallback boundary stays explicit and unchanged.
  - Verify the writable slice is explicit and fail-closed.
- **item-2**
  - Verify the diff stays inside the item-1 writable slice.
  - Verify focused regression coverage for the frozen packet is updated or
    replayed honestly.
...
- **item-3**
  - Verify the settlement surface cites exact focused/full-gate provenance from
    accepted sources and does not upgrade one bounded packet into general repo
    readiness.
