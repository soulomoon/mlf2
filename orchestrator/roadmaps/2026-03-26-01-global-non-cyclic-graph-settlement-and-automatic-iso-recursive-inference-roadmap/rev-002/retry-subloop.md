...
rounds on the reopened `non-cyclic-graph` revision lane inside the current
same-family automatic iso-recursive inference roadmap.
...
- roadmap items `2` through `4` may retry inside the same round.
...
Every reopened-scope / subject-selection / contract / bind / decision review
must record:
...
- `max_attempts` is `100` for roadmap items `2`, `3`, and `4`
- item `1` and item `5` are aggregate-only and therefore finalize or reject;
...
- the guider may publish a later roadmap revision under the same
  `roadmap_id` if accepted reopened-lane evidence requires a narrower
  architecture-amendment handoff, a stop decision, or a return to blocked
  implementation items under an explicitly re-settled boundary; and
- the guider may not rewrite completed-item truth or silently widen into
  multi-SCC search, second interfaces, fallback paths, or a broader
  capability claim without saying so.

## Historical Compatibility

Rounds `round-001` through `round-103` remain historical predecessor
