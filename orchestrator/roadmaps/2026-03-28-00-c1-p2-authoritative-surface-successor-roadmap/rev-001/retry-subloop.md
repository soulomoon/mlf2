...
the bounded `C1` / `P2` authoritative-surface successor loop.
...
- the guider may refine later pending items while staying inside the active
  bounded `C1` / `P2` authoritative-surface family;
- the guider may publish a later roadmap revision under the same `roadmap_id`
  if item `4` chooses stop or bounded continuation; and
- the guider may not silently reopen the settled same-lane pocket, promote
  `P5` into scope, claim repo-level capability success, authorize cyclic
