...
1. **Roadmap lineage, pointer, and preserved-history consistency**
   ...
   - Confirm accepted `round-211`, merged as `5b775b2`, accepted
     ...
     `7616109`, are treated as the merged predecessor baseline chain rather
     than as unmerged live-round diffs.
2. **Diff hygiene**
...
- **milestone-3**
  ...
  - Verify the merged `7616109` baseline wins remain green:
    ...
    milestone-3 clear-boundary anchor after that,
    `sameLaneOctupleAliasFrameClearBoundaryExpr` as the next explicit
    milestone-3 clear-boundary anchor after that,
  ...
  - Verify the round broadens the representative broader-positive corpus
    honestly beyond the merged first eight clear-boundary anchors by making
    `sameLaneNonupleAliasFrameClearBoundaryExpr` explicit on
  ...
  - Verify the round keeps `sameLaneAliasFrameClearBoundaryExpr` as preserved
    ...
    after that, and does not widen the live extraction into deeper alias
    shells.
