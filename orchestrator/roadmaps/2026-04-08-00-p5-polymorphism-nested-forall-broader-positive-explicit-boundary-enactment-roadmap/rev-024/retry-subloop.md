...
newly exposed blocker while keeping `rev-024` fit for a direct follow-up.
...
- Do not widen the `rev-024` continuation beyond the preserved writable slice
  in
...
- Keep the live pointer stubs in
  ...
  surfaces aligned with the published `rev-024` bundle, and do the same for
