...
- `milestone-2` retries for:
  code outside the `rev-004` writable slice,
  ...
  an attempted combined annotation-translation plus post-annotation-handoff
  repair that still leaves the selected packet blocked either in the same
  Phase 6 `reifyInst` path or in the downstream close/preserve/re-typecheck
  handoff, or that reopens public facades, fallback-core seams, or blocked
  `round-208` / `round-209` / `round-210` artifacts instead of clearing the
  combined seam on a fresh round.
...
- Do not widen the `rev-004` milestone-2 continuation beyond the exact
  combined seam:
...
- Treat `round-208`, `round-209`, and `round-210` as immutable blocked
  predecessor evidence only. Continue milestone-2 with a fresh round; do not
  edit any blocked round worktree artifacts.
- Do not reopen
