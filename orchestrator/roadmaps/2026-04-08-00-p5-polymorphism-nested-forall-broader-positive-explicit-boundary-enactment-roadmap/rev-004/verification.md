...
1. **Roadmap lineage, pointer, and preserved-history consistency**
   ...
   - Confirm `round-208`, `round-209`, and `round-210` artifacts remain
     immutable predecessor evidence if the round cites them, and confirm any
     continued milestone-2 work happens on a fresh round rather than inside
     any blocked round worktree.
2. **Diff hygiene**
...
5. **Broader-positive boundary discipline**
   ...
   - For milestone-2 rounds under `rev-004`, confirm the continuation stays
     limited to the exact combined seam:
...
- **milestone-2**
  - Verify the diff stays inside the milestone-1 writable slice as superseded
    ...
    `test/PipelineSpec.hs`, and
    `test/Research/P5ClearBoundarySpec.hs`.
  - Verify the selected code-bearing change clears the Phase 6 authoritative
    annotation translation for the selected same-wrapper nested-`forall`
    packet and carries that same packet through the downstream
    post-annotation close/preserve/re-typecheck handoff without fallback
    rescue, public shim insertion, or a second interface.
  - If the round touches
    `src/MLF/Elab/Elaborate/Annotation.hs` or
    `src/MLF/Elab/Legacy.hs`, verify the repair stays on the existing
    no-fallback authoritative translation path and does not invent a broad
    compatibility helper, a new public seam, or a different packet family.
  - If the round touches
    `src/MLF/Elab/Run/Pipeline.hs` or
    `src/MLF/Elab/TermClosure.hs`, verify the repair stays on the already-
    produced post-annotation term candidates and does not re-enter
    annotation, reconstruct a fallback result, or broaden retained-child
    alias handling beyond the selected packet.
  ...
  - Verify `test/ElaborationSpec.hs` captures the exact Phase 6 seam or exact
    selected-packet translation behavior rather than only broad pipeline
    symptoms.
  - Verify
    ...
    remain untouched in this exact continuation.
...
```json
{
  ...
  "roadmap_revision": "rev-004",
  "roadmap_dir": "orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-004",
