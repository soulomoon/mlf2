...
1. **Roadmap lineage, pointer, and preserved-history consistency**
   ...
   - Confirm accepted `round-211`, merged as `5b775b2`, accepted
     ...
     `2091c39`, and accepted `round-214`, merged as `ed66291`, are treated
     as the merged predecessor baseline chain rather than as unmerged
     live-round diffs.
2. **Diff hygiene**
...
- **milestone-3**
  ...
  - Verify the merged `ed66291` baseline wins remain green:
  ...
  - Verify the round broadens the representative broader-positive corpus
    honestly beyond the merged first three clear-boundary anchors by making
    `sameLaneQuadrupleAliasFrameClearBoundaryExpr` explicit on
  ...
  - Verify the round keeps `sameLaneAliasFrameClearBoundaryExpr` as preserved
    ...
    anchor after that, and does not widen the live extraction into quintuple-
    /deeper-alias shells.
