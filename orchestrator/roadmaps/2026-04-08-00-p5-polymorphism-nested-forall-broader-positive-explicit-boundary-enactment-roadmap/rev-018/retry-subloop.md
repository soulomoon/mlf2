...
- `milestone-3` retries for:
  ...
  `sameLaneTripleAliasFrameClearBoundaryExpr` next anchor after that,
  failure to make `sameLaneQuadrupleAliasFrameClearBoundaryExpr` honest as
  the next explicit milestone-3 representative anchor on the `P5` research /
  pipeline / elaboration surfaces,
  regressions to the merged `ed66291` baseline wins,
  loss of checked-authoritative parity,
  reopening the retained-child predecessor lane as live debt,
  widening from the selected quadruple-alias packet into quintuple/deeper
  ...
  merged milestone-2 baseline and first three merged milestone-3 anchors
  honestly.
...
newly exposed blocker while keeping `rev-018` fit for a direct follow-up.
...
- Do not widen from the accepted explicit-only / iso-recursive boundary into
  ...
  explicitly.
- Do not widen the `rev-018` continuation beyond the preserved writable slice
  in
...
- Treat accepted `round-211`, merged as `5b775b2`, accepted `round-212`,
  ...
  accepted `round-214`, merged as `ed66291`, as the merged predecessor
  baseline chain. Do not relitigate the milestone-2 handoff repair or the
  first three clear-boundary anchor publications as if they were still live
  blockers.
- Keep the live pointer stubs in both
  `orchestrator/roadmap.md`,
  `orchestrator/retry-subloop.md`, and
  `orchestrator/verification.md`
  surfaces aligned with the active `rev-018` bundle, and do the same for the
...
- Do not treat the merged selected same-wrapper nested-`forall` packet, the
  ...
  preserved `sameLaneAliasFrameClearBoundaryExpr` predecessor lane as full
...
- Do not widen the exact next corpus-promotion packet beyond
  `sameLaneQuadrupleAliasFrameClearBoundaryExpr` unless a later accepted
