...
1. **Roadmap lineage, pointer, and preserved-history consistency**
   ...
   - Confirm `round-208` and `round-209` artifacts remain immutable
     predecessor evidence if the round cites them, and confirm any continued
     milestone-2 work happens on a fresh round rather than inside either
     blocked round worktree.
2. **Diff hygiene**
...
5. **Broader-positive boundary discipline**
   ...
   - For milestone-2 rounds under `rev-003`, confirm the continuation stays
     limited to the exact post-annotation authoritative-result handoff
     centered on `src/MLF/Elab/Run/Pipeline.hs`, the exact companion surface
     `src/MLF/Elab/TermClosure.hs`, and only the required focused test
     surfaces `test/PipelineSpec.hs` and
...
- **milestone-2**
  - Verify the diff stays inside the milestone-1 writable slice as superseded
    ...
    and `test/Research/P5ClearBoundarySpec.hs`.
  - Verify the selected code-bearing change stops treating
    ...
    where the accepted revised ledger requires, specifically across the
    post-annotation close/preserve/re-typecheck handoff.
  - If the round touches `src/MLF/Elab/Run/Pipeline.hs` or
    `src/MLF/Elab/TermClosure.hs`, verify the repair does not reopen
    fallback-like recovery, re-annotation shims, cyclic search, or a second
    interface.
