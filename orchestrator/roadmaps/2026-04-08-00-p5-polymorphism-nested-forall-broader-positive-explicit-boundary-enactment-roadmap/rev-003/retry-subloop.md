...
- `milestone-2` retries for:
  code outside the `rev-003` writable slice,
  ...
  an attempted post-annotation authoritative-result-handoff repair that still
  leaves the selected packet blocked after close/preserve/re-typecheck or that
  reopens `Annotation.hs` / `Legacy.hs` / `test/ElaborationSpec.hs` instead of
  clearing the handoff seam,
  or a same-round edit attempt against round-208 or round-209 artifacts
  instead of a fresh follow-on round.
...
- Do not widen the `rev-003` milestone-2 continuation beyond the exact
  post-annotation authoritative-result handoff centered on
  `src/MLF/Elab/Run/Pipeline.hs`, the exact companion surface
  `src/MLF/Elab/TermClosure.hs`, and the exact focused test surfaces
  `test/PipelineSpec.hs` and `test/Research/P5ClearBoundarySpec.hs` unless a
  later accepted roadmap revision authorizes more.
- Treat `round-208` and `round-209` as immutable blocked predecessor evidence
  only. Continue milestone-2 with a fresh round; do not edit either blocked
  round's worktree artifacts.
- Do not reopen `src/MLF/Elab/Elaborate/Annotation.hs`,
