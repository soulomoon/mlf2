...
1. **Roadmap lineage, pointer, and preserved-history consistency**
   ...
   - Confirm accepted `round-211`, merged as `5b775b2`, is treated as merged
     predecessor baseline rather than as an unmerged live-round diff.
2. **Diff hygiene**
...
- **milestone-3**
  ...
  - Verify the merged `5b775b2` baseline wins remain green:
    the selected same-wrapper nested-`forall` packet on both authoritative
    entrypoints, checked-authoritative parity,
    `BUG-2026-02-06-002`,
  ...
  - Verify the round broadens the representative broader-positive corpus
    honestly beyond the single selected packet, using
    `test/Research/P5ClearBoundarySpec.hs` and the matching
    `test/PipelineSpec.hs` / `test/ElaborationSpec.hs` rows as the reviewable
    evidence surface.
  ...
  - Verify direct success remains real on both authoritative entrypoints and
    does not depend on helper-only output or compatibility shims.
  - Verify
