...
- `milestone-3` retries for:
  ...
  missing representative-corpus coverage beyond the selected same-wrapper
  nested-`forall` packet,
  regressions to the merged `5b775b2` baseline wins,
...
newly exposed blocker while keeping `rev-015` fit for a direct follow-up.
...
- Do not widen from the accepted explicit-only / iso-recursive boundary into
  ...
  explicitly.
- Do not widen the `rev-015` continuation beyond the preserved writable slice
  in
...
- Treat accepted `round-211`, merged as `5b775b2`, as the merged milestone-2
  baseline. Do not relitigate its freshness, merge scope, or admitted
  helper-local scaffold as if they were still live blockers.
- Keep the live pointer stubs in both
  `orchestrator/roadmap.md`,
  `orchestrator/retry-subloop.md`, and
  `orchestrator/verification.md`
  surfaces aligned with the active `rev-015` bundle, and do the same for the
...
- Do not treat one repaired packet as full broader-positive frontier closure.
