...
1. **Roadmap lineage, pointer, and preserved-history consistency**
   ...
   - Confirm accepted `round-211`, merged as `5b775b2`, accepted
     ...
     `round-215`, merged as `1b62ad5`, and accepted `round-216`, merged as
     `21fddba`, are treated as the merged predecessor baseline chain rather
...
- **milestone-3**
  ...
  - Verify the merged `21fddba` baseline wins remain green:
  ...
  - Verify the round broadens the representative broader-positive corpus
    honestly beyond the merged first five clear-boundary anchors by making
    `sameLaneSextupleAliasFrameClearBoundaryExpr` explicit on
  ...
  - Verify the round keeps `sameLaneAliasFrameClearBoundaryExpr` as preserved
    ...
    after that, and does not widen the live extraction into septuple-/deeper-
    alias shells.
