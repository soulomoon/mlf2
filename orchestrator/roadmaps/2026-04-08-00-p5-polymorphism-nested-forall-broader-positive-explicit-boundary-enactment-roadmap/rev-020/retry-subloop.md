...
- `milestone-3` retries for:
  ...
  `sameLaneDoubleAliasFrameClearBoundaryExpr` next anchor, and the merged
  `sameLaneTripleAliasFrameClearBoundaryExpr` next anchor after that, and the
  merged `sameLaneQuadrupleAliasFrameClearBoundaryExpr` next anchor after
  that, and the merged
  `sameLaneQuintupleAliasFrameClearBoundaryExpr` next anchor after that,
  failure to make `sameLaneSextupleAliasFrameClearBoundaryExpr` honest as
  the next explicit milestone-3 representative anchor on the `P5` research /
  pipeline / elaboration surfaces,
  regressions to the merged `21fddba` baseline wins,
  loss of checked-authoritative parity,
  reopening the retained-child predecessor lane as live debt,
  widening from the selected sextuple-alias packet into septuple/deeper
  alias shells as substitute closure,
  ...
  merged milestone-2 baseline and first five merged milestone-3 anchors
  honestly.
...
newly exposed blocker while keeping `rev-020` fit for a direct follow-up.
...
- Do not widen from the accepted explicit-only / iso-recursive boundary into
  ...
  explicitly.
- Do not widen the `rev-020` continuation beyond the preserved writable slice
  in
...
- Treat accepted `round-211`, merged as `5b775b2`, accepted `round-212`,
  merged as `9bb2229`, accepted `round-213`, merged as `2091c39`, and
  accepted `round-214`, merged as `ed66291`, and accepted `round-215`,
  merged as `1b62ad5`, and accepted `round-216`, merged as `21fddba`, as the
  merged predecessor baseline chain. Do not relitigate the milestone-2
  handoff repair or the first five clear-boundary anchor publications as if
  they were still live blockers.
- Keep the live pointer stubs in both
  `orchestrator/roadmap.md`,
  `orchestrator/retry-subloop.md`, and
  `orchestrator/verification.md`
  surfaces aligned with the published `rev-020` bundle, and do the same for
...
- Do not treat the merged selected same-wrapper nested-`forall` packet, the
  ...
  after that, and the preserved `sameLaneAliasFrameClearBoundaryExpr`
  predecessor lane as full broader-positive frontier closure.
...
- Do not widen the exact next corpus-promotion packet beyond
  `sameLaneSextupleAliasFrameClearBoundaryExpr` unless a later accepted
  roadmap revision authorizes a deeper alias-budget extraction explicitly.
