...
1. **Roadmap lineage, pointer, and preserved-history consistency**
   ...
   - Confirm accepted `round-211`, merged as `5b775b2`, accepted
     `round-212`, merged as `9bb2229`, and accepted `round-213`, merged as
     `2091c39`, are treated as the merged predecessor baseline chain rather
...
- **milestone-3**
  ...
  - Verify the merged `2091c39` baseline wins remain green:
  ...
  - Verify the round broadens the representative broader-positive corpus
    honestly beyond the merged first two clear-boundary anchors by making
    `sameLaneTripleAliasFrameClearBoundaryExpr` explicit on
  ...
  - Verify the round keeps `sameLaneAliasFrameClearBoundaryExpr` as preserved
    ...
    and does not widen the live extraction into quadruple-/deeper-alias
    shells.
