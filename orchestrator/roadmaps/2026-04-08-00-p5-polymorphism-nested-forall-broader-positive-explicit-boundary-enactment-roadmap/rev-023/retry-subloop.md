...
- `milestone-3` retries for:
  ...
  that, failure to make `sameLaneOctupleAliasFrameClearBoundaryExpr` honest as
...
newly exposed blocker while keeping `rev-023` fit for a direct follow-up.
...
- Do not widen from the accepted explicit-only / iso-recursive boundary into
  ...
  explicitly.
- Do not widen the `rev-023` continuation beyond the preserved writable slice
  in
...
- Keep the live pointer stubs in both
  ...
  surfaces aligned with the published `rev-023` bundle, and do the same for
...
- Do not treat the merged selected same-wrapper nested-`forall` packet, the
  ...
  the preserved `sameLaneAliasFrameClearBoundaryExpr` predecessor lane as full
