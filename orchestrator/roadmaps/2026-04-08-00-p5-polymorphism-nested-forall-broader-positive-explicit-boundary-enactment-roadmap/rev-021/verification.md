...
1. **Roadmap lineage, pointer, and preserved-history consistency**
   ...
   - Confirm accepted `round-211`, merged as `5b775b2`, accepted
     ...
     `21fddba`, and accepted `round-217`, merged as `f405079`, are treated as
...
- **milestone-3**
  ...
  - Verify the merged `f405079` baseline wins remain green:
  ...
  - Verify the round broadens the representative broader-positive corpus
    honestly beyond the merged first six clear-boundary anchors by making
    `sameLaneSeptupleAliasFrameClearBoundaryExpr` explicit on
  ...
  - Verify the round keeps `sameLaneAliasFrameClearBoundaryExpr` as preserved
    ...
    after that, and does not widen the live extraction into octuple-/deeper-
    alias shells.
