...
proved that `rev-009` still stopped one seam too early.

The same live `round-211` branch/worktree remains the authoritative
continuation baseline, and `rev-010` preserves its inherited truth exactly:
...
`rev-009` is now exhausted after the failed closure-only continuation. The
authoritative review and implementation notes preserve three controlling facts:

- the only tested direct continuation in
  `src/MLF/Elab/TermClosure.hs` around
  `closeTermWithSchemeSubstIfNeeded` was reverted;
- that temporary alias-closure experiment could remove the nested-let
  alias-side `TCLetTypeMismatch` only by turning the fail-fast probe into
  false success `forall a. a -> a`; and
- the restored final diff keeps the inherited `round-211` baseline, while the
  mandatory full gate remains `1341` examples / `26` failures and
  thesis-conformance still fails on the remaining A6 `TCExpectedArrow`
  cluster.

The newly admitted seam in `rev-010` is therefore one step downstream of the
exhausted `rev-009` closure-only lane and no broader:

- keep `src/MLF/Elab/TermClosure.hs` closed again and treat `rev-009` as
  blocker evidence, not as an active writable surface;
- admit `src/MLF/Elab/Elaborate/Algebra.hs` only in the exact post-closure
  authoritative classification / handoff locals around `rhsAbs0`, `rhsAbs`,
  `rhsFinal`, and the immediate `ALetF` `bodyElab` / `env'` consumer path
  that classifies the closed alias result for authoritative use;
- admit `src/MLF/Elab/Elaborate/Annotation.hs` only inside the authoritative
  ...
  `instSeqApps`) when reviewer-visible evidence proves that the post-closure
  classification repair cannot restore strict fail-fast behavior without that
  immediate companion; and
...
- Record `rev-009` honestly as exhausted after the failed closure-only
  ...
  probe turns into false success `forall a. a -> a`, so the next lawful move
  is downstream post-closure authoritative classification / witness
  refinement, not another direct `TermClosure.hs` retry.
...
- Completion notes:
  ...
  - `rev-010` records `rev-009` as exhausted after the failed closure-only
    continuation: the alias-side mismatch can be removed only by false
    success `forall a. a -> a`, so the active contract closes
    `TermClosure.hs` again and admits only the next downstream post-closure
    authoritative classification / witness seam.
...
- Coordination notes:
  ...
  - stay inside the exact `rev-010` continuation without overclaiming whole
...
- Progress notes:
  ...
  - `rev-009` is now exhausted because the direct `TermClosure.hs`
    continuation removed the alias-side mismatch only by producing false
    success `forall a. a -> a`, then had to be reverted; and
  - the required full gate remains red with `26` failures concentrated in the
    remaining A6 `TCExpectedArrow` cluster and the nested-let strict-failure
    shape, so the next exact move is downstream post-closure authoritative
    classification / witness refinement, not another closure-only retry and
    not a reopen of pipeline/public/fallback surfaces.
...
- Direction id: `direction-2b-thread-the-selected-semantics-through-authoritative-pipeline-seams`
  ...
  annotation, post-closure classification, and witness-visible seams so the
  selected behavior is visible where the family measures success.
  Why it matters now: accepted `round-207` and live `round-211` already
  repaired the selected packet itself; `rev-009` further proved that the
  remaining blocker is no longer direct closure but the downstream
  authoritative classification / witness read needed to turn false success
  back into the accepted strict fail-fast behavior.
  Preconditions:
  - `milestone-1`;
  ...
  - the authoritative `round-211` attempt-4 review proof that the direct
    `TermClosure.hs` continuation is exhausted and the next lawful seam is
    downstream post-closure classification / witness refinement.
  ...
  - admit `src/MLF/Elab/Elaborate/Algebra.hs` only around the exact
    post-closure `ALetF` locals `rhsAbs0`, `rhsAbs`, `rhsFinal`, and the
    immediate authoritative `bodyElab` / `env'` handoff;
  ...
  - exact same-round continuation task:
    `continue-round-211-selected-same-wrapper-nested-forall-through-post-closure-authoritative-classification-and-witness-refinement-seam`
  - current task/title:
    `Continue the selected same-wrapper nested-forall packet through the post-closure authoritative classification / witness refinement seam while preserving the live round-211 baseline`
  - preserve the current `round-211` diff rather than discarding it on a
    fresh round;
  - keep the inherited `Algebra.hs` packet repair as baseline rather than
    relitigating it as unresolved work;
...
- Continuation id: `round-211-live-post-closure-authoritative-classification-and-witness-refinement-continuation`
  Direction: `direction-2b-thread-the-selected-semantics-through-authoritative-pipeline-seams`
  ...
  - records `rev-009` honestly as exhausted because the direct
    `TermClosure.hs` continuation removed the alias-side mismatch only by
    false success `forall a. a -> a`, while the restored baseline still ends
    at `1341` examples / `26` failures and an A6 `TCExpectedArrow` cluster;
    and
  - makes the next exact move a same-round continuation that preserves the
    current diff, keeps `TermClosure.hs` closed, and minimally reopens only
    the downstream post-closure authoritative classification / witness
    refinement seam.
