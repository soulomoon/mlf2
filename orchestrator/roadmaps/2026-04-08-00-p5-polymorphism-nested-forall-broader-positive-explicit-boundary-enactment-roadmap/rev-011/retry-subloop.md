...
- `milestone-2` retries for:
  ...
  checked-authoritative parity, the classic let-polymorphism /
  explicit-`forall` positives, `BUG-2026-02-06-002`, or the retained-child
  exact packet,
  unchanged controlling behavior in the newly admitted downstream
  application-consumer recovery seam,
  missing focused evidence for the remaining A6 / nested-let /
  authoritative-instantiation blocker cluster,
  ...
  6 stop, the downstream `PhiReorder: missing binder identity` detour, or the
  previously cleared `ALetF` handoff progress now preserved as baseline,
  an attempted repair that still leaves the A6 dual-coercion pair or the
  non-local proxy wrapper `g g` at `TCExpectedArrow` after the admitted seam
  has been exercised,
  an attempted repair that still leaves the nested-let fail-fast / invariant
  probes at `TCArgumentMismatch` after the admitted seam has been exercised,
  an attempted repair that still leaves `BUG-2026-02-17-002` /
  `Phi alignment C4` at authoritative-instantiation non-applicability after
  the admitted seam has been exercised,
  or any attempt that discards the preserved `round-211` diff, restarts on a
  fresh round, retries `TermClosure.hs` or the bounded `ALetF` post-closure
  slice as if they were still the active seam, or reopens pipeline/public/
  fallback surfaces instead of clearing the exact downstream consumer /
  immediate authoritative-companion seam.
...
- Do not widen from the accepted explicit-only / iso-recursive boundary into
  ...
  explicitly.
- Do not widen the `rev-011` milestone-2 continuation beyond the preserved
  ...
  plus the newly admitted downstream `ALamF` / `AAppF` consumer seam and its
  immediate authoritative-instantiation companion.
...
- Do not relitigate the inherited selected-packet or `ALetF` handoff repairs
  as if they were still unresolved. Preserve those as settled baseline and
  only adjust the newly admitted downstream `ALamF` / `AAppF` consumer locals
  and immediate helper reads.
