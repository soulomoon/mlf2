...
- `milestone-3` retries for:
  ...
  after that, and the merged
  `sameLaneSextupleAliasFrameClearBoundaryExpr` next anchor after that,
  the merged `sameLaneSeptupleAliasFrameClearBoundaryExpr` next anchor after
  that,
  failure to make `sameLaneOctupleAliasFrameClearBoundaryExpr` honest as
  the next explicit milestone-3 representative anchor on the `P5` research /
  pipeline / elaboration surfaces,
  regressions to the merged `7a127e2` baseline wins,
  loss of checked-authoritative parity,
  reopening the retained-child predecessor lane as live debt,
  widening from the selected octuple-alias packet into nonuple/deeper
  ...
  merged milestone-2 baseline and first seven merged milestone-3 anchors
  honestly.
...
newly exposed blocker while keeping `rev-022` fit for a direct follow-up.
...
- Do not widen from the accepted explicit-only / iso-recursive boundary into
  ...
  explicitly.
- Do not widen the `rev-022` continuation beyond the preserved writable slice
  in
...
- Treat accepted `round-211`, merged as `5b775b2`, accepted `round-212`,
  ...
  `round-217`, merged as `f405079`, and accepted `round-218`, merged as
  `7a127e2`, as the merged predecessor baseline chain. Do not relitigate the
  milestone-2 handoff repair or the first seven clear-boundary anchor
...
- Keep the live pointer stubs in both
  ...
  surfaces aligned with the published `rev-022` bundle, and do the same for
...
- Do not widen the exact next corpus-promotion packet beyond
  `sameLaneOctupleAliasFrameClearBoundaryExpr` unless a later accepted
