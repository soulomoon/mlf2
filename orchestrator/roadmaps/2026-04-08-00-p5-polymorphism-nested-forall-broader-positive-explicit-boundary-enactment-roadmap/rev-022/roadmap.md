...
`rev-022` preserves the merged baseline exactly:
...
`rev-022` therefore keeps milestone-3 live with a concrete post-round-218
contract:
...
- Record `rev-012`, `rev-013`, and `rev-014` honestly as exhausted
  ...
  seventh milestone-3 post-merge publication epoch. `rev-022` supersedes
  `rev-021` only for post-round-218 coordination; it does not rewrite
  accepted evidence.
- `rev-022` does not authorize milestone-4 closeout work.
