...
Succeed the completed planning-only
`2026-04-07-00-p5-polymorphism-nested-forall-explicit-boundary-revision-roadmap`
family by enacting one direct code-bearing family that consumes accepted
`round-205` as its family-entry authority and stops treating
polymorphic-mediation `mu` absorption as the controlling behavior for the
broader positive `P5 polymorphism-nested-forall` frontier beyond the one
settled retained-child clear-boundary lane.

This family must turn the accepted revised planning ledger into production
behavior and evidence by:

- preserving recursive structure across the broader positive quantified-
  crossing frontier where the accepted revised ledger now requires that read,
- making the representative broader positive frontier pass on both
  `runPipelineElab` and `runPipelineElabChecked`,
- preserving the inherited explicit-only / iso-recursive /
...
- recording the new controlling behavior honestly once the code and evidence
  actually earn it.
...
- The family is code-bearing. `src/`, `src-public/`, `test/`, docs, and
...
- Preserve accepted `round-203` as the controlling revised freeze,
  accepted `round-204` as the broader-positive planning ledger, and accepted
  `round-205` as the one exact downstream bind. Those artifacts remain
  immutable predecessor authority and must not be rewritten in place.
...
- `milestone-1` must complete before any code-bearing enactment round. The
  first round in this family must extract from `milestone-1`.
- `milestone-2` may change production semantics only inside the writable slice
  frozen by accepted `milestone-1`.
