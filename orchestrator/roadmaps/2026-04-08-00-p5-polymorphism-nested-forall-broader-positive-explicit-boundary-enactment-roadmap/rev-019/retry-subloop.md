...
- `milestone-3` retries for:
  ...
  merged milestone-2 baseline and first four merged milestone-3 anchors
  honestly.
...
newly exposed blocker while keeping `rev-019` fit for a direct follow-up.
...
- Do not widen from the accepted explicit-only / iso-recursive boundary into
  ...
  explicitly.
- Do not widen the `rev-019` continuation beyond the preserved writable slice
  in
...
- Treat accepted `round-211`, merged as `5b775b2`, accepted `round-212`,
  ...
  merged as `1b62ad5`, as the merged predecessor baseline chain. Do not
  relitigate the milestone-2 handoff repair or the first four clear-boundary
  anchor publications as if they were still live blockers.
- Keep the live pointer stubs in both
  `orchestrator/roadmap.md`,
  `orchestrator/retry-subloop.md`, and
  `orchestrator/verification.md`
  surfaces aligned with the published `rev-019` bundle, and do the same for
...
- Do not treat the merged selected same-wrapper nested-`forall` packet, the
  ...
  merged `sameLaneQuadrupleAliasFrameClearBoundaryExpr` next anchor after
  that, and the preserved `sameLaneAliasFrameClearBoundaryExpr` predecessor
  lane as full broader-positive frontier closure.
...
- Do not widen the exact next corpus-promotion packet beyond
  `sameLaneQuintupleAliasFrameClearBoundaryExpr` unless a later accepted
