...
Continue the same code-bearing enactment family after blocked `round-208`
proved that `rev-001` stopped one seam too early for the selected
...
What changes in `rev-002` is only the milestone-2 continuation contract:
the writable slice now expands just enough to let a fresh follow-on round
repair the authoritative annotation translation path centered on
`src/MLF/Elab/Elaborate/Annotation.hs`, plus only the exact continuity files
that seam may require.
...
- `rev-002` does not authorize milestone-3 corpus widening, milestone-4
  closeout work, or edits outside the exact annotation-seam expansion named
  below.
...
- `milestone-2` may change production semantics only inside the writable slice
  frozen by accepted `milestone-1`, as superseded narrowly by `rev-002`.
...
- Treat blocked `round-208` as predecessor evidence only. Continue from a
  fresh follow-on round rather than mutating round-208 artifacts in place.
...
- Coordination notes: consume accepted `round-206` and its milestone-1
  ...
  internal mechanism slice, consume blocked `round-208` only as predecessor
  evidence that the next seam is the authoritative annotation translation
  path, stay inside the exact `rev-002` writable slice, keep the family
  within the accepted explicit-only / iso-recursive / non-equi-recursive /
...
- Blocked predecessor notes: blocked `round-208` preserved fresh evidence in
  ...
  Its `Run/Pipeline`-only experiments did not change that failure, so the
  remaining exact seam sits in `src/MLF/Elab/Elaborate/Annotation.hs`, outside
  the old `rev-001` writable slice. Milestone-2 therefore remains pending,
  and the next serial move is a fresh follow-on round for
  `direction-2b-thread-the-selected-semantics-through-authoritative-pipeline-seams`
  with the extracted task
  `repair-same-wrapper-nested-forall-authoritative-annotation-translation`.
...
- Direction id: `direction-2b-thread-the-selected-semantics-through-authoritative-pipeline-seams`
  ...
  the family measures success, beginning with the authoritative annotation
  translation seam now known to block the selected packet.
  ...
  translation seam, and narrowly justified focused tests only.
  Extraction notes: the first fresh round after `rev-002` should own
  `test/Research/P5ClearBoundarySpec.hs`,
  `test/PipelineSpec.hs`,
  `src/MLF/Elab/Elaborate/Annotation.hs`,
  and only the exact companion surfaces the no-fallback authoritative
  translation seam requires:
  `src/MLF/Elab/Legacy.hs`,
  `test/ElaborationSpec.hs`,
  plus already-frozen pipeline/public/term-closure files only if the selected
  packet truly needs them to stay honest end to end. Reapply the round-208
  authoritative-expectation changes on a fresh branch rather than mutating
  round-208 artifacts, and stop again if another seam outside this bounded
  slice is required.
...
- Evidence id: `round-208-blocked-authoritative-annotation-translation-seam`
  ...
  which `rev-002` now admits explicitly.
