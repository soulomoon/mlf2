...
1. **Roadmap lineage, pointer, and preserved-history consistency**
   ...
   - Confirm `round-208` artifacts remain immutable predecessor evidence if
     the round cites them, and confirm any continued milestone-2 work happens
     on a fresh round rather than inside the round-208 worktree.
2. **Diff hygiene**
...
5. **Broader-positive boundary discipline**
   ...
   - For milestone-2 rounds under `rev-002`, confirm any writable-slice
     expansion stays limited to the exact authoritative annotation
     translation seam centered on `src/MLF/Elab/Elaborate/Annotation.hs` and
     the exact companion surfaces `src/MLF/Elab/Legacy.hs` and
     `test/ElaborationSpec.hs` when they are truly required.
...
- **milestone-2**
  ...
  - If the round touches `src/MLF/Elab/Elaborate/Annotation.hs` or
    `src/MLF/Elab/Legacy.hs`, verify the repair consumes already-recorded
    authoritative instantiation/expansion evidence only and does not reopen
    fallback-like recovery, cyclic search, or a second interface.
  - Verify the authoritative packet is covered honestly on both entrypoints
    in `test/PipelineSpec.hs` and `test/Research/P5ClearBoundarySpec.hs`, and
    verify `test/ElaborationSpec.hs` carries a focused Phase-6 regression if
    the annotation seam is touched.
