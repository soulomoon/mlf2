...
1. **Roadmap lineage, pointer, and preserved-history consistency**
   ...
   - Confirm accepted `round-211`, merged as `5b775b2`, and accepted
     `round-212`, merged as `9bb2229`, are treated as the merged predecessor
...
- **milestone-3**
  ...
  - Verify the merged `9bb2229` baseline wins remain green:
  ...
  - Verify the round broadens the representative broader-positive corpus
    honestly beyond the merged first `sameLaneClearBoundaryExpr` anchor by
    making `sameLaneDoubleAliasFrameClearBoundaryExpr` explicit on
  ...
  - Verify the round keeps `sameLaneAliasFrameClearBoundaryExpr` as preserved
    predecessor truth and does not widen the live extraction into
    triple-/deeper-alias shells.
