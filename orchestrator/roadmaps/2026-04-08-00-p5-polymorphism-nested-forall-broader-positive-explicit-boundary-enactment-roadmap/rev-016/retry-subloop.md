...
- `milestone-3` retries for:
  ...
  same-wrapper nested-`forall` packet and the merged
  `sameLaneClearBoundaryExpr` first anchor,
  failure to make `sameLaneDoubleAliasFrameClearBoundaryExpr` honest as the
  next explicit milestone-3 representative anchor on the `P5` research /
  pipeline / elaboration surfaces,
  regressions to the merged `9bb2229` baseline wins,
  loss of checked-authoritative parity,
  reopening the retained-child predecessor lane as live debt,
  widening from the selected double-alias packet into triple/deeper alias
  ...
  merged milestone-2 baseline honestly.
...
newly exposed blocker while keeping `rev-016` fit for a direct follow-up.
...
- Do not widen from the accepted explicit-only / iso-recursive boundary into
  ...
  explicitly.
- Do not widen the `rev-016` continuation beyond the preserved writable slice
  in
...
- Treat accepted `round-211`, merged as `5b775b2`, and accepted `round-212`,
  merged as `9bb2229`, as the merged predecessor baseline chain. Do not
  relitigate the milestone-2 handoff repair or the first clear-boundary anchor
  publication as if they were still live blockers.
- Keep the live pointer stubs in both
  `orchestrator/roadmap.md`,
  `orchestrator/retry-subloop.md`, and
  `orchestrator/verification.md`
  surfaces aligned with the active `rev-016` bundle, and do the same for the
...
- Do not treat the merged selected same-wrapper nested-`forall` packet, the
  merged `sameLaneClearBoundaryExpr` first anchor, and the preserved
...
- Do not widen the exact next corpus-promotion packet beyond
  `sameLaneDoubleAliasFrameClearBoundaryExpr` unless a later accepted roadmap
