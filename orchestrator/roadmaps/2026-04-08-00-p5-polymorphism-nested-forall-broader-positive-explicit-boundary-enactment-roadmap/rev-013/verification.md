...
5. **Broader-positive boundary discipline**
   ...
   - For milestone-2 rounds under `rev-013`, confirm the continuation stays
...
- Verify the admitted `rev-013` continuation matches the blocker proof:
...
- Verify the remaining blocker cluster is explicitly rechecked:
  ...
  `BUG-2026-02-17-002`, and
  `g g`.
- Verify direct let-polymorphism repair does not merely trade
  `arrow vs forall` for `arrow vs arrow-of-arrow` or another equally
  non-landable mismatch while leaving A6 or nested-let red.
...
- Verify both parent-workspace and canonical round-worktree live pointer stubs
  still match the active `rev-013` bundle.
...
to stay inside the enacted `rev-013` boundary without silently widening
semantics, interfaces, or architecture and without merely reshuffling the
same blocker family into a different visible error shape.
