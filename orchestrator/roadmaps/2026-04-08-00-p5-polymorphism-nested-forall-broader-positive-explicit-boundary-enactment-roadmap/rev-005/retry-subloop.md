...
- `milestone-2` retries for:
  ...
  an attempted same-round application-builder repair that regresses the
  selected packet back to the old Phase 6 `reifyInst` stop or the downstream
  `PhiReorder: missing binder identity` detour,
  an attempted same-round application-builder repair that still leaves the
  selected packet blocked at Phase 7 `TCArgumentMismatch`,
  ...
  clearing the exact `AAppF` continuation.
...
- Do not widen the `rev-005` milestone-2 continuation beyond the preserved
  ...
  plus the one newly admitted application-builder seam
...
- Do not broaden `src/MLF/Elab/Elaborate/Algebra.hs` beyond the `AAppF`
  path and its exact helper logic around authoritative argument
  instantiation / `mu` coercion.
