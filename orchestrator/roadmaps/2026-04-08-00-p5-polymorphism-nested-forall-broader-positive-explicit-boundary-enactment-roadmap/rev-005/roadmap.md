...
proved that `rev-004` still stopped one seam too early in a more local way:
the combined annotation-plus-handoff continuation now clears the old
Phase 6 authoritative translation stop and the downstream
`PhiReorder: missing binder identity` detour, but the selected same-wrapper
nested-`forall` packet still dies later in Phase 7 with
`TCArgumentMismatch` inside the authoritative application term that
elaboration builds.
...
What changes in `rev-005` is only the milestone-2 continuation contract:
live `round-211` materially advanced the same selected packet on its current
branch/worktree baseline. The preserved round-211 diff in
`src/MLF/Elab/Elaborate/Annotation.hs`,
`src/MLF/Elab/Legacy.hs`,
`test/ElaborationSpec.hs`,
`test/PipelineSpec.hs`, and
`test/Research/P5ClearBoundarySpec.hs`
must stay in place. The newly authorized addition is exactly one more
production owner:
`src/MLF/Elab/Elaborate/Algebra.hs`,
limited to the `AAppF` path and the exact helper logic around authoritative
argument instantiation / `mu` coercion for that same selected packet, plus
only the same focused tests if they truly need companion updates.
`src/MLF/Elab/Run/Pipeline.hs` and `src/MLF/Elab/TermClosure.hs` return to
...
- Preserve live `round-211` as the same-round continuation baseline, not as
  ...
  detour for the selected packet, but it has not yet earned acceptance or
  merge because the same packet still fails later in Phase 7 with
  `TCArgumentMismatch`.
...
- `rev-005` does not authorize milestone-3 corpus widening, milestone-4
...
- Coordination notes: consume accepted `round-206` and its milestone-1
  ...
  two earlier stops but still fails later in Phase 7 application building,
  stay inside the exact `rev-005` continuation, keep the family within the
...
- Live continuation notes: live `round-211` remains on its existing
  ...
  `PhiReorder: missing binder identity` detour, and the remaining focused
  failure is
  `Phase 7 (type checking): TCArgumentMismatch`
  on the authoritative application term. The next exact move is therefore not
  a fresh round and not a reopened pipeline/handoff seam; it is a same-round
  continuation under
  `direction-2b-thread-the-selected-semantics-through-authoritative-pipeline-seams`
  that preserves the current round-211 diff and adds exactly the
  application-builder seam in
  `src/MLF/Elab/Elaborate/Algebra.hs`,
  limited to `AAppF` plus the exact helper logic around authoritative
...
- Direction id: `direction-2b-thread-the-selected-semantics-through-authoritative-pipeline-seams`
  ...
  and application construction so the selected behavior is visible where the
  family measures success, continuing the same round-211 packet from the
  preserved annotation-side baseline into the exact `AAppF`
  application-builder seam that still emits the Phase 7
  `TCArgumentMismatch`.
  ...
  branch/worktree, but the authoritative entrypoints still fail later in
  application building with `TCArgumentMismatch`, so the next move is to
  preserve that current diff and repair only the remaining application-builder
  seam.
  ...
  `test/PipelineSpec.hs`, and
  `test/Research/P5ClearBoundarySpec.hs`,
  plus one additional application-builder seam in
  `src/MLF/Elab/Elaborate/Algebra.hs`
  limited to `AAppF` and the exact helper logic around authoritative argument
  instantiation / `mu` coercion. Keep
  `src/MLF/Elab/Run/Pipeline.hs`,
  `src/MLF/Elab/TermClosure.hs`,
  `src/MLF/Elab/Pipeline.hs`,
  `src-public/MLF/Pipeline.hs`,
  ...
  `continue-round-211-selected-same-wrapper-nested-forall-through-application-builder-aappf`.
  ...
  logic, and touch the same focused tests only if the `AAppF` repair truly
...
- Continuation id: `round-211-live-application-builder-aappf-blocker`
  Direction: `direction-2b-thread-the-selected-semantics-through-authoritative-pipeline-seams`
  ...
  packet. The remaining blocker is
  `Phase 7 (type checking): TCArgumentMismatch`
  on the authoritative application term. The next exact move is to continue
  on the same round-211 branch/worktree, preserve that current diff, and add
  only
  `src/MLF/Elab/Elaborate/Algebra.hs`
  limited to the `AAppF` path and the exact helper logic around authoritative
  argument instantiation / `mu` coercion.
