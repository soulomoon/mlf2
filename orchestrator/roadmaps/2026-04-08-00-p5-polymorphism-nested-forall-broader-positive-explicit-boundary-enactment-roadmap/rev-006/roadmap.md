...
proved that `rev-005` is a real dead-end, not stale bookkeeping:
an `Algebra.hs`-only `AAppF` repair can clear the first selected-packet
Phase 7 `TCArgumentMismatch`, but only by exposing an immediate
Phase 7 `TCLetTypeMismatch` against the preserved let-scheme /
...
live `round-211` still stays on its current branch/worktree baseline, and
the preserved round-211 diff in
`src/MLF/Elab/Elaborate/Annotation.hs`,
`src/MLF/Elab/Legacy.hs`,
`test/ElaborationSpec.hs`,
`test/PipelineSpec.hs`, and
`test/Research/P5ClearBoundarySpec.hs`
must stay in place. `rev-005`'s attempted `AAppF`-only continuation proved
the next writable seam is still entirely inside
`src/MLF/Elab/Elaborate/Algebra.hs`,
but it is broader than `AAppF` alone: the selected `ALetF` let-scheme
finalization / closure logic around scheme selection,
`closeTermWithSchemeSubstIfNeeded`, and `rhsFinal`, with the surrounding
`AAppF` context still available if needed inside that same file to keep the
selected packet coherent. `src/MLF/Elab/Run/Pipeline.hs`,
`src/MLF/Elab/TermClosure.hs`, public facades, and fallback seams remain
read-only continuity anchors for this continuation.
...
- Preserve live `round-211` as the same-round continuation baseline, not as
  ...
  current branch/worktree diff already clears the old Phase 6
  `reifyInst` stop and the downstream `PhiReorder: missing binder identity`
  detour for the selected packet. `round-211`'s preserved
  `implementation-notes.md` now also proves that an `AAppF`-only
  `Algebra.hs` repair merely trades the first Phase 7
  `TCArgumentMismatch` for an immediate Phase 7 `TCLetTypeMismatch`
  against the preserved let scheme, so the live round still has not earned
  acceptance or merge.
...
- `milestone-2` may change production semantics only inside the writable slice
  frozen by accepted `milestone-1`, as superseded narrowly by `rev-005`.
...
- `rev-005` supersession note: live `round-211` materially advanced the
  ...
  `PhiReorder: missing binder identity` detour, but focused authoritative
  checks now stop later with
  `Phase 7 (type checking): TCArgumentMismatch`
  in the application builder. The active contract therefore preserves the
  current round-211 diff in
  `src/MLF/Elab/Elaborate/Annotation.hs`,
  `src/MLF/Elab/Legacy.hs`,
  `test/ElaborationSpec.hs`,
  `test/PipelineSpec.hs`, and
  `test/Research/P5ClearBoundarySpec.hs`
  as the live same-round baseline, and admits exactly one additional owner:
  `src/MLF/Elab/Elaborate/Algebra.hs`.
  That added seam is limited to the `AAppF` path and the exact helper logic
  around authoritative argument instantiation / `mu` coercion for the same
  selected packet. No other part of the milestone-1 freeze changes, and
  `src/MLF/Elab/Run/Pipeline.hs`,
  `src/MLF/Elab/TermClosure.hs`,
  `src/MLF/Elab/Pipeline.hs`,
  `src-public/MLF/Pipeline.hs`,
  `src/MLF/Elab/Run/ResultType/Fallback.hs`, and
  `src/MLF/Elab/Run/ResultType/Fallback/Core.hs`
  are read-only continuity anchors for this revision.
- `rev-006` supersession note: live `round-211` then proved that the
  tightened `rev-005` continuation is a real dead-end rather than stale
  bookkeeping. An `AAppF`-only `Algebra.hs` repair can clear the first
  selected-packet Phase 7 `TCArgumentMismatch`, but only by exposing an
  immediate Phase 7 `TCLetTypeMismatch` against the preserved let scheme /
  authoritative handoff contract. The active contract therefore preserves the
  current round-211 diff in
  `src/MLF/Elab/Elaborate/Annotation.hs`,
  `src/MLF/Elab/Legacy.hs`,
  `test/ElaborationSpec.hs`,
  `test/PipelineSpec.hs`, and
  `test/Research/P5ClearBoundarySpec.hs`
  as the live same-round baseline, keeps
  `src/MLF/Elab/Elaborate/Algebra.hs`
  as the only newly authored production owner, and broadens that same-file
  seam only enough to include the selected `ALetF` let-scheme finalization /
  closure logic around scheme selection,
  `closeTermWithSchemeSubstIfNeeded`, and
  `rhsFinal`, with the surrounding `AAppF` context still available if needed
  inside that same file. No other part of the milestone-1 freeze changes, and
  `src/MLF/Elab/Run/Pipeline.hs`,
  `src/MLF/Elab/TermClosure.hs`,
  `src/MLF/Elab/Pipeline.hs`,
  `src-public/MLF/Pipeline.hs`,
...
- Coordination notes: consume accepted `round-206` and its milestone-1
  ...
  before `Run/Pipeline` reaches the selected handoff code, consume live
  `round-211` as the continuing same-round baseline that already clears those
  two earlier stops, consume its preserved implementation notes as proof that
  the tightened `rev-005` `AAppF` continuation is a real dead-end, stay
  inside the exact `rev-006` continuation, keep the family within the
...
- Blocked predecessor notes: blocked `round-208` preserved fresh evidence in
  ...
  exact move was the combined round-211 continuation under
...
- Live continuation notes: live `round-211` remains on its existing
  ...
  `reifyInst` stop is gone, the post-annotation path now clears the earlier
  `PhiReorder: missing binder identity` detour, and the clean preserved
  baseline still fails at
  `Phase 7 (type checking): TCArgumentMismatch`
  on the authoritative application term. The preserved
  `implementation-notes.md` for that same round now proves more: an
  `AAppF`-only `Algebra.hs` repair can clear that first application mismatch,
  but it immediately exposes
  `Phase 7 (type checking): TCLetTypeMismatch`
  at the preserved let-scheme / authoritative handoff contract. The next
  exact move is therefore still same-round and still inside
  `src/MLF/Elab/Elaborate/Algebra.hs`, but broader than `AAppF` alone:
  preserve the current round-211 diff and continue only through the selected
  `ALetF` let-scheme finalization / closure seam around scheme selection,
  `closeTermWithSchemeSubstIfNeeded`, and
  `rhsFinal`, with `AAppF` context available only if needed inside that same
  file to keep the packet coherent.
...
- Direction id: `direction-2b-thread-the-selected-semantics-through-authoritative-pipeline-seams`
  ...
  and let/application construction so the selected behavior is visible where
  the family measures success, continuing the same round-211 packet from the
  preserved annotation-side baseline through the now-justified `ALetF`
  let-scheme finalization / closure seam in `Algebra.hs`, with same-file
  `AAppF` context available only if the let repair needs it.
  ...
  path before `Run/Pipeline` reaches that downstream handoff. Live
  `round-211` now clears both of those earlier stops on the same
  branch/worktree, but its preserved implementation notes also prove that an
  `AAppF`-only repair just trades the first Phase 7 `TCArgumentMismatch` for
  an immediate Phase 7 `TCLetTypeMismatch` against the preserved let scheme.
  The next move is therefore to preserve that current diff and repair only the
  remaining same-file let-scheme / closure seam.
  Preconditions: `milestone-1`; accepted progress from
  `direction-2a` via `round-207`; blocked predecessor evidence from
  `round-208`, `round-209`, and `round-210`; live round-211 baseline.
  ...
  `src/MLF/Elab/Elaborate/Algebra.hs`
  limited to the selected `ALetF` let-scheme finalization / closure logic
  around scheme selection,
  `closeTermWithSchemeSubstIfNeeded`, and
  `rhsFinal`, with `AAppF` context available only if the same-file let repair
  needs it. Keep
  `src/MLF/Elab/Run/Pipeline.hs`,
  `src/MLF/Elab/TermClosure.hs`,
  `src/MLF/Elab/Pipeline.hs`,
  ...
  logic, and touch the same focused tests only if the `ALetF` / closure
...
- Continuation id: `round-211-live-let-scheme-finalization-aletf-blocker`
  Direction: `direction-2b-thread-the-selected-semantics-through-authoritative-pipeline-seams`
  ...
  packet. The preserved `implementation-notes.md` for that same round now
  also proves that an `AAppF`-only repair can clear the first Phase 7
  `TCArgumentMismatch` only by exposing an immediate Phase 7
  `TCLetTypeMismatch` against the preserved let scheme / authoritative
  handoff contract. The next exact move is to continue on the same
  round-211 branch/worktree, preserve that current diff, and add only
  `src/MLF/Elab/Elaborate/Algebra.hs`
  limited to the selected `ALetF` let-scheme finalization / closure logic
  around scheme selection,
  `closeTermWithSchemeSubstIfNeeded`, and
  `rhsFinal`, with same-file `AAppF` context available only if needed.
