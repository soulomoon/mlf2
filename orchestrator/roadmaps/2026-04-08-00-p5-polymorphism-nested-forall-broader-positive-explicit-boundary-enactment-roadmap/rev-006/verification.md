...
- **milestone-2**
  ...
  - Verify the continuing round preserves the already-cleared Phase 6
    ...
    `round-211` baseline, treats the `rev-005` `AAppF` dead-end honestly,
    and repairs the remaining selected-packet contradiction by changing only
    the selected `ALetF` let-scheme finalization / closure logic around
    scheme selection,
    `closeTermWithSchemeSubstIfNeeded`, and
    `rhsFinal`, using same-file `AAppF` context only if the let repair needs
    it.
  - Verify
    `src/MLF/Elab/Elaborate/Annotation.hs` and
    `src/MLF/Elab/Legacy.hs`
    remain the preserved round-211 baseline and are not reopened as new
    continuation surfaces in this revision.
  - If the round touches
    `src/MLF/Elab/Elaborate/Algebra.hs`, verify the repair stays inside the
    selected `ALetF` let-scheme finalization / closure logic and any same-file
    `AAppF` context is only the minimum needed; it must not broaden other
  ...
  - Verify the authoritative packet is covered honestly on both entrypoints
    ...
    the preserved post-annotation handoff baseline stay cleared.
  - Verify
