...
the bounded `P1 local-recursive-shape authoritative-surface` successor loop.
...
- the guider may refine later pending items while staying inside the active
  bounded `P1` authoritative-surface family; and
- the guider may not silently reopen `C1`, the same-lane pocket, or `P5`,
  claim repo-level capability success, authorize cyclic search, second
  interfaces, fallback paths, or rollout without saying so explicitly.
