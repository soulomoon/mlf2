...
- `milestone-1` retries for:
  stale or incomplete inherited-boundary refreeze,
  missing or ambiguous reopened round-151 semantic status,
...
- `rev-002` is planning-only. Do not change `src/`, `src-public/`, `app/`,
  `test/`, or `mlf2.cabal`.
- Do not revise the inherited explicit-only / iso-recursive /
  ...
  honestly.
- Do not silently restore the accepted `round-151` reclassification to closed
  predecessor truth inside `rev-002`; the reopened semantic status must be
  handled only through fresh accepted authority in this family.
- Do not reopen the accepted retained-child clear-boundary settlement, the
  ...
  as live debt without fresh accepted authority inside this family.
