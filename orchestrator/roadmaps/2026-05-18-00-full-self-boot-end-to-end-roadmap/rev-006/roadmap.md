...
- Checker-facing run posture: future checker, checker-parity,
  compiler-source-package, driver, and conformance-validation rounds should
  prefer one aggregate public program run with labelled per-case
  output/evidence when multiple cases can share checker or program setup
  context. Multiple invocations remain lawful when semantic isolation,
  diagnostic boundaries, failure independence, stage-owned output isolation,
  or the public interface being tested requires separate runs.
- Execution granularity posture: planners own process classification for each
  selected round. They must record `execution_mode`, `complexity`, and
  `verification_profile` in `round-plan-record.json`. Standard delegated
  implementation and review remain the default, but simple, bounded,
  same-owner work may use `execution_mode: simple-direct` with focused
  verification when it does not need reviewer judgment, milestone closeout,
  semantic roadmap update, schema/contract changes, public contract changes,
  verification-meaning changes, or cross-owner integration.
...
- Future checker-related validation should batch public cases into one
  shared-context program run when the cases can honestly share checker/program
  setup. The aggregate run must expose labelled per-case output, diagnostics,
  spans, or failure evidence so public-interface checks and committed expected
  outputs remain reviewable. Split the cases into multiple invocations only
  when required by semantics, diagnostic boundaries, failure independence,
  stage-owned output isolation, or the public command surface being tested.
- Planners should batch closely related simple slices into one lawful round
  when they share the same owner surface, failure mode, and verification
  commands. Do not split one fixture, one wording cleanup, or one tiny parser
  harness adjustment into a separate delegated review round when a single
  focused slice can honestly cover the work.
- `simple-direct` is available only for simple, focused, same-owner work with a
  bounded direct write scope and no closeout or semantic-roadmap meaning. The
  planner may implement that selected simple task and write direct evidence,
  but the controller may skip implementer/reviewer dispatch only after the
  direct predicates in the control-plane schemas pass.
...
- round-325 completed item-325-parser-library-higher-order-function-field-extension: the shared parser-owned library now covers the bounded higher-order function-field fixture and malformed case-branch-arrow diagnostic; reviewer evidence passed focused parser checks, the rev-005 aggregate public batch, standalone fixture diff, cabal build all, cabal test, and thesis gate.
- round-326 completed item-326-parser-library-authoritative-recursive-let-extension: the shared parser-owned library now covers the bounded authoritative recursive-let fixture and malformed case-branch-arrow diagnostic; reviewer evidence passed focused parser checks, the rev-005 aggregate public batch, standalone fixture diff, cabal build all, cabal test, and thesis gate.
- round-327 completed item-327-parser-library-cross-module-let-polymorphism-extension: the shared parser-owned library now covers the bounded authoritative cross-module let-polymorphism fixture and malformed def-semicolon diagnostic; reviewer evidence passed focused parser checks, the rev-005 aggregate public batch, standalone fixture diff, cabal build all, cabal test, and thesis gate.
- round-328 completed item-328-parser-library-recursive-adt-plain-nat-extension: the shared parser-owned library now covers the bounded recursive-ADT plain Nat fixture and malformed case-branch-arrow diagnostic; reviewer evidence passed focused parser checks, static guards, the rev-005 aggregate public batch, standalone fixture diff, cabal build all, cabal test, and thesis gate.
- round-329 completed item-329-parser-library-recursive-list-tail-extension: the shared parser-owned library now covers the bounded recursive-list tail fixture and malformed case-branch-arrow diagnostic; reviewer evidence passed current diff hygiene, shortcut audit, standalone fixture diff, focused parser checks, the rev-005 aggregate public batch, full parser-parity group, cabal build all, cabal test, and thesis gate.
- round-330 completed item-330-parser-library-recursive-tree-extension: the shared parser-owned library covers bounded recursive-tree first-order and deriving parser-parity fixtures plus malformed case-branch-arrow evidence; reviewer evidence passed cleanup scope checks, shortcut audit, focused and aggregate parser-parity evidence, cabal build all, cabal test, and thesis gate.
...
After the shared parser library exists, remaining milestone-4 parser grammar
extensions should be planned at the natural parser-surface granularity rather
than one round per tiny fixture. The planner may batch adjacent grammar-family,
fixture harness, and diagnostic evidence changes when they share the same
parser-owned write surface and focused parser-parity verification. The planner
may choose `execution_mode: simple-direct` only for a strictly bounded parser
library or fixture-harness extension that stays within one owner surface,
records a focused parser-parity verification command, does not alter parser
public contracts or diagnostics policy, and does not claim milestone-4
closeout. Milestone-4 closeout, grammar architecture changes, shared parser
contract changes, and roadmap semantic updates still require the delegated
review path.
...
- Extraction notes: select `item-310-parser-library-consolidation` as the next
  ...
  Batch related parser-surface fixtures when they share a verification path.
  Use `simple-direct` only for simple, bounded, same-owner parser or fixture
  harness extensions with focused verification and no milestone closeout,
  semantic update, schema/contract change, or public diagnostic-policy change.
...
- Coordination notes: trusted substrate may provide filesystem, subprocess,
  ...
  semantics no longer fit one bounded execution front. Checker-facing
  validation groups that can share compiler-package setup should use aggregate
  public runs with labelled per-case evidence instead of repeated independent
  checker invocations.
...
- Extraction notes: choose vertical compiler components that produce
  ...
  implementation continues. For checker or checker-parity validation groups,
  prefer one aggregate public program run with labelled per-case evidence when
  the cases can share context; require separate invocations only for semantic,
  diagnostic, failure-independence, stage-isolation, or public-interface
  reasons.
...
- Coordination notes: `prove-self-boot` is not required in the first compiler
  ...
  driver work. Driver and conformance-runner validation should batch eligible
  checker-facing cases into a single public run with labelled per-case evidence
  when the command mode and semantics allow shared setup.
...
- Extraction notes: start with `check` and conformance execution, then native
  ...
  driver boundaries. When several `check` or conformance-execution cases share
  setup and command semantics, prefer an aggregate public run with labelled
  per-case output; keep separate invocations for command-specific public
  interface evidence, isolation, or independent failure records.
...
- Coordination notes: native object and executable bytes are regenerated and
  ...
  stage-equivalence closeout when they need separate evidence. Within a single
  stage, conformance-validation cases that can share checker/program setup
  should use aggregate public runs with labelled per-case evidence; this must
  not weaken stage-owned output isolation or cross-stage cache separation.
...
- Extraction notes: first runs may be proof-runner hardening rounds before the
  ...
  round. Conformance-first proof execution should batch eligible same-stage
  checker-facing cases into aggregate public runs only when per-case labels,
  diagnostics, and stage-owned output records remain explicit.
