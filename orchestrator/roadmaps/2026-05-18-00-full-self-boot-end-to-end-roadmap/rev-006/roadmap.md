...
- In scope: readiness ledger, shared conformance corpus, native-capable text
  substrate, canonical `.mlfp` parser parity, platform contract substrate,
  compiler source package, small self-boot driver, and first proof evidence.
- Out of scope: preserving legacy parser aliases, reopening settled ADR
  decisions, adding broad compatibility bridges, or claiming full independent
  self-hosting before the staged proof exists.
...
- `text-parser`: broad native-capable text substrate and canonical parser
  parity.
...
Intent: Make the canonical `.mlfp` parser accept and reject the language forms
needed by the shared corpus and future compiler source package.
Completion signal: Parser parity covers the required canonical syntax with
shared parser-library behavior and focused conformance evidence, including
negative cases and recursive tree forms.
...
parser parity slices currently merged into `master`.
Parallel lane: text-parser
...
- Direction id: direction-4a-canonical-parser-parity
  ...
  Extraction notes: Prefer the next smallest shared parser-library or
    conformance-backed syntax family that can be verified by one aggregate
    parser run.
