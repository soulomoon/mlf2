...
3. **Scope discipline**
   ...
   - Confirm future checker-facing plans prefer one aggregate public program
     run with labelled per-case output/evidence when multiple cases can share
     checker or program setup context. Separate `check-program`, `run-program`,
     or equivalent checker-facing invocations are valid only when the plan
     names a semantic-isolation, diagnostic-boundary, failure-independence,
     stage-owned-output-isolation, or public-interface reason.
...
4. **Lineage and closeout**
   - Confirm `selection-record.json`, `round-plan-record.json`, and the
     finalization artifact for the selected execution mode name the active
     `roadmap_id`, `roadmap_revision`, and `roadmap_dir`.
   - For delegated rounds, confirm `review-record.json` validates against
     `orchestrator/round-finalization-schema.md`.
   - For `simple-direct` rounds, confirm `simple-direct-record.json` validates
     against `orchestrator/round-finalization-schema.md`.
...
5. **Execution profile**
   - Confirm the planner recorded `execution_mode`, `complexity`, and
     `verification_profile` in `round-plan-record.json` or the plan's
     `Execution Profile`.
   - `focused` verification is valid only for simple, bounded, same-owner work
     whose direct or delegated checks exercise the changed public behavior or
     control-plane surface. It must still include `git diff --check`.
   - `standard` verification is the default for behavior-changing delegated
     implementation rounds: run the focused slice named by the plan, the
     relevant task-specific checks, and any roadmap-required gates for touched
     semantics.
   - `closeout` verification is required for milestone closeout, completion
     pointers, status changes, semantic roadmap updates, proof/readiness
     claims, broad parser architecture changes, public contract changes, or
     verification-meaning changes.
   - For `execution_mode: simple-direct`, confirm
     `complexity: simple`, `verification_profile: focused`,
     `worker_mode: none`, bounded `direct_write_scope`,
     planner-authored `implementation-notes.md`, and
     `simple-direct-record.json` with passing verification commands. Direct
     rounds may not change milestone status, completion pointers, roadmap
     history, role prompts, schemas, public contracts, verification meaning, or
     semantic roadmap content.
...
- Aggregated checker-facing runs must remain public-interface evidence: each
  case needs labelled output, diagnostics, spans, or failure evidence, and
  committed expected outputs must remain reviewable instead of becoming an
  opaque aggregate.
...
- **milestone-2 (Shared File-Based Conformance Corpus)**
  ...
  - For any future conformance-validation expansion or repair, verify
    checker-facing cases that can share package/checker setup use one aggregate
    public run with labelled per-case evidence. Keep separate invocations when
    fixture metadata, expected-output boundaries, or pass/fail independence
    requires them.
...
- **milestone-4 (Full Canonical `.mlfp` Parser Parity)**
  ...
  - When parser-parity public evidence runs through a checker-facing `.mlfp`
    program, verify eligible fixture groups use a shared-context aggregate run
    with labelled per-case sections, without turning parser parity into checker
    implementation scope.
  - For remaining simple parser-library grammar or fixture-harness extensions,
    verify the planner batched adjacent same-surface fixtures when they share a
    focused parser-parity command, failure mode, and owner surface. Reject
    needless one-fixture delegated rounds when a bounded focused batch would be
    clearer.
  - For a planner-authored `simple-direct` parser slice, verify the diff stays
    within parser-owned library or thin fixture-harness scope, records focused
    parser-parity evidence, and does not alter parser public contracts,
    diagnostic policy, roadmap semantics, or milestone-4 closeout.
...
- **milestone-6 (Compiler Source Package In `.mlfp`)**
  ...
  - Verify checker, checker-parity, and compiler-source-package validation
    groups prefer a single public program run with labelled per-case evidence
    when cases can share setup/checker context. Require plans using multiple
    invocations to explain the semantic, diagnostic, failure-independence,
    stage-isolation, or public-interface reason.
...
- **milestone-7 (Small Real Self-Boot Driver)**
  ...
  - Verify driver and conformance-runner checks batch eligible checker-facing
    cases into one aggregate public run with labelled per-case output/evidence
    when command semantics allow shared setup. Separate command invocations
    remain valid for command-surface evidence, diagnostic isolation, or
...
- **milestone-8 (First Self-Boot Proof)**
  ...
  - Verify same-stage conformance-validation groups use aggregate public runs
    only when per-case labels, diagnostics, hashes, and stage-owned output
    records remain explicit. Never batch across stage boundaries or reuse
    cross-stage checker, semantic, backend, object, executable, link, native
    execution, or conformance-output caches.
...
- For future checker-facing validation plans, manually confirm any repeated
  public program invocations are justified by semantic isolation, diagnostic
  boundaries, failure independence, stage-owned output isolation, or the public
  interface under test. Otherwise expect one aggregate public run with labelled
  per-case evidence.
- For `simple-direct`, manually confirm the planner-owned
  `direct_write_scope`, `implementation-notes.md`, and
  `simple-direct-record.json` match the actual diff and that every recorded
...
- For milestone 4 after the active round-309 recovery/closeout decision, the
  ...
  update.
- After the shared milestone-4 parser library is established, planners should
  batch related simple parser-surface fixtures instead of spending a full
  delegated round on each tiny same-surface extension. `simple-direct` may be
  used only for bounded parser-library or thin fixture-harness changes with
  focused parser-parity verification and no milestone closeout, semantic
  roadmap update, public parser contract change, diagnostic-policy change,
  schema change, role-prompt change, or cross-owner integration.
- For future checker, checker-parity, compiler-source-package, driver, and
  conformance-validation planning, the controller and reviewer should classify
  repeated checker-facing program invocations for otherwise shared-context
  cases as requiring replanning unless the plan records an isolation or
  public-interface reason. Aggregate runs must keep labelled per-case evidence
  and committed expected outputs explicit.
