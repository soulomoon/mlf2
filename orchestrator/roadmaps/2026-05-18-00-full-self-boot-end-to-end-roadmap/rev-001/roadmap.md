...
- Compatibility posture: backwards compatibility is not a default goal.
  ...
  contract.
- Verification standard: every behavior-changing round runs focused tests
  first, then `cabal build all && cabal test` before closeout. Rounds touching
  thesis obligations, language semantics, self-boot readiness, proof evidence,
  or platform contracts also run `./scripts/thesis-conformance-gate.sh`.
...
### [pending] 1. Readiness Ledger Baseline
