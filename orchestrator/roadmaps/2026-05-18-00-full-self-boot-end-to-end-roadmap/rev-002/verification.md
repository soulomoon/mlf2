...
1. **Build, test, and thesis gate**
   - For behavior-changing implementation work, use the `tdd` skill. Each
     selected behavior slice must follow a vertical RED -> GREEN -> refactor
     cycle: write one public-interface behavior test, confirm it fails,
     implement only enough code to pass, and refactor only while the focused
     slice is green.
...
3. **Scope discipline**
   ...
   - Confirm behavior-changing implementation plans are vertical TDD slices
     with a first public behavior and focused failing test named before coding.
     If the only available plan is horizontal, require replanning or semantic
...
- For behavior-changing implementation rounds, manually inspect
  ...
  and reject horizontal all-tests-then-all-code batches.
...
- If a retry or next selection reveals that a coarse milestone needs smaller
  ...
  implementation.
- The `tdd` skill requirement applies to behavior-changing implementation
  rounds only. Pure docs-only, control-plane-only, review-only, semantic
  roadmap-update, and status-only closeout rounds are exempt, but their normal
  artifact, diff hygiene, and claim-audit checks still apply.
- A planner may not select a behavior-changing implementation round unless it
  can name the first public-interface behavior and focused RED test slice.
  If the work can only be expressed as a horizontal layer, split or resequence
  through semantic-update mode before implementation.
