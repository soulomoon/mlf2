...
the automatic iso-recursive type inference implementation loop.
...
- All implementation items (`item-1` through `item-5`) may retry inside the
  same round when a reviewer rejects with fixable issues.
- `item-6` (docs-only) retries only for formatting or content defects.
