...
### Verification
- Reviewer approved `round-361` with no retry target.
- Roadmap closeout mode: none.
- `git diff --check`: pass.
- Focused platform contract and package lock tests: pass.
- Static platform package-lock guard: pass with `round-361 platform package-lock static guard passed`.
- Full Cabal gate: `ghcup run --ghc 9.14.1 -- cabal build all && ghcup run --ghc 9.14.1 -- cabal test`, pass with 2752 examples, 0 failures.
