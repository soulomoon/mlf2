# Round 216 Plan (rev-019 / milestone-3 `sameLaneQuintupleAliasFrameClearBoundaryExpr` promotion)

**Goal:** Promote `sameLaneQuintupleAliasFrameClearBoundaryExpr` from inherited
...
sextuple / deeper alias shells stay outside this extraction.

**Architecture:** Start from merged base-branch `HEAD = 1b62ad5` and treat the
merged `round-211` / `round-212` / `round-213` / `round-214` / `round-215`
mechanism as fixed baseline truth. The default move is evidence-surface
promotion inside
`test/Research/P5ClearBoundarySpec.hs`,
`test/PipelineSpec.hs`, and
`test/ElaborationSpec.hs`:
publish the exact quintuple-alias packet as the next explicit milestone-3
anchor, prefer sharpening the existing pipeline continuity row rather than
creating a second pipeline packet, and keep sextuple / deeper alias shells
...
- Keep this round on the exact packet
  `sameLaneQuintupleAliasFrameClearBoundaryExpr` only.
...
- Keep `sameLaneSextupleAliasFrameClearBoundaryExpr` and deeper alias shells
...
- Default to test/evidence-surface promotion first; production edits are
  lawful only after the explicit quintuple-alias packet is shown failing and a
...
- Verify-only: `orchestrator/worktrees/round-216/orchestrator/state.json`
- Verify-only:
  `orchestrator/worktrees/round-216/orchestrator/rounds/round-216/selection.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-019/roadmap.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-019/verification.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-019/retry-subloop.md`
- Verify-only: `orchestrator/rounds/round-215/review-record.json`
- Verify-only: `orchestrator/rounds/round-215/merge.md`
- Verify-only: `TODO.md`
...
- [ ] Confirm controller state, canonical round-worktree state, round
      ...
      `rev-019` lineage and the extracted item
      `promote-same-lane-quintuple-alias-clear-boundary-packet-to-next-explicit-milestone-3-representative-corpus-anchor`.
- [ ] Re-read merged `round-215` review / merge evidence so the preserved
      baseline is explicit before any edits:
      `sameLaneQuadrupleAliasFrameClearBoundaryExpr` is already the merged next
      explicit milestone-3 anchor after the merged triple-alias anchor, the
      promotion was test-only, and the milestone-2 production mechanism stayed
      the controlling baseline.
- [ ] Re-read the `rev-019` roadmap / verification / retry bundle and the
      ...
      `sameLaneQuadrupleAliasFrameClearBoundaryExpr` are merged baseline truth,
      ...
      and the next still-unpublished packet is the exact quintuple-alias lane.
- [ ] Inspect
      `test/Research/P5ClearBoundarySpec.hs`,
      `test/PipelineSpec.hs`, and
      `test/ElaborationSpec.hs`
      to confirm the live planning gap:
      `test/PipelineSpec.hs` already carries the bounded quintuple-alias
      continuity row plus sextuple/deeper continuity guards, while the
      research surface does not yet publish quintuple-alias as the next
      explicit milestone-3 anchor and the elaboration surface does not yet
      carry the matching exact-edge authoritative-instantiation guard.
...
python3 -m json.tool orchestrator/worktrees/round-216/orchestrator/state.json >/dev/null
rg -n 'roadmap_id|roadmap_revision|roadmap_dir|milestone-3|direction-3a|promote-same-lane-quintuple-alias-clear-boundary-packet' orchestrator/state.json orchestrator/worktrees/round-216/orchestrator/state.json orchestrator/worktrees/round-216/orchestrator/rounds/round-216/selection.md
rg -n 'sameLaneClearBoundaryExpr|sameLaneAliasFrameClearBoundaryExpr|sameLaneDoubleAliasFrameClearBoundaryExpr|sameLaneTripleAliasFrameClearBoundaryExpr|sameLaneQuadrupleAliasFrameClearBoundaryExpr|sameLaneQuintupleAliasFrameClearBoundaryExpr|sameLaneSextupleAliasFrameClearBoundaryExpr|selected same-wrapper nested-forall' test/Research/P5ClearBoundarySpec.hs test/PipelineSpec.hs test/ElaborationSpec.hs
git -C orchestrator/worktrees/round-216 diff --name-only -- src/MLF/Elab/TermClosure.hs src/MLF/Elab/Run/Pipeline.hs src/MLF/Elab/Pipeline.hs src-public/MLF/Pipeline.hs src/MLF/Elab/Run/ResultType/Fallback.hs src/MLF/Elab/Run/ResultType/Fallback/Core.hs
```

### Task 2: Promote the exact quintuple-alias packet across the three milestone-3 review surfaces
...
- [ ] In `test/Research/P5ClearBoundarySpec.hs`, add
      `sameLaneQuintupleAliasFrameClearBoundaryExpr` plus
      `sameLaneQuintupleAliasFrameClearBoundaryFallbackType`, then publish the
      ...
      new live packet (`sameLaneQuintupleAliasFrameClearBoundaryExpr`),
      then preserved selected same-wrapper nested-`forall`.
- [ ] In `test/PipelineSpec.hs`, sharpen the existing
      `sameLaneQuintupleAliasFrameClearBoundaryExpr` entrypoint row into the
      ...
      over adding a second quintuple-alias packet. Leave
      `sameLaneQuadrupleAliasFrameClearBoundaryExpr` explicitly described as
      the merged next anchor after the triple-alias anchor, and leave the
      separate sextuple / deeper alias-shell rows plus the shared helper /
...
- [ ] In `test/ElaborationSpec.hs`, add the exact-edge authoritative
      ...
      `sameLaneQuintupleAliasFrameClearBoundaryExpr` immediately after the
      quadruple-alias guard. Mirror the existing packet extractor shape,
      extend it to the
      `k -> hold -> keep -> more -> deep -> tail -> u`
      quintuple-alias form, and lock the concrete
      ...
      into generic alias-depth helpers or sextuple-alias coverage.
- [ ] If local cleanup is needed to keep the tests readable, keep it inside
      ...
      production surface, or move sextuple / deeper alias shells onto the
...
- [ ] If the newly explicit quintuple-alias rows fail during authoritative
      ...
      handoff. The repair must only make the exact quintuple-alias packet earn
...
- [ ] If the failure instead appears after annotation translation, localize
      ...
      `rhsAbs0`, and
      `rhsAbs`
      so the exact quintuple-alias packet survives the admitted
      post-annotation seam on both authoritative entrypoints without changing
      the already merged first four anchors.
...
- [ ] After any repair, rerun the exact packet and preserved-anchor cluster
      ...
      that, the merged quadruple-alias next anchor after that, alias-frame
      predecessor truth, selected same-wrapper nested-`forall` success,
      checked-authoritative parity, or the fail-closed
      `P2` / `N1` / `N2` / `N6` shell probes.

**Verification commands:**

```bash
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneQuintupleAliasFrameClearBoundaryExpr exact edge authoritative instantiation translation"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneQuintupleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneQuadrupleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneTripleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneDoubleAliasFrameClearBoundaryExpr"'
...
- [ ] Re-run the exact packet evidence proving
      `sameLaneQuintupleAliasFrameClearBoundaryExpr` now succeeds honestly on
...
- [ ] Re-run the no-sextuple boundary evidence:
      `sameLaneSextupleAliasFrameClearBoundaryExpr` and deeper alias shells
      must stay continuity-only and green in `test/PipelineSpec.hs`, and
      sextuple / deeper alias-shell names must remain absent from
      `test/Research/P5ClearBoundarySpec.hs` and
      `test/ElaborationSpec.hs`.
- [ ] Verify diff hygiene and scope discipline against merge-base `1b62ad5`.
...
- [ ] Run the full gates. Expected end state:
      ...
      `1350 examples` baseline to `1353 examples, 0 failures`.
...
base=$(git -C orchestrator/worktrees/round-216 merge-base codex/automatic-recursive-type-inference HEAD)
git -C orchestrator/worktrees/round-216 diff --name-only "$base" -- test/Research/P5ClearBoundarySpec.hs test/PipelineSpec.hs test/ElaborationSpec.hs src/MLF/Elab/Elaborate/Annotation.hs src/MLF/Elab/Elaborate/Algebra.hs src/MLF/Elab/Legacy.hs
git -C orchestrator/worktrees/round-216 diff --name-only "$base" -- src/MLF/Elab/TermClosure.hs src/MLF/Elab/Run/Pipeline.hs src/MLF/Elab/Pipeline.hs src-public/MLF/Pipeline.hs src/MLF/Elab/Run/ResultType/Fallback.hs src/MLF/Elab/Run/ResultType/Fallback/Core.hs
git -C orchestrator/worktrees/round-216 diff --check "$base" --
...
- `sameLaneQuintupleAliasFrameClearBoundaryExpr` is explicitly named as the
...
- Sextuple / deeper alias shells remain continuity-only rather than newly
