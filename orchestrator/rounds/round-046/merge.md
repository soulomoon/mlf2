# Round `round-046` Merge Preparation (`G1`)

## Proposed Squash Commit Title

`docs(g1): bind next multi-inst bounded target`

## Summary

- The accepted `G1` round is docs-only and finalizes the next bounded bind for
  repaired `URI-R2-C1` after the accepted local-binding scheme-alias/base-like
  `F2` / `F3` baseline and accepted `F4 = continue-bounded` outcome.
- `docs/plans/2026-03-19-uri-r2-c1-g1-next-target-bind.md` freezes exactly one
  future `G2` slice: the local-binding `rootHasMultiInst`
...
- The accepted artifact preserves the inherited explicit-only /
  ...
  boundary, keeps accepted `U2` / `U3` / `U4` negatives binding, and leaves
  replay reopen, `MLF.Elab.Inst`, `InstBot`, `boundVarTarget` widening,
  non-local widening, and `instArgRootMultiBase` out of scope.
- `orchestrator/rounds/round-046/implementation-notes.md` records that the
  accepted retry changed only stale `selection.md` facts; the canonical `G1`
  artifact path, selected `rootHasMultiInst` family, and docs-only full-gate
  skip remained unchanged across the retry.

## Review And Retry Consistency Check

- `orchestrator/rounds/round-046/review.md` matches
  `orchestrator/rounds/round-046/reviews/attempt-2.md`, and the latest review
  snapshot records `Implemented stage result: pass`, `Attempt verdict:
  accepted`, `Stage action: finalize`, `Retry reason: none`, and `Fix
  hypothesis: none`.
- `orchestrator/rounds/round-046/review-record.json` is authoritative and
  ...
  `review_snapshot: orchestrator/rounds/round-046/reviews/attempt-2.md`, and
...
- `orchestrator/rounds/round-046/attempt-log.jsonl` is internally consistent
  with that authoritative record and the preserved review snapshots: `attempt-1`
  records the rejected `selection-doc-stale-facts`, and `attempt-2` records the
  accepted finalization with
  `review_snapshot: orchestrator/rounds/round-046/reviews/attempt-2.md` and
  the same canonical
  `docs/plans/2026-03-19-uri-r2-c1-g1-next-target-bind.md` artifact path named
  by `review-record.json`.

## Readiness Statement

Round `round-046` is ready for squash merge preparation. The latest review
snapshot is lawful `accepted + finalize`, the authoritative review record and
retry summary match it, and the accepted round satisfies the `G1` stage gate as
...
- This round preserves continuity from completed rounds `round-001` through
  `round-045`, the recursive-types packet, the replay-repair track, the
  ...
  and the accepted `F1` / `F2` / `F3` / `F4` chain in `round-042` through
  `round-045`.
- The live subject remains repaired `URI-R2-C1`; accepted
  `F4 = continue-bounded` remains the controlling predecessor outcome; and the
  accepted local-binding scheme-alias/base-like `F2` / `F3` baseline remains
  inherited context only while `G1` binds the next adjacent local-binding
...
- Controller/guider-owned post-merge work should treat the accepted `G1`
  artifact plus `review-record.json` as the authoritative bind for `G2` and
  preserve the frozen local-binding `rootHasMultiInst`
  `keepTargetFinal` / `targetC` target exactly.
- `instArgRootMultiBase` remains the only other unopened `keepTargetFinal`
