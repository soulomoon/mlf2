# Merge Preparation (`round-217` / `milestone-3` / `direction-3a`)
...
- `roadmap_item_id`: absent in the active `rev-020` state / selection / review lineage for this round
- `milestone_id`: `milestone-3`
- `direction_id`: `direction-3a-expand-the-broader-positive-representative-corpus`
- `extracted_item_id`: `promote-same-lane-sextuple-alias-clear-boundary-packet-to-next-explicit-milestone-3-representative-corpus-anchor`

## Round Context

- Base branch: `codex/automatic-recursive-type-inference`
- Round branch: `orchestrator/round-217-promote-p5-sextuple-alias-clear-boundary-anchor`
- Canonical worktree: `orchestrator/worktrees/round-217`
...
- Merge the approved `milestone-3` / `direction-3a` extraction that promotes
  `sameLaneSextupleAliasFrameClearBoundaryExpr` from inherited bounded
  guard/control evidence to the next explicit broader-positive representative
  corpus anchor after the merged quintuple-alias anchor.
- Keep the squash substance honest and bounded to the approved implementation
  payload in:
  `test/Research/P5ClearBoundarySpec.hs`,
  `test/PipelineSpec.hs`, and
  `test/ElaborationSpec.hs`,
  plus the intended round-local notes under `orchestrator/rounds/round-217/`.
  No production files changed for this round.
- On the research surface, the round adds the sextuple-alias fallback anchor
  and both authoritative-entrypoint checks while preserving
  ...
  success.
- On the pipeline surface, the round promotes the existing sextuple-alias row
  into the explicit milestone-3 representative check on both authoritative
  entrypoints while keeping septuple/deeper alias shells as continuity-only
  evidence outside this extraction.
- On the elaboration surface, the round adds the exact-edge authoritative
  instantiation guard for the sextuple-alias packet, pinning
  `ExpInstantiate [NodeId 49]` together with
  `InstSeq (InstApp (TVar "t50")) (InstApp (TVar "t56"))`.
...
- `orchestrator/rounds/round-217/review.md` records
...
- `orchestrator/rounds/round-217/review-record.json` preserves the same
...
- Merge readiness: confirmed, provided the squash stays limited to the three
  ...
  `orchestrator/rounds/round-217/`, including this `merge.md`.
...
- No merge-order blocker is visible in the active `rev-020` roadmap:
...
- Base freshness is exact for the recorded local squash target.
  `HEAD`,
  `orchestrator/round-217-promote-p5-sextuple-alias-clear-boundary-anchor`,
  `codex/automatic-recursive-type-inference`, and
  `git merge-base codex/automatic-recursive-type-inference HEAD`
  all resolve to `21fddba470a78569c20ffee7fb98ff5011053ff5`, and
...
- The existing remote-tracking snapshot is older, not fresher:
  `origin/codex/automatic-recursive-type-inference` resolves to
  `fde4339b0c8c554ede0d246129bd2c9bf5d4c112`,
  `git rev-list --left-right --count codex/automatic-recursive-type-inference...origin/codex/automatic-recursive-type-inference`
  reports `290 0`, and
  `git rev-list --left-right --count origin/codex/automatic-recursive-type-inference...HEAD`
  reports `0 290`.
  That older tracking ref is not a blocker for the requested local squash
  merge.
- `round-217` is ready for squash merge.
...
- Post-merge summaries should keep the milestone-3 ordering explicit:
  ...
  `sameLaneSextupleAliasFrameClearBoundaryExpr` becomes the next explicit
...
- Keep later milestone-3 and milestone-4 work honest about what did not land
  ...
  success,
  septuple/deeper alias shells remain continuity-only,
