# Round 217 Plan (rev-020 / milestone-3 `sameLaneSextupleAliasFrameClearBoundaryExpr` promotion)

**Goal:** Promote `sameLaneSextupleAliasFrameClearBoundaryExpr` from inherited
...
the preserved `sameLaneAliasFrameClearBoundaryExpr` predecessor lane, the
merged selected same-wrapper nested-`forall` win, checked-authoritative
parity, the fail-closed `P2` / `N1` / `N2` / `N6` shell probes, and the
bounded rule that septuple / deeper alias shells stay outside this
extraction.

**Architecture:** Start from merged base-branch `HEAD = 21fddba` and treat the
merged `round-211` / `round-212` / `round-213` / `round-214` / `round-215` /
`round-216` mechanism as fixed baseline truth. The default move is
evidence-surface promotion inside
`test/Research/P5ClearBoundarySpec.hs`,
`test/PipelineSpec.hs`, and
`test/ElaborationSpec.hs`:
publish the exact sextuple-alias packet as the next explicit milestone-3
anchor, prefer sharpening the existing pipeline continuity row rather than
creating a second pipeline packet, and keep septuple / deeper alias shells
...
- Keep this round on the exact packet
  `sameLaneSextupleAliasFrameClearBoundaryExpr` only.
...
- Keep `sameLaneSeptupleAliasFrameClearBoundaryExpr` and deeper alias shells
...
- Default to test/evidence-surface promotion first; production edits are
  lawful only after the explicit sextuple-alias packet is shown failing and a
...
- Verify-only: `orchestrator/worktrees/round-217/orchestrator/state.json`
- Verify-only:
  `orchestrator/worktrees/round-217/orchestrator/rounds/round-217/selection.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-020/roadmap.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-020/verification.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-020/retry-subloop.md`
- Verify-only: `orchestrator/rounds/round-216/review-record.json`
- Verify-only: `orchestrator/rounds/round-216/merge.md`
- Verify-only: `TODO.md`
...
- [ ] Confirm controller state, canonical round-worktree state, round
      ...
      `rev-020` lineage and the extracted item
      `promote-same-lane-sextuple-alias-clear-boundary-packet-to-next-explicit-milestone-3-representative-corpus-anchor`.
- [ ] Re-read merged `round-216` review / merge evidence so the preserved
      baseline is explicit before any edits:
      `sameLaneQuintupleAliasFrameClearBoundaryExpr` is already the merged
      next explicit milestone-3 anchor after the merged quadruple-alias
      anchor, the promotion was test-only, and the milestone-2 production
      mechanism stayed the controlling baseline.
- [ ] Re-read the `rev-020` roadmap / verification / retry bundle and the
      ...
      `sameLaneQuintupleAliasFrameClearBoundaryExpr` are merged baseline
      truth,
...
- [ ] Inspect
      ...
      `sameLaneSextupleAliasFrameClearBoundaryExpr` continuity row plus
      septuple/deeper continuity guards, while the research surface does not
      yet publish sextuple-alias as the next explicit milestone-3 anchor and
      the elaboration surface does not yet carry the matching exact-edge
...
python3 -m json.tool orchestrator/worktrees/round-217/orchestrator/state.json >/dev/null
rg -n 'roadmap_id|roadmap_revision|roadmap_dir|milestone-3|direction-3a|promote-same-lane-sextuple-alias-clear-boundary-packet' orchestrator/state.json orchestrator/worktrees/round-217/orchestrator/state.json orchestrator/worktrees/round-217/orchestrator/rounds/round-217/selection.md
...
git -C orchestrator/worktrees/round-217 diff --name-only -- src/MLF/Elab/TermClosure.hs src/MLF/Elab/Run/Pipeline.hs src/MLF/Elab/Pipeline.hs src-public/MLF/Pipeline.hs src/MLF/Elab/Run/ResultType/Fallback.hs src/MLF/Elab/Run/ResultType/Fallback/Core.hs
```

### Task 2: Promote the exact sextuple-alias packet across the three milestone-3 review surfaces
...
- [ ] In `test/Research/P5ClearBoundarySpec.hs`, add
      `sameLaneSextupleAliasFrameClearBoundaryExpr` plus
      `sameLaneSextupleAliasFrameClearBoundaryFallbackType`, then publish the
      ...
      new live packet (`sameLaneSextupleAliasFrameClearBoundaryExpr`),
      then preserved selected same-wrapper nested-`forall`.
- [ ] In `test/PipelineSpec.hs`, sharpen the existing
      `sameLaneSextupleAliasFrameClearBoundaryExpr` entrypoint row into the
      ...
      over adding a second sextuple-alias packet. Leave
      `sameLaneQuintupleAliasFrameClearBoundaryExpr` explicitly described as
      the merged next anchor after the quadruple-alias anchor, and leave the
      separate septuple / deeper alias-shell rows plus the shared helper /
...
- [ ] In `test/ElaborationSpec.hs`, add the exact-edge authoritative
      ...
      `sameLaneSextupleAliasFrameClearBoundaryExpr` immediately after the
      quintuple-alias guard. Mirror the existing packet extractor shape,
      extend it to the
      `k -> hold -> keep -> more -> deep -> tail -> leaf -> u`
      sextuple-alias form, and lock the concrete
      ...
      into generic alias-depth helpers or septuple-alias coverage.
- [ ] If local cleanup is needed to keep the tests readable, keep it inside
      ...
      production surface, or move septuple / deeper alias shells onto the
...
- [ ] If the newly explicit sextuple-alias rows fail during authoritative
      ...
      handoff. The repair must only make the exact sextuple-alias packet earn
...
- [ ] If the failure instead appears after annotation translation, localize
      ...
      `rhsAbs0`, and
      `rhsAbs`
      so the exact sextuple-alias packet survives the admitted
      post-annotation seam on both authoritative entrypoints without changing
      the already merged first five anchors.
...
- [ ] After any repair, rerun the exact packet and preserved-anchor cluster
      ...
      quintuple-alias next anchor after that, alias-frame predecessor truth,
      selected same-wrapper nested-`forall` success, checked-authoritative
      parity, or the fail-closed `P2` / `N1` / `N2` / `N6` shell probes.

**Verification commands:**

```bash
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneSextupleAliasFrameClearBoundaryExpr exact edge authoritative instantiation translation"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneSextupleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneQuintupleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneQuadrupleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneTripleAliasFrameClearBoundaryExpr"'
...
- [ ] Re-run the exact packet evidence proving
      `sameLaneSextupleAliasFrameClearBoundaryExpr` now succeeds honestly on
...
- [ ] Re-run the no-septuple boundary evidence:
      `sameLaneSeptupleAliasFrameClearBoundaryExpr` and deeper alias shells
      must stay continuity-only and green in `test/PipelineSpec.hs`, and
      septuple / deeper alias-shell names must remain absent from
      `test/Research/P5ClearBoundarySpec.hs` and
      `test/ElaborationSpec.hs`.
- [ ] Verify diff hygiene and scope discipline against merge-base `21fddba`.
...
- [ ] Run the full gates. Expected end state:
      ...
      `1353 examples` baseline to `1356 examples, 0 failures`.
...
cabal test mlf2-test --test-show-details=direct --test-options='--match "fail-closed once it leaves the local TypeRef lane"'
rg -n 'sameLaneSeptupleAliasFrameClearBoundaryExpr|sameLaneOctupleAliasFrameClearBoundaryExpr|sameLaneNonupleAliasFrameClearBoundaryExpr' test/Research/P5ClearBoundarySpec.hs test/ElaborationSpec.hs test/PipelineSpec.hs
base=$(git -C orchestrator/worktrees/round-217 merge-base codex/automatic-recursive-type-inference HEAD)
git -C orchestrator/worktrees/round-217 diff --name-only "$base" -- test/Research/P5ClearBoundarySpec.hs test/PipelineSpec.hs test/ElaborationSpec.hs src/MLF/Elab/Elaborate/Annotation.hs src/MLF/Elab/Elaborate/Algebra.hs src/MLF/Elab/Legacy.hs
git -C orchestrator/worktrees/round-217 diff --name-only "$base" -- src/MLF/Elab/TermClosure.hs src/MLF/Elab/Run/Pipeline.hs src/MLF/Elab/Pipeline.hs src-public/MLF/Pipeline.hs src/MLF/Elab/Run/ResultType/Fallback.hs src/MLF/Elab/Run/ResultType/Fallback/Core.hs
git -C orchestrator/worktrees/round-217 diff --check "$base" --
...
- `sameLaneSextupleAliasFrameClearBoundaryExpr` is explicitly named as the
...
- Septuple / deeper alias shells remain continuity-only rather than newly
