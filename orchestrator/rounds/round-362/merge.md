...
### Verification
- Reviewer approved `round-362` with no retry target.
- Roadmap closeout mode: none.
- `git diff --check`: pass.
- Focused platform contract and native link record tests: pass.
- Static platform native-link-record guard: pass with `round-362 platform native-link-record static guard passed`.
- Full Cabal gate: `ghcup run --ghc 9.14.1 -- cabal build all && ghcup run --ghc 9.14.1 -- cabal test`, pass with 2757 examples, 0 failures.
