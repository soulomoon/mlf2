...
- Baseline checks:
  ...
  - `rg -n '"contract_version": 2|"retry": null|"retry": \{' orchestrator/rounds/round-033/state-snapshot.json` -> pass (`2:  "contract_version": 2,`, `13:  "retry": null`).
  - `rg -n '^\d+\. \[(pending|in-progress|done)\]' orchestrator/roadmaps/2026-03-14-01-unannotated-iso-recursive-inference-successor-roadmap/rev-011/roadmap.md` -> pass (ordered `U1` through `U6` list intact, `U6` still pending pre-merge).
...
- Task-specific checks:
  ...
  - `U6-SUBJECT-BOUND` -> pass: the artifact keeps repaired `URI-R2-C1` as the only live subject, and `git status --short --untracked-files=all -- src src-public app test mlf2.cabal orchestrator/rounds/round-033/state-snapshot.json orchestrator/roadmaps/2026-03-14-01-unannotated-iso-recursive-inference-successor-roadmap/rev-011/roadmap.md Bugs.md` returned no output, so no production/test/controller drift or silent widening path landed.
  ...
  - `U6-FILES-CHANGED` -> fail: the plan requires the artifact to record the exact files changed by round `033`, but [docs/plans/2026-03-17-uri-r2-c1-u6-next-widening-decision-gate.md](docs/plans/2026-03-17-uri-r2-c1-u6-next-widening-decision-gate.md#L85) through [docs/plans/2026-03-17-uri-r2-c1-u6-next-widening-decision-gate.md](docs/plans/2026-03-17-uri-r2-c1-u6-next-widening-decision-gate.md#L92) list only the U6 artifact and `implementation-notes.md`, while `git status --short --untracked-files=all -- docs/plans/2026-03-17-uri-r2-c1-u6-next-widening-decision-gate.md orchestrator/rounds/round-033` also shows `orchestrator/rounds/round-033/plan.md` and `orchestrator/rounds/round-033/selection.md`. The recorded file list is therefore not exact and does not satisfy Task 3 / Task 5 of the round plan.
