# Merge Preparation (`round-218` / `milestone-3` / `direction-3a`)
...
- `roadmap_item_id`: absent in the active `rev-021` state / selection / review lineage for this round
- `milestone_id`: `milestone-3`
- `direction_id`: `direction-3a-expand-the-broader-positive-representative-corpus`
- `extracted_item_id`: `promote-same-lane-septuple-alias-clear-boundary-packet-to-next-explicit-milestone-3-representative-corpus-anchor`

## Round Context

- Base branch: `codex/automatic-recursive-type-inference`
- Round branch: `orchestrator/round-218-promote-p5-septuple-alias-clear-boundary-anchor`
- Canonical worktree: `orchestrator/worktrees/round-218`
...
- Merge the approved `milestone-3` / `direction-3a` extraction that promotes
  `sameLaneSeptupleAliasFrameClearBoundaryExpr` from inherited bounded
  guard/control evidence to the next explicit broader-positive representative
  corpus anchor after the merged sextuple-alias anchor.
- Keep the squash payload bounded to the approved implementation diff in:
  `test/Research/P5ClearBoundarySpec.hs`,
  `test/PipelineSpec.hs`, and
  `test/ElaborationSpec.hs`,
  plus the intended round-local notes under `orchestrator/rounds/round-218/`.
  No production files changed for this round.
- On the research surface, the round adds the septuple-alias fallback anchor,
  the exact septuple packet fixture, and the paired authoritative-entrypoint
  ...
  success.
- On the pipeline surface, the round sharpens the existing septuple-alias row
  into the live milestone-3 representative check on both authoritative
  entrypoints while keeping octuple/nonuple alias shells as
  continuity-only evidence outside this extraction.
- On the elaboration surface, the round adds the exact-edge authoritative
  instantiation guard for the septuple-alias packet, pinning
  `ExpInstantiate [NodeId 52]` together with
  `InstSeq (InstApp (TVar "t53")) (InstApp (TVar "t59"))`.
...
- `orchestrator/rounds/round-218/review.md` records
...
- `orchestrator/rounds/round-218/review-record.json` preserves the same
...
- Merge readiness: confirmed, provided the squash stays limited to the three
  ...
  `orchestrator/rounds/round-218/`, including this `merge.md`.
...
- No merge-order blocker is visible in the active `rev-021` roadmap:
...
- Base freshness is exact for the recorded local squash target.
  `HEAD`,
  `orchestrator/round-218-promote-p5-septuple-alias-clear-boundary-anchor`,
  `codex/automatic-recursive-type-inference`, and
  `git merge-base codex/automatic-recursive-type-inference HEAD`
  all resolve to `f4050793e0d9431d96ca65f252fb6aaacfe972c9`, and
...
- `round-218` is ready for squash merge.
...
- Post-merge summaries should keep the milestone-3 ordering explicit:
  ...
  `sameLaneSeptupleAliasFrameClearBoundaryExpr` becomes the next explicit
...
- Keep later milestone-3 and milestone-4 work honest about what did not land
  ...
  success,
  octuple/deeper alias shells remain continuity-only,
