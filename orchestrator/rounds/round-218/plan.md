# Round 218 Plan (rev-021 / milestone-3 `sameLaneSeptupleAliasFrameClearBoundaryExpr` promotion)

**Goal:** Promote `sameLaneSeptupleAliasFrameClearBoundaryExpr` from inherited
...
that, the preserved `sameLaneAliasFrameClearBoundaryExpr` predecessor lane,
the merged selected same-wrapper nested-`forall` win, checked-authoritative
parity, the fail-closed `P2` / `N1` / `N2` / `N6` shell probes, and the
bounded rule that octuple / deeper alias shells stay outside this
extraction.

**Architecture:** Start from merged base-branch `HEAD = f405079` and treat the
merged `round-211` / `round-212` / `round-213` / `round-214` / `round-215` /
`round-216` / `round-217` mechanism as fixed baseline truth. The default move
is evidence-surface promotion inside
`test/Research/P5ClearBoundarySpec.hs`,
`test/PipelineSpec.hs`, and
`test/ElaborationSpec.hs`:
publish the exact septuple-alias packet as the next explicit milestone-3
anchor, prefer sharpening the existing pipeline continuity row rather than
creating a second pipeline packet, and keep octuple / deeper alias shells
...
- Keep this round on the exact packet
  `sameLaneSeptupleAliasFrameClearBoundaryExpr` only.
...
- Keep `sameLaneOctupleAliasFrameClearBoundaryExpr` and deeper alias shells
...
- Default to test/evidence-surface promotion first; production edits are
  lawful only after the explicit septuple-alias packet is shown failing and a
...
- Verify-only: `orchestrator/worktrees/round-218/orchestrator/state.json`
- Verify-only:
  `orchestrator/worktrees/round-218/orchestrator/rounds/round-218/selection.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-021/roadmap.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-021/verification.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-021/retry-subloop.md`
- Verify-only: `orchestrator/rounds/round-217/review-record.json`
- Verify-only: `orchestrator/rounds/round-217/merge.md`
- Verify-only: `TODO.md`
...
- [ ] Confirm controller state, canonical round-worktree state, round
      ...
      `rev-021` lineage and the extracted item
      `promote-same-lane-septuple-alias-clear-boundary-packet-to-next-explicit-milestone-3-representative-corpus-anchor`.
- [ ] Re-read merged `round-217` review / merge evidence so the preserved
      baseline is explicit before any edits:
      `sameLaneSextupleAliasFrameClearBoundaryExpr` is already the merged
      next explicit milestone-3 anchor after the merged quintuple-alias
      anchor, the promotion was test-only, and the milestone-2 production
      mechanism stayed the controlling baseline.
- [ ] Re-read the `rev-021` roadmap / verification / retry bundle and the
      ...
      `sameLaneSextupleAliasFrameClearBoundaryExpr` are merged baseline
      truth,
...
- [ ] Inspect
      ...
      `sameLaneSeptupleAliasFrameClearBoundaryExpr` continuity row plus
      octuple / deeper continuity guards, while the research surface does
      not yet publish septuple-alias as the next explicit milestone-3 anchor
...
python3 -m json.tool orchestrator/worktrees/round-218/orchestrator/state.json >/dev/null
rg -n 'roadmap_id|roadmap_revision|roadmap_dir|milestone-3|direction-3a|promote-same-lane-septuple-alias-clear-boundary-packet' orchestrator/state.json orchestrator/worktrees/round-218/orchestrator/state.json orchestrator/worktrees/round-218/orchestrator/rounds/round-218/selection.md
...
git -C orchestrator/worktrees/round-218 diff --name-only -- src/MLF/Elab/TermClosure.hs src/MLF/Elab/Run/Pipeline.hs src/MLF/Elab/Pipeline.hs src-public/MLF/Pipeline.hs src/MLF/Elab/Run/ResultType/Fallback.hs src/MLF/Elab/Run/ResultType/Fallback/Core.hs
```

### Task 2: Promote the exact septuple-alias packet across the three milestone-3 review surfaces
...
- [ ] In `test/Research/P5ClearBoundarySpec.hs`, add
      `sameLaneSeptupleAliasFrameClearBoundaryExpr` plus
      `sameLaneSeptupleAliasFrameClearBoundaryFallbackType`, then publish the
      ...
      new live packet (`sameLaneSeptupleAliasFrameClearBoundaryExpr`),
      then preserved selected same-wrapper nested-`forall`.
- [ ] In `test/PipelineSpec.hs`, sharpen the existing
      `sameLaneSeptupleAliasFrameClearBoundaryExpr` entrypoint row into the
      ...
      over adding a second septuple-alias packet. Leave
      `sameLaneSextupleAliasFrameClearBoundaryExpr` explicitly described as
      the merged next anchor after the quintuple-alias anchor, and leave the
      separate octuple / deeper alias-shell rows plus the shared helper /
...
- [ ] In `test/ElaborationSpec.hs`, add the exact-edge authoritative
      ...
      `sameLaneSeptupleAliasFrameClearBoundaryExpr` immediately after the
      sextuple-alias guard. Mirror the existing packet extractor shape,
      extend it to the septuple-alias fixture shape already declared for this
      ...
      into generic alias-depth helpers or octuple-alias coverage.
- [ ] If local cleanup is needed to keep the tests readable, keep it inside
      ...
      production surface, or move octuple / deeper alias shells onto the
...
- [ ] If the newly explicit septuple-alias rows fail during authoritative
      ...
      handoff. The repair must only make the exact septuple-alias packet earn
...
- [ ] If the failure instead appears after annotation translation, localize
      ...
      `rhsAbs0`, and
      `rhsAbs`
      so the exact septuple-alias packet survives the admitted
      post-annotation seam on both authoritative entrypoints without changing
      the already merged first six anchors.
...
- [ ] After any repair, rerun the exact packet and preserved-anchor cluster
      ...
      anchor after that, alias-frame predecessor truth, selected same-wrapper
      nested-`forall` success, checked-authoritative parity, or the
      fail-closed `P2` / `N1` / `N2` / `N6` shell probes.

**Verification commands:**

```bash
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneSeptupleAliasFrameClearBoundaryExpr exact edge authoritative instantiation translation"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneSeptupleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneSextupleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneQuintupleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneQuadrupleAliasFrameClearBoundaryExpr"'
...
- [ ] Re-run the exact packet evidence proving
      `sameLaneSeptupleAliasFrameClearBoundaryExpr` now succeeds honestly on
...
- [ ] Re-run the no-octuple boundary evidence:
      `sameLaneOctupleAliasFrameClearBoundaryExpr` and deeper alias shells
      must stay continuity-only and green in `test/PipelineSpec.hs`, and
      octuple / deeper alias-shell names must remain absent from
      `test/Research/P5ClearBoundarySpec.hs` and
      `test/ElaborationSpec.hs`.
- [ ] Verify diff hygiene and scope discipline against merge-base `f405079`.
...
- [ ] Run the full gates. Expected end state:
      ...
      `1356 examples` baseline to `1359 examples, 0 failures`.
...
cabal test mlf2-test --test-show-details=direct --test-options='--match "fail-closed once it leaves the local TypeRef lane"'
rg -n 'sameLaneOctupleAliasFrameClearBoundaryExpr|sameLaneNonupleAliasFrameClearBoundaryExpr|sameLaneDecupleAliasFrameClearBoundaryExpr' test/Research/P5ClearBoundarySpec.hs test/ElaborationSpec.hs test/PipelineSpec.hs
base=$(git -C orchestrator/worktrees/round-218 merge-base codex/automatic-recursive-type-inference HEAD)
git -C orchestrator/worktrees/round-218 diff --name-only "$base" -- test/Research/P5ClearBoundarySpec.hs test/PipelineSpec.hs test/ElaborationSpec.hs src/MLF/Elab/Elaborate/Annotation.hs src/MLF/Elab/Elaborate/Algebra.hs src/MLF/Elab/Legacy.hs
git -C orchestrator/worktrees/round-218 diff --name-only "$base" -- src/MLF/Elab/TermClosure.hs src/MLF/Elab/Run/Pipeline.hs src/MLF/Elab/Pipeline.hs src-public/MLF/Pipeline.hs src/MLF/Elab/Run/ResultType/Fallback.hs src/MLF/Elab/Run/ResultType/Fallback/Core.hs
git -C orchestrator/worktrees/round-218 diff --check "$base" --
...
- `sameLaneSeptupleAliasFrameClearBoundaryExpr` is explicitly named as the
...
- Octuple / deeper alias shells remain continuity-only rather than newly
