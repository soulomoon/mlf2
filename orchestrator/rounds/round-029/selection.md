...
`orchestrator/state.json` is parked at `stage: select-task` for `round-029` with `current_task: null` and `retry: null`, so no same-round retry is active and no earlier stage is forced.

`orchestrator/roadmap.md` records `U1` as done (finalized in `round-028`) and `U2` as the lowest-numbered unfinished item. Under the guider contract, this makes `U2` the next lawful selection.
