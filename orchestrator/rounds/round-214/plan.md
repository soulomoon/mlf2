# Round 214 Plan (rev-017 / milestone-3 `sameLaneTripleAliasFrameClearBoundaryExpr` promotion)

**Goal:** Promote `sameLaneTripleAliasFrameClearBoundaryExpr` from inherited
...
`sameLaneDoubleAliasFrameClearBoundaryExpr` next anchor, the preserved
...
`P2` / `N1` / `N2` / `N6` shell probes, and the no-quadruple-alias boundary.

**Architecture:** Start from merged base-branch `HEAD = 2091c39` and treat the
merged `round-211` / `round-212` / `round-213` mechanism as fixed baseline
truth. The default move is evidence-surface promotion inside
`test/Research/P5ClearBoundarySpec.hs`,
`test/PipelineSpec.hs`, and
`test/ElaborationSpec.hs`:
publish the exact triple-alias packet as the next explicit milestone-3 anchor,
prefer sharpening the existing pipeline continuity row rather than creating a
second pipeline packet, and keep quadruple / deeper alias shells as continuity
evidence only. Production edits in
...
- Keep `sameLaneQuadrupleAliasFrameClearBoundaryExpr` and deeper alias shells
  as inherited continuity evidence only; do not promote them onto the
  research or elaboration milestone-3 surfaces.
...
- Default to test/evidence-surface promotion first; production edits are
  lawful only after the explicit triple-alias packet is shown failing and the
  trace narrows the fault to the admitted within-slice seam.

### Task 1: Audit the merged baseline and freeze the exact promotion target

**Files:**
- Verify-only: `orchestrator/state.json`
- Verify-only: `orchestrator/worktrees/round-214/orchestrator/state.json`
- Verify-only:
  `orchestrator/worktrees/round-214/orchestrator/rounds/round-214/selection.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-017/roadmap.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-017/verification.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-017/retry-subloop.md`
- Verify-only: `orchestrator/rounds/round-213/review.md`
- Verify-only: `orchestrator/rounds/round-213/implementation-notes.md`
...
- [ ] Confirm controller state, canonical round-worktree state, round
      ...
      `rev-017` lineage and the extracted item
      `promote-same-lane-triple-alias-clear-boundary-packet-to-next-explicit-milestone-3-representative-corpus-anchor`.
- [ ] Re-read merged `round-213` review / notes so the preserved baseline is
      explicit before any edits:
      `sameLaneDoubleAliasFrameClearBoundaryExpr` is already the merged next
      explicit milestone-3 anchor, the promotion was test-only, and the
      milestone-2 production mechanism stayed the controlling baseline.
- [ ] Re-read merged `round-212`, accepted `round-211`, and accepted
      `round-197` so the preserved predecessor truths stay explicit:
      `sameLaneClearBoundaryExpr` remains the first anchor,
      `sameLaneAliasFrameClearBoundaryExpr` remains predecessor truth only,
      and the selected same-wrapper nested-`forall` packet remains merged
      success rather than reopened scope.
- [ ] Inspect
      `test/Research/P5ClearBoundarySpec.hs`,
      `test/PipelineSpec.hs`, and
      `test/ElaborationSpec.hs`
      to confirm the live planning gap:
      `test/PipelineSpec.hs` already carries the bounded triple-alias
      continuity row and the quadruple no-widening guard, while the research
      surface does not yet publish triple-alias as the next explicit
...
python3 -m json.tool orchestrator/worktrees/round-214/orchestrator/state.json >/dev/null
rg -n 'roadmap_id|roadmap_revision|roadmap_dir|milestone-3|direction-3a|promote-same-lane-triple-alias-clear-boundary-packet' orchestrator/state.json orchestrator/worktrees/round-214/orchestrator/state.json orchestrator/worktrees/round-214/orchestrator/rounds/round-214/selection.md
rg -n 'sameLaneClearBoundaryExpr|sameLaneAliasFrameClearBoundaryExpr|sameLaneDoubleAliasFrameClearBoundaryExpr|sameLaneTripleAliasFrameClearBoundaryExpr|sameLaneQuadrupleAliasFrameClearBoundaryExpr|selected same-wrapper nested-forall' test/Research/P5ClearBoundarySpec.hs test/PipelineSpec.hs test/ElaborationSpec.hs
git -C orchestrator/worktrees/round-214 diff --name-only -- src/MLF/Elab/TermClosure.hs src/MLF/Elab/Run/Pipeline.hs src/MLF/Elab/Pipeline.hs src-public/MLF/Pipeline.hs src/MLF/Elab/Run/ResultType/Fallback.hs src/MLF/Elab/Run/ResultType/Fallback/Core.hs
```

### Task 2: Promote the exact triple-alias packet across the three milestone-3 review surfaces
...
- [ ] In `test/Research/P5ClearBoundarySpec.hs`, add
      `sameLaneTripleAliasFrameClearBoundaryExpr` plus
      `sameLaneTripleAliasFrameClearBoundaryFallbackType`, then publish the
      ...
      new live packet (`sameLaneTripleAliasFrameClearBoundaryExpr`),
      then preserved selected same-wrapper nested-`forall`.
- [ ] In `test/PipelineSpec.hs`, sharpen the existing
      `sameLaneTripleAliasFrameClearBoundaryExpr` entrypoint row into the live
      milestone-3 promotion check for both authoritative entrypoints while
      preserving the current recursive-output, type-check, and checked-parity
      substance. Prefer renaming / tightening the existing row over adding a
      second triple-alias packet. Leave
      `sameLaneDoubleAliasFrameClearBoundaryExpr` explicitly described as the
      merged next anchor, and leave the separate
      `sameLaneQuadrupleAliasFrameClearBoundaryExpr` row plus the depth-3
      budget guard as continuity-only / no-quadruple-boundary evidence.
- [ ] In `test/ElaborationSpec.hs`, add the exact-edge authoritative
      instantiation translation guard for
      `sameLaneTripleAliasFrameClearBoundaryExpr` immediately after the
      double-alias guard. Mirror the existing packet extractor shape, but
      extend it to the
      `k -> hold -> keep -> more -> u`
      triple-alias form and lock the concrete
      ...
      into generic alias-depth helpers or quadruple-alias coverage.
- [ ] If local cleanup is needed to keep the tests readable, keep it inside
      ...
      production surface, or move quadruple / deeper alias shells onto the
...
- [ ] If the newly explicit triple-alias rows fail during authoritative
      ...
      handoff. The repair must only make the exact triple-alias packet earn
...
- [ ] If the failure instead appears after annotation translation, localize
      ...
      `rhsAbs0`, and
      `rhsAbs`
      so the exact triple-alias packet survives the admitted post-annotation
      seam on both authoritative entrypoints without changing the already
      merged first two anchors.
...
- [ ] After any repair, rerun the exact packet trio first and stop if the fix
      changes the merged first anchor, the merged double-alias next anchor,
...
```bash
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneTripleAliasFrameClearBoundaryExpr exact edge authoritative instantiation translation"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneTripleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneDoubleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneClearBoundaryExpr"'
...
- [ ] Re-run the exact packet evidence proving
      `sameLaneTripleAliasFrameClearBoundaryExpr` now succeeds honestly on
...
- [ ] Re-run the no-quadruple boundary evidence:
      `sameLaneQuadrupleAliasFrameClearBoundaryExpr` must stay continuity-only
      and green in `test/PipelineSpec.hs`, and it must remain absent from
      `test/Research/P5ClearBoundarySpec.hs` and `test/ElaborationSpec.hs`.
- [ ] Verify diff hygiene and scope discipline against merge-base `2091c39`.
...
- [ ] Run the full gates. Expected end state:
      ...
      prefer the existing pipeline row plus two new research rows and one new
      elaboration row, the full test count should rise from the merged
      `1344 examples` baseline to `1347 examples, 0 failures`.
...
cabal test mlf2-test --test-show-details=direct --test-options='--match "fail-closed once it leaves the local TypeRef lane"'
rg -n 'sameLaneQuadrupleAliasFrameClearBoundaryExpr' test/Research/P5ClearBoundarySpec.hs test/ElaborationSpec.hs test/PipelineSpec.hs
base=$(git -C orchestrator/worktrees/round-214 merge-base codex/automatic-recursive-type-inference HEAD)
git -C orchestrator/worktrees/round-214 diff --name-only "$base" -- test/Research/P5ClearBoundarySpec.hs test/PipelineSpec.hs test/ElaborationSpec.hs src/MLF/Elab/Elaborate/Annotation.hs src/MLF/Elab/Elaborate/Algebra.hs src/MLF/Elab/Legacy.hs
git -C orchestrator/worktrees/round-214 diff --name-only "$base" -- src/MLF/Elab/TermClosure.hs src/MLF/Elab/Run/Pipeline.hs src/MLF/Elab/Pipeline.hs src-public/MLF/Pipeline.hs src/MLF/Elab/Run/ResultType/Fallback.hs src/MLF/Elab/Run/ResultType/Fallback/Core.hs
git -C orchestrator/worktrees/round-214 diff --check "$base" --
