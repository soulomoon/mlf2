...
- Extracted item id: `item-274-string-take-native-tracer`
...
### Goal
Add the next native-capable broad text slicing tracer for `.mlfp`: a public Prelude `stringTake : String -> Int -> String` operation should take a non-negative count of Unicode scalar values from the start of a `String` and return that prefix through source checking, interpreter/runtime, backend LLVM emission, object-code validation, and linked native execution.

This follows round-265's `Char` literal tracer, round-266's non-ASCII `String` literal tracer, round-267's `stringLength` Unicode-scalar counting tracer, round-268's `stringIsEmpty` classification tracer, round-269's `stringContainsChar` scalar search tracer, round-270's `stringContains` substring search tracer, round-271's `stringStartsWith` prefix search tracer, round-272's `stringEndsWith` suffix search tracer, and round-273's `stringDrop` drop-slicing tracer. It advances milestone-3 with one observable prefix-slicing operation without claiming `String`/`List Char` conversion, complete slicing coverage, formatting, broader classification predicates, full cursor APIs, parser parity, platform contracts, or milestone-3 completion.
...
Implementation ownership for this round is the single implementer in the canonical round-274 worktree. The owned write scope is:
...
- Round evidence only after implementation: `orchestrator/rounds/round-274/implementation-notes.md`.

Proceed behavior-first in one vertical RED -> GREEN -> refactor cycle from this first public-interface behavior:

> `.mlfp` source files importing Prelude `stringTake` classify Unicode scalar prefix counts by returning `stringTake "λab" 1 == "λ"` and `stringTake "aλb" 2 == "aλ"`, and both programs pass `check-program`, `run-program`, `emit-backend`, object validation, `emit-native`, and linked native execution with matching public `String` output.

Focused RED test slice to write first:

Add one Hspec example in `test/BackendLLVMSpec.hs` named `stringTake slices Unicode scalar prefixes through native execution`. Use the existing public CLI helpers and LLVM test support from that spec: write each source through `withTempProgram`, call `checkProgramFile`, `runProgramFile`, `emitBackendFile`, and `emitNativeFile`, validate backend/native LLVM with `validateLLVMAssembly`, validate object generation with `validateLLVMObjectCode`, and run the emitted native LLVM with `runLLVMNativeExecutable`.

Before adding the primitive, Prelude binding, interpreter behavior, or backend/native lowering, run the focused matcher and record the RED failure in `implementation-notes.md`. The expected RED should show that `stringTake` is not yet a public checked/native-capable Prelude operation, not an unwired test or compile error.

### Steps
1. Reconfirm the active rev-003 lineage and this selection record. `milestone-3` is dependency-ready and in progress after approved rounds 265 through 273, and `direction-3a-broad-string-char-substrate` remains the selected direction.
...
3. Write the RED test only:
   - add one Hspec example named `stringTake slices Unicode scalar prefixes through native execution`;
   - use these sources:
...
```
   ...
   - assert the first program `runProgramFile path == Right "\"\\955\"\n"` and the second program `runProgramFile path == Right "\"a\\955\"\n"`;
...
5. Implement only enough to pass this behavior:
   - add one primitive inventory entry for the reserved operation, such as `__string_take : String -> Int -> String`, with native support classified in `MLF.Primitive.Inventory`;
   - export `stringTake` from the built-in Prelude and define `stringTake : String -> Int -> String = __string_take;`;
   - extend `run-program` primitive evaluation so `stringTake "λab" 1` returns `"λ"` and `stringTake "aλb" 2` returns `"aλ"`;
   - extend backend/native lowering so the primitive accepts the current runtime `String` pointer plus an `Int` count and copies a null-terminated prefix at valid Unicode scalar boundaries, not byte counts, escaped rendering, or display text;
   - preserve Unicode scalar semantics and do not introduce byte-indexed public APIs;
   - preserve existing ASCII `String`, non-ASCII `String`, `Char`, `stringLength`, `stringIsEmpty`, `stringContainsChar`, `stringContains`, `stringStartsWith`, `stringEndsWith`, `stringDrop`, IO, and native result-rendering behavior;
   - keep negative, out-of-range, `String`/`List Char` conversion, formatting, arbitrary range slicing, and cursor semantics out of this selected tracer unless the implementation must fail closed for correctness.
6. Update relevant docs without overclaiming:
   - `docs/mlfp-language-reference.md` should list `stringTake : String -> Int -> String` as the first public non-negative Unicode-scalar prefix-slicing tracer;
   - `docs/backend-native-pipeline.md` should mention the native-capable `stringTake` tracer if it lists native-supported text operations;
   - `docs/mlfp-self-boot-readiness.md` should record that a first take-based slicing operation exists while `String`/`List Char` conversion, full slicing coverage, formatting, broader classification predicates, full cursor APIs, and parser parity remain future work;
...
Run it once after adding the test and before implementation; it must fail for the missing public `stringTake` behavior. Run it again after implementation; it must pass.
...
```bash
rg -n 'stringTake : String -> Int -> String|__string_take|stringTakePrimitiveName|PrimitiveNativeStringTake|RuntimeStringTake' src test docs README.md CHANGELOG.md
```
...
- Confirm `stringTake "λab" 1` returns `"λ"` and `stringTake "aλb" 2` returns `"aλ"`, proving non-negative Unicode scalar prefix slicing over non-ASCII input.
...
- Confirm existing round-265 `Char`, round-266 non-ASCII `String`, round-267 `stringLength`, round-268 `stringIsEmpty`, round-269 `stringContainsChar`, round-270 `stringContains`, round-271 `stringStartsWith`, round-272 `stringEndsWith`, and round-273 `stringDrop` tracers still pass.
- Confirm docs say only the first take-based prefix-slicing tracer landed; they must not claim `String`/`List Char` conversion, complete slicing, formatting, full cursor APIs, parser parity, locale, regex, platform contracts, compiler package work, self-boot proof completion, or milestone-3 completion.
- Confirm no shared conformance expected output is regenerated dynamically and no milestone-2 corpus contract is weakened.

### Scope Boundaries
- In scope: one public `stringTake : String -> Int -> String` non-negative Unicode-scalar prefix-slicing tracer across source checking, interpreter/runtime, backend emission, object generation, native execution, narrow docs, and changelog.
- Out of scope: `String`/`List Char` conversion, arbitrary range slicing, negative or out-of-range semantics beyond fail-closed correctness if required, formatting, broader character classification predicates, full cursor APIs, parser-owned combinator work, locale, regex, platform ABI/FFI/GC contracts, compiler source package implementation, driver work, proof records, roadmap status edits, controller state edits, or semantic roadmap updates.
