...
`item-274-string-take-native-tracer` is implemented within the selected scope. The public Prelude binding `stringTake : String -> Int -> String` now exercises the required Unicode scalar prefix-slicing cases, with the current escaped string display expectations, through source checking, run-program, backend LLVM/object validation, emit-native/native object validation, and linked native execution.

## Plan Compliance

- Lineage matches the active round selection: `round-274`, `milestone-3`, `direction-3a-broad-string-char-substrate`, `item-274-string-take-native-tracer`.
- The implementation adds the primitive in the inventory owner as `PrimitiveNativeStringTake` / `__string_take`, exposes `stringTake` through the Prelude, handles it in the interpreter/runtime as `RuntimeStringTake`, and lowers it in the LLVM/native backend.
- The focused backend test covers both selected examples:
  - `stringTake "λab" 1` displays as `"\\955"`.
  - `stringTake "aλb" 2` displays as `"a\\955"`.
...
- The native helper copies whole UTF-8 scalar byte sequences for the selected lambda examples instead of byte-counting the output prefix. The interpreter path uses Haskell `String` character taking, so the source/runtime behavior matches the native tracer for the selected cases.
- Prior broad text/native tracers remain covered by the neighbor matcher set.
- Docs and changelog record only the new first take-slicing tracer and preserve future-work boundaries for `String/List Char` alignment, broader slicing semantics, formatting, cursor, and parser parity. I did not find self-boot or full-string-substrate overclaims.
...
- `rg -n 'stringTake \\"λab\\" 1|stringTake \\"aλb\\" 2' test/BackendLLVMSpec.hs`
...
- `cabal build all`
  - Passed.
- `cabal test`
  - Passed: 2578 examples, 0 failures.
