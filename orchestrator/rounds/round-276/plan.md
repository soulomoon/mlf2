...
- Extracted item id: `item-276-string-char-at-native-tracer`
...
### Goal
Add the first native-capable broad text cursor/index tracer for `.mlfp`: a public Prelude `stringCharAt : String -> Int -> Char` operation should read an in-range Unicode scalar at a non-negative scalar index and return that `Char` through source checking, interpreter/runtime, backend LLVM emission, object-code validation, and linked native execution.

This follows round-265's `Char` literal tracer, round-266's non-ASCII `String` literal tracer, round-267's `stringLength` Unicode-scalar counting tracer, round-268's `stringIsEmpty` classification tracer, round-269's `stringContainsChar` scalar search tracer, round-270's `stringContains` substring search tracer, round-271's `stringStartsWith` prefix search tracer, round-272's `stringEndsWith` suffix search tracer, round-273's `stringDrop` drop-slicing tracer, round-274's `stringTake` take-slicing tracer, and round-275's `stringSlice` range-slicing tracer. It advances milestone-3 with one observable parser-needed cursor operation without claiming `String`/`List Char` conversion, complete cursor semantics, formatting, broader classification predicates, parser parity, platform contracts, or milestone-3 completion.
...
Implementation ownership for this round is the single implementer in the canonical round-276 worktree. The owned write scope is:
...
- Round evidence only after implementation: `orchestrator/rounds/round-276/implementation-notes.md`.

Proceed behavior-first in one vertical RED -> GREEN -> refactor cycle from this first public-interface behavior:

> `.mlfp` source files importing Prelude `stringCharAt` read Unicode scalar cursor positions by returning `stringCharAt "aλb" 1 == 'λ'` and `stringCharAt "λab" 2 == 'b'`, and both programs pass `check-program`, `run-program`, `emit-backend`, object validation, `emit-native`, and linked native execution with matching public `Char` output.

Focused RED test slice to write first:

Add one Hspec example in `test/BackendLLVMSpec.hs` named `stringCharAt indexes Unicode scalar cursor positions through native execution`. Use the existing public CLI helpers and LLVM test support from that spec: write each source through `withTempProgram`, call `checkProgramFile`, `runProgramFile`, `emitBackendFile`, and `emitNativeFile`, validate backend/native LLVM with `validateLLVMAssembly`, validate object generation with `validateLLVMObjectCode`, and run the emitted native LLVM with `runLLVMNativeExecutable`.

Before adding the primitive, Prelude binding, interpreter behavior, or backend/native lowering, run the focused matcher and record the RED failure in `implementation-notes.md`. The expected RED should show that `stringCharAt` is not yet a public checked/native-capable Prelude operation, not an unwired test or compile error.

### Steps
1. Reconfirm the active rev-003 lineage and this selection record. `milestone-3` is dependency-ready and in progress after approved rounds 265 through 275, and `direction-3a-broad-string-char-substrate` remains the selected direction.
...
3. Write the RED test only:
   - add one Hspec example named `stringCharAt indexes Unicode scalar cursor positions through native execution`;
   - use these sources:
...
```
   ...
   - assert the first program `runProgramFile path == Right "'\\955'\n"` and the second program `runProgramFile path == Right "'b'\n"`;
   - assert each `emitBackendFile path` succeeds, contains a `Main__main` backend function returning `i32`, and passes `validateLLVMAssembly` plus `validateLLVMObjectCode`;
   - assert each `emitNativeFile path` succeeds, contains the native C ABI `main`, passes `validateLLVMAssembly` plus `validateLLVMObjectCode`, and linked execution returns `NativeRunResult ExitSuccess "'\\955'\n" ""` or `NativeRunResult ExitSuccess "'b'\n" ""` respectively.
...
5. Implement only enough to pass this behavior:
   - add one primitive inventory entry for the reserved operation, such as `__string_char_at : String -> Int -> Char`, with native support classified in `MLF.Primitive.Inventory`;
   - export `stringCharAt` from the built-in Prelude and define `stringCharAt : String -> Int -> Char = __string_char_at;`;
   - extend `run-program` primitive evaluation so `stringCharAt "aλb" 1` returns `'λ'` and `stringCharAt "λab" 2` returns `'b'`;
   - extend backend/native lowering so the primitive accepts the current runtime `String` pointer plus an `Int` index and returns the Unicode scalar value at a scalar boundary as the existing `Char` representation, not a byte value or display text;
   - preserve Unicode scalar semantics and do not introduce byte-indexed public APIs;
   - preserve existing ASCII `String`, non-ASCII `String`, `Char`, `stringLength`, `stringIsEmpty`, `stringContainsChar`, `stringContains`, `stringStartsWith`, `stringEndsWith`, `stringDrop`, `stringTake`, `stringSlice`, IO, and native result-rendering behavior;
   - keep negative or out-of-range indexing semantics, `String`/`List Char` conversion, formatting, complete cursor API design, and parser combinator semantics out of this selected tracer unless the implementation must fail closed for correctness.
6. Update relevant docs without overclaiming:
   - `docs/mlfp-language-reference.md` should list `stringCharAt : String -> Int -> Char` as the first public in-range Unicode-scalar cursor/index tracer;
   - `docs/backend-native-pipeline.md` should mention the native-capable `stringCharAt` tracer if it lists native-supported text operations;
   - `docs/mlfp-self-boot-readiness.md` should record that a first cursor/index operation exists while `String`/`List Char` conversion, formatting, complete cursor API semantics, broader classification predicates, and parser parity remain future work;
...
Run it once after adding the test and before implementation; it must fail for the missing public `stringCharAt` behavior. Run it again after implementation; it must pass.
...
- Confirm `stringCharAt "aλb" 1` returns `'λ'` and `stringCharAt "λab" 2` returns `'b'`, proving indexes advance over Unicode scalar values rather than UTF-8 bytes.
...
- Confirm existing round-265 `Char`, round-266 non-ASCII `String`, round-267 `stringLength`, round-268 `stringIsEmpty`, round-269 `stringContainsChar`, round-270 `stringContains`, round-271 `stringStartsWith`, round-272 `stringEndsWith`, round-273 `stringDrop`, round-274 `stringTake`, and round-275 `stringSlice` tracers still pass.
- Confirm docs say only the first in-range cursor/index tracer landed; they must not claim `String`/`List Char` conversion, complete cursor semantics, formatting, broader classification predicates, parser parity, locale, regex, platform contracts, compiler package work, self-boot proof completion, or milestone-3 completion.
- Confirm no shared conformance expected output is regenerated dynamically and no milestone-2 corpus contract is weakened.

### Scope Boundaries
- In scope: one public `stringCharAt : String -> Int -> Char` in-range Unicode-scalar cursor/index tracer across source checking, interpreter/runtime, backend emission, object generation, native execution, narrow docs, and changelog.
- Out of scope: `String`/`List Char` conversion, negative or out-of-range cursor semantics beyond fail-closed correctness if required, formatting, broader character classification predicates, complete cursor API design, parser-owned combinator work, locale, regex, platform ABI/FFI/GC contracts, compiler source package implementation, driver work, proof records, roadmap status edits, controller state edits, or semantic roadmap updates.
