...
- Extracted item id: `item-277-char-is-digit-native-tracer`
...
### Goal
Add the first native-capable `Char` classification tracer for `.mlfp`: a public Prelude `charIsDigit : Char -> Bool` operation should classify source decimal digit characters and non-digit Unicode scalar characters through source checking, interpreter/runtime, backend LLVM emission, object-code validation, and linked native execution.

This follows round-265's `Char` literal tracer, round-266's non-ASCII `String` literal tracer, round-267's `stringLength` Unicode-scalar counting tracer, round-268's `stringIsEmpty` classification tracer, round-269's `stringContainsChar` scalar search tracer, round-270's `stringContains` substring search tracer, round-271's `stringStartsWith` prefix search tracer, round-272's `stringEndsWith` suffix search tracer, round-273's `stringDrop` drop-slicing tracer, round-274's `stringTake` take-slicing tracer, round-275's `stringSlice` range-slicing tracer, and round-276's `stringCharAt` cursor/index tracer. It advances milestone-3 with one parser-relevant character classification operation without claiming the full classification family, `String`/`List Char` conversion, formatting, complete cursor APIs, parser parity, platform contracts, or milestone-3 completion.
...
Implementation ownership for this round is the single implementer in the canonical round-277 worktree. The owned write scope is:
...
- Round evidence only after implementation: `orchestrator/rounds/round-277/implementation-notes.md`.

Proceed behavior-first in one vertical RED -> GREEN -> refactor cycle from this first public-interface behavior:

> `.mlfp` source files importing Prelude `charIsDigit` classify decimal digit characters by returning `charIsDigit '7' == true` and `charIsDigit 'λ' == false`, and both programs pass `check-program`, `run-program`, `emit-backend`, object validation, `emit-native`, and linked native execution with matching public `Bool` output.

Focused RED test slice to write first:

Add one Hspec example in `test/BackendLLVMSpec.hs` named `charIsDigit classifies decimal Char values through native execution`. Use the existing public CLI helpers and LLVM test support from that spec: write each source through `withTempProgram`, call `checkProgramFile`, `runProgramFile`, `emitBackendFile`, and `emitNativeFile`, validate backend/native LLVM with `validateLLVMAssembly`, validate object generation with `validateLLVMObjectCode`, and run the emitted native LLVM with `runLLVMNativeExecutable`.

Before adding the primitive, Prelude binding, interpreter behavior, or backend/native lowering, run the focused matcher and record the RED failure in `implementation-notes.md`. The expected RED should show that `charIsDigit` is not yet a public checked/native-capable Prelude operation, not an unwired test or compile error.

### Steps
1. Reconfirm the active rev-003 lineage and this selection record. `milestone-3` is dependency-ready and in progress after approved rounds 265 through 276, and `direction-3a-broad-string-char-substrate` remains the selected direction.
...
3. Write the RED test only:
   - add one Hspec example named `charIsDigit classifies decimal Char values through native execution`;
   - use these sources:
...
```
   ...
   - assert the first program `runProgramFile path == Right "true\n"` and the second program `runProgramFile path == Right "false\n"`;
...
5. Implement only enough to pass this behavior:
   - add one primitive inventory entry for the reserved operation, such as `__char_is_digit : Char -> Bool`, with native support classified in `MLF.Primitive.Inventory`;
   - export `charIsDigit` from the built-in Prelude and define `charIsDigit : Char -> Bool = __char_is_digit;`;
   - extend `run-program` primitive evaluation so `charIsDigit '7'` returns `true` and `charIsDigit 'λ'` returns `false`;
   - extend backend/native lowering so the primitive accepts the existing `Char` scalar representation and returns `Bool` for ASCII decimal digit code points `0` through `9`;
   - preserve Unicode scalar representation for `Char` and do not introduce byte-indexed or locale-sensitive public APIs;
   - preserve existing ASCII `String`, non-ASCII `String`, `Char`, `stringLength`, `stringIsEmpty`, `stringContainsChar`, `stringContains`, `stringStartsWith`, `stringEndsWith`, `stringDrop`, `stringTake`, `stringSlice`, `stringCharAt`, IO, and native result-rendering behavior;
   - keep alphabetic, whitespace, punctuation, case conversion, locale, regex, formatting, `String`/`List Char` conversion, and parser combinator semantics out of this selected tracer.
6. Update relevant docs without overclaiming:
   - `docs/mlfp-language-reference.md` should list `charIsDigit : Char -> Bool` as the first public native-capable `Char` classification tracer;
   - `docs/backend-native-pipeline.md` should mention the native-capable `charIsDigit` tracer if it lists native-supported text operations;
   - `docs/mlfp-self-boot-readiness.md` should record that a first `Char` classification operation exists while `String`/`List Char` conversion, formatting, broader classification family coverage, complete cursor API semantics, and parser parity remain future work;
...
Run it once after adding the test and before implementation; it must fail for the missing public `charIsDigit` behavior. Run it again after implementation; it must pass.
...
- Confirm `charIsDigit '7'` returns `true` and `charIsDigit 'λ'` returns `false`, proving the selected `Char` classification behavior over both ASCII and non-ASCII Unicode scalar values.
