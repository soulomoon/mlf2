...
**Current task/title**:
`Promote sameLaneOctupleAliasFrameClearBoundaryExpr from inherited bounded guard/control evidence to the next explicit milestone-3 representative anchor in test/Research/P5ClearBoundarySpec.hs, the matching test/PipelineSpec.hs rows, and an exact-edge authoritative-instantiation guard in test/ElaborationSpec.hs, while preserving sameLaneClearBoundaryExpr, sameLaneDoubleAliasFrameClearBoundaryExpr, sameLaneTripleAliasFrameClearBoundaryExpr, sameLaneQuadrupleAliasFrameClearBoundaryExpr, sameLaneQuintupleAliasFrameClearBoundaryExpr, sameLaneSextupleAliasFrameClearBoundaryExpr, sameLaneSeptupleAliasFrameClearBoundaryExpr, the merged selected same-wrapper nested-forall packet, sameLaneAliasFrameClearBoundaryExpr predecessor truth, checked-authoritative parity, and the bounded rule that nonuple/deeper alias shells stay outside this exact extraction.`

**Round metadata**:
- round_id: `round-219`
- branch:
  `orchestrator/round-219-promote-p5-octuple-alias-clear-boundary-anchor`
- worktree_path: `orchestrator/worktrees/round-219`
- active_round_dir: `orchestrator/rounds/round-219`
...
- roadmap_dir:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-024`
...
`rev-024` records `milestone-3` as the lowest-numbered unfinished milestone on
top of merged base-branch `HEAD = 7a127e2`, and `retry = null` means no
same-round retry state displaces the normal next selection.

Accepted `round-218`, now merged as `7a127e2`, exhausted the seventh
milestone-3 publication step by making
`sameLaneSeptupleAliasFrameClearBoundaryExpr` the next explicit
clear-boundary anchor on the research, pipeline, and elaboration surfaces.
`rev-024` names the lawful next unfinished extraction directly:
`promote-same-lane-octuple-alias-clear-boundary-packet-to-next-explicit-milestone-3-representative-corpus-anchor`.

That choice is concrete and honest on the merged baseline. The current tree
already preserves `sameLaneOctupleAliasFrameClearBoundaryExpr` as green
inherited guard/control evidence inside `test/PipelineSpec.hs` on both
authoritative entrypoints and as bounded alias-budget continuity evidence, but
it has not yet been promoted to the next explicit milestone-3 representative
anchor in `test/Research/P5ClearBoundarySpec.hs` plus the matching
authoritative pipeline/elaboration publication surfaces. The merged
...
lane, and the merged selected same-wrapper nested-`forall` packet therefore
remain baseline truth only, not whole-frontier closure.

## Current baseline

- `orchestrator/state.json` and the canonical round-worktree state both point
  `round-219` at `milestone-3` /
  `direction-3a-expand-the-broader-positive-representative-corpus` under
  `rev-024`, with branch
  `orchestrator/round-219-promote-p5-octuple-alias-clear-boundary-anchor`
  and canonical worktree `orchestrator/worktrees/round-219`.
- The controlling merged baseline is base-branch `HEAD = 7a127e2`. Preserve
...
- Treat `rev-022` and `rev-023` as stale unusable recovery publications only.
  They do not participate in the accepted post-round-218 coordination
  lineage, and this selection must stay grounded in `rev-024`.
- Preserve the green merged wins named by `rev-024`:
  ...
  milestone-3 clear-boundary anchor after that,
  `sameLaneQuintupleAliasFrameClearBoundaryExpr` as the next explicit
  milestone-3 clear-boundary anchor after that,
  `sameLaneSextupleAliasFrameClearBoundaryExpr` as the next explicit
  milestone-3 clear-boundary anchor after that,
  `sameLaneSeptupleAliasFrameClearBoundaryExpr` as the next explicit
  milestone-3 clear-boundary anchor after that,
...
- On this baseline, `sameLaneOctupleAliasFrameClearBoundaryExpr` is already
  observable as inherited guard/control evidence in `test/PipelineSpec.hs`,
  but it is still missing explicit `P5` representative-corpus status in
  `test/Research/P5ClearBoundarySpec.hs`, and `test/ElaborationSpec.hs`
  still stops its exact-edge authoritative-instantiation publication ladder at
  the merged septuple-alias anchor.
- Keep the deeper alias-budget ladder honest:
  nonuple/deeper alias shells remain inherited continuity/guard evidence
  only, not the selected publication packet for this round.
...
- Keep the selected packet exact:
  `sameLaneOctupleAliasFrameClearBoundaryExpr` is the only new
  ...
  merged `sameLaneDoubleAliasFrameClearBoundaryExpr` next anchor, the already
  merged `sameLaneTripleAliasFrameClearBoundaryExpr` next anchor after that,
  the already merged `sameLaneQuadrupleAliasFrameClearBoundaryExpr` next
  anchor after that, the already merged
  `sameLaneQuintupleAliasFrameClearBoundaryExpr` next anchor after that, the
  already merged `sameLaneSextupleAliasFrameClearBoundaryExpr` next anchor
  after that, or the already merged
  `sameLaneSeptupleAliasFrameClearBoundaryExpr` next anchor after that as live
  debt, do not reopen `sameLaneAliasFrameClearBoundaryExpr` beyond preserved
  predecessor truth, and do not widen into nonuple/deeper alias shells or
...
- Treat the bounded evidence surface as:
  ...
  existing `rev-024` writable slice and remain subordinate to this exact
  packet.
- Preserve the merged `sameLaneClearBoundaryExpr` first anchor, the merged
  ...
  alias-frame predecessor truth, checked-authoritative parity, and the
  current fail-closed quantified contrasts while making the octuple-alias
...
- Do not widen into fallback rescue, a second interface, cyclic or multi-SCC
  search, equi-recursive reasoning, nonuple-/deeper-alias substitute
  closure, or any restatement that treats the merged first seven anchors plus
