...
`orchestrator/state.json` is parked at `active_round_id: round-037`, `stage: select-task`, `current_task: null`, and `retry: null`, so no same-round retry is active and no prior review outcome forces a retry resume ahead of normal roadmap selection.

`orchestrator/roadmap.md` marks item 1 (`C1`), item 2 (`C2`), and item 3 (`C3`) done, leaving item 4 (`C4`) as the lowest-numbered unfinished roadmap entry. Under the guider contract, that makes `C4` the next lawful selection.
...
`Bugs.md` still carries open replay-path bug `BUG-2026-03-16-001`, but it remains replay-lane continuity context only and does not authorize replay reopen, `MLF.Elab.Inst` edits, or broader automatic recursive inference selection in this round. Current repository status is already non-pristine (`orchestrator/state.json` modified and `tasks/todo/2026-03-18-continue-bounded-orchestrator-run/` untracked), so selecting the narrow aggregate `C4` decision gate best respects existing work while advancing the next bounded roadmap item.
