# Round 220 Plan (rev-025 / milestone-3 `sameLaneNonupleAliasFrameClearBoundaryExpr` promotion)

**Goal:** Promote `sameLaneNonupleAliasFrameClearBoundaryExpr` from inherited
...
after that, the merged `sameLaneOctupleAliasFrameClearBoundaryExpr` next
anchor after that, the preserved
`sameLaneAliasFrameClearBoundaryExpr` predecessor lane, the merged selected
same-wrapper nested-`forall` win,
checked-authoritative parity, the fail-closed `P2` / `N1` / `N2` / `N6`
shell probes, and the bounded rule that deeper alias shells stay
outside this extraction.

**Architecture:** Start from merged base-branch `HEAD = 7616109` and treat the
merged `round-211` / `round-212` / `round-213` / `round-214` / `round-215` /
`round-216` / `round-217` / `round-218` / `round-219` mechanism as fixed
baseline truth.
The default move is evidence-surface promotion inside
`test/Research/P5ClearBoundarySpec.hs`,
`test/PipelineSpec.hs`, and
`test/ElaborationSpec.hs`:
publish the exact nonuple-alias packet as the next explicit milestone-3
anchor, prefer sharpening the existing pipeline continuity row rather than
creating a second pipeline packet, and keep deeper alias shells outside this
extraction as continuity evidence only. Production edits in
...
- Keep this round on the exact packet
  `sameLaneNonupleAliasFrameClearBoundaryExpr` only.
...
- Preserve `sameLaneSeptupleAliasFrameClearBoundaryExpr` as the merged next
  ...
  it as live debt.
- Preserve `sameLaneOctupleAliasFrameClearBoundaryExpr` as the merged next
  explicit milestone-3 anchor after the septuple-alias anchor; do not reopen
  it as live debt.
...
- Keep deeper alias shells outside this extraction as inherited continuity
  evidence only; do not promote them onto the research or elaboration
...
- Default to test/evidence-surface promotion first; production edits are
  lawful only after the explicit nonuple-alias packet is shown failing and a
...
- Verify-only: `orchestrator/worktrees/round-220/orchestrator/state.json`
- Verify-only:
  `orchestrator/worktrees/round-220/orchestrator/rounds/round-220/selection.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-025/roadmap.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-025/verification.md`
- Verify-only:
  `orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-025/retry-subloop.md`
- Verify-only: `orchestrator/rounds/round-219/review-record.json`
- Verify-only: `orchestrator/rounds/round-219/merge.md`
- Verify-only: `TODO.md`
...
- [ ] Re-read the `rev-025` roadmap / verification / retry bundle and the
...
- [ ] Inspect
      ...
      `sameLaneNonupleAliasFrameClearBoundaryExpr` continuity row plus
      nonuple / deeper continuity guards, while the research surface does
      not yet publish nonuple-alias as the next explicit milestone-3 anchor
...
python3 -m json.tool orchestrator/worktrees/round-220/orchestrator/state.json >/dev/null
rg -n 'roadmap_id|roadmap_revision|roadmap_dir|milestone-3|direction-3a|promote-same-lane-nonuple-alias-clear-boundary-packet' orchestrator/state.json orchestrator/worktrees/round-220/orchestrator/state.json orchestrator/worktrees/round-220/orchestrator/rounds/round-220/selection.md
rg -n 'Revision: `rev-025`|HEAD = 7616109|sameLaneSeptupleAliasFrameClearBoundaryExpr|sameLaneNonupleAliasFrameClearBoundaryExpr|rev-022|rev-023' orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-025/roadmap.md orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-025/verification.md orchestrator/roadmaps/2026-04-08-00-p5-polymorphism-nested-forall-broader-positive-explicit-boundary-enactment-roadmap/rev-025/retry-subloop.md
rg -n 'sameLaneClearBoundaryExpr|sameLaneAliasFrameClearBoundaryExpr|sameLaneDoubleAliasFrameClearBoundaryExpr|sameLaneTripleAliasFrameClearBoundaryExpr|sameLaneQuadrupleAliasFrameClearBoundaryExpr|sameLaneQuintupleAliasFrameClearBoundaryExpr|sameLaneSextupleAliasFrameClearBoundaryExpr|sameLaneSeptupleAliasFrameClearBoundaryExpr|sameLaneNonupleAliasFrameClearBoundaryExpr|sameLaneNonupleAliasFrameClearBoundaryExpr|selected same-wrapper nested-forall' test/Research/P5ClearBoundarySpec.hs test/PipelineSpec.hs test/ElaborationSpec.hs
git -C orchestrator/worktrees/round-220 diff --name-only -- src/MLF/Elab/TermClosure.hs src/MLF/Elab/Run/Pipeline.hs src/MLF/Elab/Pipeline.hs src-public/MLF/Pipeline.hs src/MLF/Elab/Run/ResultType/Fallback.hs src/MLF/Elab/Run/ResultType/Fallback/Core.hs
```

### Task 2: Promote the exact nonuple-alias packet across the three milestone-3 review surfaces
...
- [ ] In `test/Research/P5ClearBoundarySpec.hs`, add
      `sameLaneNonupleAliasFrameClearBoundaryExpr` plus
      ...
      new live packet (`sameLaneNonupleAliasFrameClearBoundaryExpr`),
      then preserved selected same-wrapper nested-`forall`.
- [ ] In `test/PipelineSpec.hs`, sharpen the existing
      `sameLaneNonupleAliasFrameClearBoundaryExpr` entrypoint row into the
      ...
      over adding a second nonuple-alias packet. Leave
...
- [ ] In `test/ElaborationSpec.hs`, add the exact-edge authoritative
      ...
      `sameLaneNonupleAliasFrameClearBoundaryExpr` immediately after the
      octuple-alias guard. Mirror the existing packet extractor shape,
      extend it to the nonuple-alias fixture shape already declared for this
...
- [ ] If the newly explicit nonuple-alias rows fail during authoritative
      ...
      handoff. The repair must only make the exact nonuple-alias packet earn
...
- [ ] If the failure instead appears after annotation translation, localize
      ...
      `rhsAbs0`, and
      `rhsAbs`
      so the exact nonuple-alias packet survives the admitted
      post-annotation seam on both authoritative entrypoints without changing
      the already merged first eight anchors.
...
- [ ] After any repair, rerun the exact packet and preserved-anchor cluster
      ...
      anchor after that, the merged octuple-alias next anchor after that,
...
```bash
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneNonupleAliasFrameClearBoundaryExpr exact edge authoritative instantiation translation"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneNonupleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneSeptupleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneSextupleAliasFrameClearBoundaryExpr"'
cabal test mlf2-test --test-show-details=direct --test-options='--match "sameLaneQuintupleAliasFrameClearBoundaryExpr"'
...
- [ ] Re-run the exact packet evidence proving
      `sameLaneNonupleAliasFrameClearBoundaryExpr` now succeeds honestly on
...
- [ ] Re-run the no-nonuple boundary evidence:
      ...
      `test/ElaborationSpec.hs`.
- [ ] Verify diff hygiene and scope discipline against merge-base `7616109`.
...
base=$(git -C orchestrator/worktrees/round-220 merge-base codex/automatic-recursive-type-inference HEAD)
git -C orchestrator/worktrees/round-220 diff --name-only "$base" -- test/Research/P5ClearBoundarySpec.hs test/PipelineSpec.hs test/ElaborationSpec.hs src/MLF/Elab/Elaborate/Annotation.hs src/MLF/Elab/Elaborate/Algebra.hs src/MLF/Elab/Legacy.hs
git -C orchestrator/worktrees/round-220 diff --name-only "$base" -- src/MLF/Elab/TermClosure.hs src/MLF/Elab/Run/Pipeline.hs src/MLF/Elab/Pipeline.hs src-public/MLF/Pipeline.hs src/MLF/Elab/Run/ResultType/Fallback.hs src/MLF/Elab/Run/ResultType/Fallback/Core.hs
git -C orchestrator/worktrees/round-220 diff --check "$base" --
...
- `sameLaneNonupleAliasFrameClearBoundaryExpr` is explicitly named as the
