# Merge Preparation (`round-215` / `milestone-3` / `direction-3a`)
...
- `roadmap_item_id`: absent in the active `rev-018` state / selection / review lineage for this round
...
- Round branch: `orchestrator/round-215-promote-p5-quadruple-alias-clear-boundary-anchor`
- Canonical worktree: `orchestrator/worktrees/round-215`
...
- Merge the approved `milestone-3` / `direction-3a` extraction that promotes
  `sameLaneQuadrupleAliasFrameClearBoundaryExpr` from inherited bounded
  guard/control evidence to the next explicit broader-positive representative
  corpus anchor after the merged triple-alias anchor.
- Keep the squash substance honest and bounded to the approved implementation
  payload in:
  `test/Research/P5ClearBoundarySpec.hs`,
  `test/PipelineSpec.hs`, and
  `test/ElaborationSpec.hs`,
  plus the intended round-local notes under `orchestrator/rounds/round-215/`.
  No production files changed for this round.
- On the research surface, the round adds the quadruple-alias fallback anchor
  ...
  success.
- On the pipeline surface, the round retitles the existing quadruple-alias row
  into the explicit milestone-3 promotion check on both authoritative
  entrypoints while keeping quintuple/deeper alias shells as continuity-only
  evidence outside this extraction.
- On the elaboration surface, the round adds the exact-edge authoritative
  instantiation guard for the quadruple-alias packet, pinning
  `ExpInstantiate [NodeId 43]` together with
...
- `orchestrator/rounds/round-215/review.md` records
...
- `orchestrator/rounds/round-215/review-record.json` preserves the same
...
- Merge readiness: confirmed, provided the squash stays limited to the three
  ...
  `orchestrator/rounds/round-215/`, including this `merge.md`.
...
- No merge-order blocker is visible in the active `rev-018` roadmap:
...
- Base freshness is exact for the recorded local squash target.
  ...
  all resolve to `ed6629128811c414af626d15afcdf17638784e1e`, and
...
- The existing remote-tracking snapshot is older, not fresher:
  ...
  merge.
- `round-215` is ready for squash merge.
...
- Post-merge summaries should keep the milestone-3 ordering explicit:
  ...
  after that, and
  `sameLaneQuadrupleAliasFrameClearBoundaryExpr` becomes the next explicit
...
- Keep later milestone-3 and milestone-4 work honest about what did not land
  ...
  success,
  quintuple/deeper alias shells remain continuity-only,
