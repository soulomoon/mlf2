...
2. WHEN an instance operation targets a node outside the expansion interior or without a computation context THEN Φ translation SHALL report a non‑translatable‑presolution error (rather than skipping the op). (Status: **present**; Evidence: non-translatable paths in `src/MLF/Elab/Phi/Omega.hs` fail with `PhiTranslatabilityError`, and normalized-witness validation in `src/MLF/Constraint/Presolution/WitnessValidation.hs` now rejects `OpRaise` that is not transitively flex-bound to expansion root (`NotTransitivelyFlexBound`); regression coverage in `test/Presolution/WitnessSpec.hs` includes both direct validator and `normalizeEdgeWitnessesM` failure sentinels.)
3. WHEN presolution emits a witness Ω THEN it SHALL be normalized and include Raise/Weaken/Merge/Graft/RaiseMerge operations in the shape expected by Fig. 15.3.4 (or deviations documented). (Status: **present**; Evidence: normalization/validation hooks in `src/MLF/Constraint/Presolution/WitnessCanon.hs`, `src/MLF/Constraint/Presolution/WitnessValidation.hs`, and wiring via `normalizeEdgeWitnessesM` in `src/MLF/Constraint/Presolution/WitnessNorm.hs`; 15-row Fig. 15.3.4 matrix rows `R-GRAFT-VALID-01`..`R-RAISEMERGE-NORM-15` are now explicitly named in `test/Presolution/WitnessSpec.hs` / `test/Presolution/MergeEmissionSpec.hs`, with matrix closure gate green: `cabal test mlf2-test --test-show-details=direct --test-options='--match R-'`.)

## Status Summary
- Semantic thesis-faithfulness requirements in this spec are currently satisfied (all requirement criteria above are present and covered by tests).
- Remaining project deltas are non-semantic assurance/documentation items (proof formalization and documentation sync), tracked in `/Volumes/src/mlf4/TODO.md` and `/Volumes/src/mlf4/implementation_notes.md`.
