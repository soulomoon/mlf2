...
- **Witness translation (`Φ`)**: `xmlf.txt` translates *normalized instance-operation witnesses* (Fig. 10) into xMLF instantiations. This repo records a per-edge `EdgeWitness` during presolution and translates it to an xMLF `Instantiation` via `MLF.Elab.Pipeline.phiFromEdgeWitness`.
  ...
  - Φ requires a representable translation context; missing contexts are reported as errors rather than dropped operations.
- **Quantifier reification (Option B)**: `Q(n)`/reification quantifies all flexibly bound nodes (not just `TyVar` binders) so bounds and contexts remain representable in Φ and generalization.
