...
- **Witness translation (`Φ`) + quantifier reordering (`Σ`)**:
  - Elaboration reifies instantiations from recorded per-edge witnesses (`prEdgeWitnesses`) via `phiFromEdgeWitnessWithTrace` (rather than `expansionToInst`), using `EdgeTrace` for copy maps/interiors.
  ...
  - `phiFromEdgeWitness` targets binders using `InstUnder` contexts (`C{·}`) and reorders binders by ≺ when needed for Raise; missing non-spine contexts are reported as errors, while rigid or non-interior ops are treated as no-ops.
...
- **Witness translation (`Φ`)**: `papers/these-finale-english.txt` translates *normalized instance-operation witnesses* into xMLF instantiations (see `papers/xmlf.txt` Fig. 10). This repo records a per-edge `EdgeWitness` during presolution and translates it to an xMLF `Instantiation` via `MLF.Elab.Pipeline.phiFromEdgeWitness`.
  ...
  - Φ requires a representable translation context; missing non-spine contexts are reported as errors, while rigid or non-interior ops are ignored.
...
- **Quantifier reordering (`Σ(g)`)**: implemented as `MLF.Elab.Sigma.sigmaReorder` (adjacent swaps per thesis Def. 15.3.4 / Fig. 15.3.5; see also `papers/xmlf.txt` §3.4). Φ translation now applies Σ(g) at the start of `phiWithSchemeOmega` whenever the scheme's binder order differs from the <P ordering, independent of whether Ω contains Raise operations. This matches the paper structure: Φ(e) = Σ prefix then Φχe(Ω). The computation is deterministic and fail-fast: missing <P order keys or dependency cycles produce `ElabError` with a stable "PhiReorder:" prefix rather than silently returning `InstId`.
