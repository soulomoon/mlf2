...
- **Quantifier reordering (`Σ(g)`)**: implemented as `MLF.Elab.Pipeline.sigmaReorder` (adjacent swaps per `papers/these-finale-english.txt`; see `papers/xmlf.txt` §3.4). `phiFromEdgeWitness` targets binders using `InstUnder` instantiation contexts (paper’s `C{·}`) rather than swapping quantifiers; `sigmaReorder` remains available as an explicit/validated Σ construction when reordering is required.
