...
|---|---|---|---|---|
| Elaboration input | Elaboration over a translatable presolution `χp` is inductive on term shape; for each instantiation edge `e`, it translates `e` via a chosen propagation witness `I` (`T(e)`), and witness choice is explicitly non-deterministic (Def. 15.3.12, Fig. 15.3.5, §15.3.6). (`papers/these-finale-english.txt`) | Active production elaboration input is `χp`-native: pipeline computes `presolutionViewForGen`, builds `generalizeAtWithView` over `PresolutionView`, and threads `eePresolutionView` into `elaborateWithEnv`; `reifyInst` calls `phiFromEdgeWitnessWithTrace ... presolutionView ...`. However, strict all-path audit (including test-only helpers) still finds solved-typed compatibility surface in `MLF.Elab.Phi.TestOnly`: `phiFromEdgeWitnessNoTrace`, alias `phiFromEdgeWitness`, and `phiFromEdgeWitnessAutoTrace` keep a `Solved` parameter, with auto-trace bridging via `fromSolved solved` + synthetic trace. No-trace remains strict fail-fast (`MissingEdgeTrace`). (`src/MLF/Elab/Run/Pipeline.hs:80-141`, `src/MLF/Elab/Elaborate.hs:58-113`, `src/MLF/Elab/Phi/Translate.hs:240-260`, `src/MLF/Elab/Phi/TestOnly.hs:41-87`, `test/PipelineSpec.hs:176-195`, `test/ElaborationSpec.hs:331-348`) | For absolute thesis-exactness under this table's criterion, retire `Solved`-typed test-only helper signatures (or isolate them as explicit compatibility shims) so all elaboration-input surfaces are `χp`/trace-native. Keep strict no-trace fail-fast (`MissingEdgeTrace`) and current guard suites. | No |
