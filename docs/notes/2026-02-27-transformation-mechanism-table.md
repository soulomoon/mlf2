...
Source revision: `0f23227`
Note: thesis exact include test-only code paths

| Transformation mechanism | Thesis pipeline | Current codebase | What to change for thesis-exact | Thesis-exact |
|---|---|---|---|---|
| Elaboration input | Thesis elaboration is over a translatable presolution `χp`, defined inductively on term shape (Fig. 15.3.5, §15.3.6). For each instantiation edge `e`, translation uses a chosen propagation witness `I` to build `T(e)` (Def. 15.3.12), and witness choice is intentionally non-deterministic ("pick any propagation witness"). (`papers/these-finale-english.txt`) | Active production elaboration input is `χp`-native end-to-end: pipeline computes `presolutionViewForGen`, builds `generalizeAtWithBuilderView` over `PresolutionView`, and threads `eePresolutionView` into `elaborateWithEnv`; Φ translation entrypoint is `phiFromEdgeWitnessWithTrace` with `PresolutionView` + required trace (`MissingEdgeTrace` on absent trace). Under this table's strict criterion (includes test-only paths), solved-typed compatibility helpers still exist in `MLF.Elab.Phi.TestOnly`: `phiFromEdgeWitnessNoTrace`, alias `phiFromEdgeWitness`, and `phiFromEdgeWitnessAutoTrace` each take `Solved`; auto-trace bridges via `fromSolved solved` plus synthetic trace, and tests still call these helpers. (`src/MLF/Elab/Run/Pipeline.hs:80-141`, `src/MLF/Elab/Elaborate.hs:64-118`, `src/MLF/Elab/Phi/Translate.hs:247-260`, `src/MLF/Elab/Phi/TestOnly.hs:49-87`, `test/ElaborationSpec.hs:1521-1521`, `test/ElaborationSpec.hs:1917-1919`) | To mark this row thesis-exact under the current criterion, retire solved-typed test-only helper signatures (or move them behind an explicit compatibility boundary excluded from thesis-exact scope) so all elaboration-input surfaces are `χp`/trace-native. Keep strict no-trace fail-fast (`MissingEdgeTrace`) and existing guard suites. | No |
...
1. Reduce `ResultType.View` solved-overlay scaffolding (`rtvSolved` + bound overlay rebuild path) to a direct `χp`-native query path where possible.
2. Simplify compatibility-heavy helper signatures that still require a `Solved` argument for generalize/reify call chains.
3. Keep `row2 closeout guard`, `checked-authoritative`, and `Dual-path verification` slices in the regular regression cadence.
