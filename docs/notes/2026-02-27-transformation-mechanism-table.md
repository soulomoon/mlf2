...
| Ordering of transformations | `SolveConstraint`: ordered edge traversal with propagation + unification. (`papers/these-finale-english.txt` §12.1.3) | Presolution now drains unification closure at edge boundaries and executes delayed-weaken flushing through owner-boundary scheduling inside the edge loop (with explicit owner-bucket flush + post-boundary empty-owner assertions), instead of loop-final-only fallback. Driver still owns explicit post-loop finalization (`materialize -> rewrite/canonicalize -> rigidify -> witness normalize`) with fail-fast construction checkpoints (`pending queues`, `TyExp` coverage/removal, `witness/trace` key alignment). (`src/MLF/Constraint/Presolution/EdgeProcessing.hs`, `src/MLF/Constraint/Presolution/EdgeUnify.hs`, `src/MLF/Constraint/Presolution/Driver.hs`) | Remaining strict-gap: owner-boundary flushing is compatibility-conservative (flushes all pending owner buckets at boundaries to preserve regressions) rather than a fully proven per-edge owner-local weakest-flush derivation. Keep narrowing toward stricter thesis-shape while preserving §15.2.1 delayed-weaken witness intent and Def. 15.2.10 constructive translatability obligations. | No |
...
1. Keep row2 guardrails (`row2 absolute thesis-exact guard`, `row2 closeout guard`, `checked-authoritative`, `Dual-path verification`) mandatory for any follow-up simplification.
2. Continue reducing non-thesis complexity in non-row2 table items, starting with per-edge propagation transform uniformity (`executeUnifiedExpansionPath` special-case retirement).
3. Maintain full-gate cadence (`cabal build all && cabal test`) after each transformation-row closeout.
