...
|---|---|---|---|---|
| Elaboration input | Thesis elaboration consumes a translatable presolution `χp`; translation of `a` to xMLF is defined inductively on term shape (Fig. 15.3.5, §15.3.6). For each instantiation edge `e`, choose a propagation witness `I` and define `T(e)` from that witness (Def. 15.3.12); witness choice is intentionally non-deterministic ("pick any propagation witness"). (`papers/these-finale-english.txt:14087-14097`, `papers/these-finale-english.txt:14112-14117`) | Active production path is `χp`-native end-to-end: pipeline computes `presolutionViewForGen`, builds `generalizeAtWithBuilderView` over `PresolutionView`, and passes `eePresolutionView` into `elaborateWithEnv`; Φ translation entrypoint `phiFromEdgeWitnessWithTrace` takes `PresolutionView` and fails fast with `MissingEdgeTrace` when trace data is absent. Under this table's strict criterion (includes test-only paths), `MLF.Elab.Phi.TestOnly` no longer exposes solved-typed helper signatures: `phiFromEdgeWitnessNoTrace`, alias `phiFromEdgeWitness`, and `phiFromEdgeWitnessAutoTrace` all take the chi-native `GeneralizeAtWith` shape, and the no-trace helper remains strict fail-fast (`MissingEdgeTrace`). Guard slices assert solved-typed API markers are absent across Elaborate/Phi/TestOnly surfaces. (`src/MLF/Elab/Run/Pipeline.hs:110-141`, `src/MLF/Elab/Elaborate.hs:97-119`, `src/MLF/Elab/Phi/Translate.hs:241-260`, `src/MLF/Elab/Phi/TestOnly.hs:39-75`, `test/ElaborationSpec.hs:331-356`, `test/ElaborationSpec.hs:1901-1928`, `test/PipelineSpec.hs:176-203`) | Closed in this migration. Keep strict-policy guardrails (including test-only paths): maintain no-trace fail-fast (`MissingEdgeTrace`) and keep `elab-input thesis-exact guard`, `checked-authoritative`, `Dual-path verification`, and full gate (`cabal build all && cabal test`) mandatory for regressions. | Yes (strict policy includes test-only paths; gates green) |
