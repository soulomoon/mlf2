...
|---|---|---|---|---|
| Elaboration input | Elaboration over a translatable presolution `χp` is inductive on term shape; for each instantiation edge `e`, it translates `e` via a chosen propagation witness `I` (`T(e)`), and witness choice is explicitly non-deterministic (Def. 15.3.12, Fig. 15.3.5, §15.3.6). (`papers/these-finale-english.txt`) | Active production elaboration input is `χp`-native: pipeline computes `presolutionViewForGen`, builds `generalizeAtWithView` over `PresolutionView`, and threads `eePresolutionView` into `elaborateWithEnv`; `reifyInst` calls `phiFromEdgeWitnessWithTrace ... presolutionView ...`. Production elaboration/Phi modules no longer expose solved-typed compatibility entrypoints: `MLF.Elab.Elaborate` exports only `ElabConfig`/`ElabEnv` + `elaborateWithEnv`, and `MLF.Elab.Phi.Translate`/`MLF.Elab.Phi` expose only `phiFromEdgeWitnessWithTrace` on the active path. Test-only Φ helpers keep debug helpers but now use chi-native callback shape (`GeneralizeAtWith` has no solved argument), while no-trace remains strict fail-fast (`MissingEdgeTrace`). Guard suites lock symbol retirement and parity invariants. (`src/MLF/Elab/Run/Pipeline.hs:80-141`, `src/MLF/Elab/Elaborate.hs:4-8`, `src/MLF/Elab/Elaborate.hs:55-112`, `src/MLF/Elab/Phi/Translate.hs:31-34`, `src/MLF/Elab/Phi/Translate.hs:247-256`, `src/MLF/Elab/Phi.hs:11-23`, `src/MLF/Elab/Phi/TestOnly.hs:41-62`, `test/PipelineSpec.hs:176-188`, `test/ElaborationSpec.hs:331-343`) | N/A for row exactness. Keep `elab-input thesis-exact guard`, `checked-authoritative`, and `Dual-path verification` as mandatory maintenance slices, and preserve strict no-trace fail-fast invariants in test-only Φ helpers. | Yes |
