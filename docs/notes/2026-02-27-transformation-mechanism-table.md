...
| Identity reconciliation mechanism | Thesis elaboration carries identity directly through named nodes, computation contexts, witness-derived computations (`ε`, `ϕR`, `T(e)`), and quantifier-spine order; it does not define a separate identity-reconciliation layer. (`papers/these-finale-english.txt:13547-13553`, `:13690-13707`, `:13827-13830`, `:13855-13858`, `:13942-13954`, `:14087-14097`, `:14114-14124`) | Production Φ identity handling is still split across implementation-only reconciliation helpers: `Translate` validates replay-bridge artifacts, `Omega` constructs `IdentityBridge` to recover/rank raw-copy-trace source candidates, and binder-spine lookup is handled by stricter local exact-id checks. This is much closer to the thesis semantics than earlier fallback-heavy versions, but the runtime object model still contains a dedicated reconciliation layer not present in the thesis. A further internal helper surface, `MLF.Elab.Phi.Binder`, also retains older canonical/base-key/copy-map reconciliation and is re-exported from `MLF.Elab.Phi`. (`src/MLF/Elab/Phi/Translate.hs:269-348`, `src/MLF/Elab/Phi/Omega.hs:120-126`, `:630-650`, `:1231-1318`, `src/MLF/Elab/Phi/IdentityBridge.hs:139-246`, `src/MLF/Elab/Phi/Binder.hs:29-62`, `src/MLF/Elab/Phi.hs:22-30`) | Collapse Φ identity handling into direct thesis-shaped witness/replay-domain translation: remove the dedicated runtime reconciliation subsystem (`IdentityBridge` split from `Translate`/`Omega`) and retire or rewrite `MLF.Elab.Phi.Binder` so no compiled Phi code preserves a separate non-thesis identity-reconciliation mechanism. | No |
