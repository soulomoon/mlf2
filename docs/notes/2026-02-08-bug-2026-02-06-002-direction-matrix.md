...
3. Focused regressions remain green:
   ...
   - `BUG-2026-02-08-004` dedicated thesis sentinel (`Int` in checked + unchecked pipelines)
...
### 2026-02-10 step-1/2/3 batch (superseded by final cleanup)
...
- This interim note is superseded by the final closure below.

## Final Winner / Result (2026-02-10)

- Winner patch set retained:
  - C18 (`src/MLF/Elab/Phi/Omega.hs`): translate delayed `graft→weaken` binder paths as `InstApp` and rescue binder arg `TBottom` to binder `TVar` when binder identity is known.
  - C21/C21.1 (`src/MLF/Elab/Elaborate.hs`): env-aware `ALet` fallback scheme derivation + `AApp` `InstApp TForall` repair.
  - Follow-up hardening (`src/MLF/Constraint/Presolution/WitnessValidation.hs`): explicit delayed-weaken validation (`DelayedWeakenViolation`) to reject ambiguous graft/weaken witness layouts.

- Hard-gate closure:
  1. `BUG-2026-02-06-002` strict target matrix: **PASS** (`4 examples, 0 failures`).
  2. Binder-level let polymorphism preserved: **PASS** (strict/theory target + focused guards remain green; no forced `make : ... -> Int` specialization).
  3. Focused regressions: **PASS**.
     - `generalizes reused constructors via make const`: PASS.
     - `redirected let-use sites keep polymorphic schemes`: PASS.
     - `BUG-2026-02-08-004` dedicated sentinel: PASS as thesis-green target (unchecked + checked both return `Int`).
  4. Full verification: **PASS** (`cabal build all && cabal test`).
  5. Thesis defensibility: **PASS**.
     - The retained path is thesis-faithful: graft and weaken are not reordered globally; translation handles delayed weaken consumption only when witness pairing is explicitly present, while validator checks prevent ambiguous pairings.

- Final decision:
  - Direction-only D1..D6 all rejected.
  - Minimal winning combination is post-matrix C18 + C21/C21.1 with delayed-weaken witness validation guardrails.
