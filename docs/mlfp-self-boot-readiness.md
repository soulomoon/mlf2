...
- After that corpus exists, target native-capable broad string/`Char` substrate
  ...
  `stringIsEmpty` classification tracer, and `stringContainsChar` scalar search
  tracer are not the complete broad string library; continue adding the Prelude-level
