...
| Standard library | The built-in Prelude supplies `Nat`, `Option`, `List`, `Eq`, `Unit`, opaque `IO`, minimal `Monad IO`, `pure`, `bind`, `putStrLn`, `stringLength`, `stringIsEmpty`, `stringContainsChar`, `stringContains`, `stringStartsWith`, `stringEndsWith`, `stringAppend`, `stringReplaceChar`, `stringReplace`, `stringIndexOfChar`, `stringIndexOf`, `stringFromChar`, `stringFromInt`, `stringFromBool`, `stringFromNat`, `stringFromUnit`, `stringFromList`, `stringToList`, `stringDrop`, `stringTake`, `stringSlice`, `stringCharAt`, `charIsDigit`, `charIsAsciiLower`, `charIsAsciiUpper`, `charIsAsciiAlpha`, `charIsAsciiAlphaNum`, `charIsAsciiIdentifierStart`, `charIsAsciiIdentifierContinue`, `charIsAsciiWhitespace`, `charIsAsciiPunctuation`, `charIsAsciiPrintable`, `and`, and `id`. The lexer seed deliberately uses seed-owned monomorphic symbolic input and token-stream ADTs instead of pretending a general source-text or collection library exists. | The seed-driven budget below identifies which stdlib gaps block a larger compiler component and which remain deferred. First self-boot additionally requires a **Prelude Package/Substrate Split**: high-level Prelude/library source should live in the **Shared Local `.mlfp` Package Set** consumed by both compiler implementations where feasible, while primitive/native operations, GC/runtime hooks, and low-level FFI shims remain declared trusted substrate. |
...
not claim splitting, regex, Unicode normalization, locale behavior, parser
...
`None`; it does not claim broader substring index APIs, splitting, regex, Unicode
...
`Some 0` for the empty needle; it does not claim splitting,
replacement-family completion beyond `stringReplace`, regex, Unicode
normalization, locale behavior, complete cursor APIs, parser parity, platform
contracts, driver work, or self-boot proof completion.
...
not claim splitting, split-family collection APIs, regex, Unicode
normalization, locale behavior, case conversion, interpolation, formatting,
parser parity, platform contracts, replacement-family completion beyond this
exact operation, driver work, or self-boot proof completion.
...
|----------|---------------|----------------------------------|----------------|-------------|
| Text, characters, or strings | `SeedSource` models source as closed `SeedInputSymbol` values and symbolic `SourceSpan` labels. The lexer never inspects raw `String`, `Char`, substrings, or offsets. | Source checking knows `String` literals, the Prelude exposes string-valued IO, and the first source-level Unicode scalar `Char` literal tracer plus the next two-byte Unicode scalar `String` literal tracer check, run, emit LLVM, validate object code, and execute natively. The first broad `String` operation, `stringLength : String -> Int`, now counts Unicode scalar values through source checking, `run-program`, raw LLVM, object validation, and linked native execution. The first broad `String` classification operation, `stringIsEmpty : String -> Bool`, classifies `""` and the non-empty Unicode scalar string `"λ"` through the same layers. The first single-character `String`/`Char` search operation, `stringContainsChar : String -> Char -> Bool`, now classifies `stringContainsChar "aλb" 'λ'` as `true` and `stringContainsChar "ab" 'λ'` as `false` through the same layers. The first substring `String` search operation, `stringContains : String -> String -> Bool`, now classifies `stringContains "aλb" "λ"` as `true` and `stringContains "ab" "λ"` as `false` through the same layers. The first prefix `String` search operation, `stringStartsWith : String -> String -> Bool`, now classifies `stringStartsWith "λab" "λ"` as `true` and `stringStartsWith "aλb" "λ"` as `false` through the same layers. The first suffix `String` search operation, `stringEndsWith : String -> String -> Bool`, now classifies `stringEndsWith "abλ" "λ"` as `true` and `stringEndsWith "λab" "λ"` as `false` through the same layers. The first append `String` operation, `stringAppend : String -> String -> String`, now classifies `stringAppend "aλ" "b"` as `"aλb"`, `stringAppend "" "λ"` as `"λ"`, and `stringAppend "λ" ""` as `"λ"` through the same layers. The first character replacement `String` operation, `stringReplaceChar : String -> Char -> Char -> String`, now classifies `stringReplaceChar "aλbλ" 'λ' 'x'` as `"axbx"` and the no-match input `stringReplaceChar "ab" 'λ' 'x'` as `"ab"` through the same layers. The first substring replacement `String` operation, `stringReplace : String -> String -> String -> String`, now classifies `stringReplace "aλbλb" "λb" "WXYZ"` as `"aWXYZWXYZ"`, `stringReplace "abc" "λ" "x"` as `"abc"`, and `stringReplace "abc" "" "x"` as `"abc"` through the same layers. The first `String`/`Char` first-match index search operation, `stringIndexOfChar : String -> Char -> Option Int`, now classifies `stringIndexOfChar "aλbλ" 'λ'` as `Some 1` and `stringIndexOfChar "ab" 'λ'` as `None` through the same layers. The first substring index search operation, `stringIndexOf : String -> String -> Option Int`, now classifies `stringIndexOf "aλbcλ" "λb"` as `Some 1`, `stringIndexOf "abc" "λ"` as `None`, and `stringIndexOf "λ" ""` as `Some 0` through the same layers. The first `Char` to singleton `String` construction operation, `stringFromChar : Char -> String`, now classifies `stringFromChar 'λ'` as `"λ"` and `stringFromChar 'A'` as `"A"` through the same layers. The first decimal `Int` to `String` conversion operation, `stringFromInt : Int -> String`, now formats `stringFromInt 42` as `"42"` and `stringFromInt 0` as `"0"` through the same layers. The first `Bool` to `String` conversion operation, `stringFromBool : Bool -> String`, now formats `stringFromBool true` as `"true"` and `stringFromBool false` as `"false"` through the same layers. The first `Nat` to `String` conversion operation, `stringFromNat : Nat -> String`, now formats `stringFromNat Zero` as `"0"` and `stringFromNat (Succ (Succ Zero))` as `"2"` through the same layers. The first `Unit` to `String` conversion operation, `stringFromUnit : Unit -> String`, now formats `stringFromUnit Unit` as `"Unit"` through a pure Prelude definition and the same layers. The first `List Char` to `String` conversion operation, `stringFromList : List Char -> String`, now classifies `stringFromList (Cons 'a' (Cons 'λ' Nil))` as `"aλ"` and `stringFromList Nil` as `""` through the same layers. The first `String` to `List Char` conversion operation, `stringToList : String -> List Char`, now classifies `stringToList "aλ"` as `Cons 'a' (Cons '\955' Nil)` and `stringToList ""` as `Nil` through the same layers. The first drop slicing `String` operation, `stringDrop : String -> Int -> String`, now classifies `stringDrop "λab" 1` as `"ab"` and `stringDrop "aλb" 2` as `"b"` through the same layers. The first take slicing `String` operation, `stringTake : String -> Int -> String`, now classifies `stringTake "λab" 1` as `"λ"` and `stringTake "aλb" 2` as `"aλ"` through the same layers. The first range slicing `String` operation, `stringSlice : String -> Int -> Int -> String`, now classifies `stringSlice "aλbc" 1 2` as `"λb"` and `stringSlice "λabc" 1 2` as `"ab"` through the same layers. The first in-range cursor/index `String` operation, `stringCharAt : String -> Int -> Char`, now classifies `stringCharAt "aλb" 1` as `'λ'` and `stringCharAt "λab" 2` as `'b'` through the same layers. The first `Char` classification operation, `charIsDigit : Char -> Bool`, now classifies `charIsDigit '7'` as `true` and `charIsDigit 'λ'` as `false` through the same layers. The explicit ASCII lowercase `Char` classification helper, `charIsAsciiLower : Char -> Bool`, now classifies `charIsAsciiLower 'a'` as `true`, `charIsAsciiLower 'A'` as `false`, and `charIsAsciiLower 'λ'` as `false` through the same layers. The explicit ASCII uppercase `Char` classification helper, `charIsAsciiUpper : Char -> Bool`, now classifies `charIsAsciiUpper 'A'` as `true`, `charIsAsciiUpper 'a'` as `false`, and `charIsAsciiUpper 'λ'` as `false` through the same layers. The explicit ASCII alphabetic `Char` classification helper, `charIsAsciiAlpha : Char -> Bool`, now classifies `charIsAsciiAlpha 'a'` and `charIsAsciiAlpha 'A'` as `true`, and `charIsAsciiAlpha '7'` and `charIsAsciiAlpha 'λ'` as `false` through the same layers. The explicit ASCII alphanumeric `Char` classification helper, `charIsAsciiAlphaNum : Char -> Bool`, now classifies `charIsAsciiAlphaNum 'a'`, `charIsAsciiAlphaNum 'A'`, and `charIsAsciiAlphaNum '7'` as `true`, while `charIsAsciiAlphaNum '_'` and `charIsAsciiAlphaNum 'λ'` are `false` through the same layers. The explicit ASCII identifier-start `Char` classification helper, `charIsAsciiIdentifierStart : Char -> Bool`, now matches the current parser start set by classifying `charIsAsciiIdentifierStart 'a'`, `charIsAsciiIdentifierStart 'A'`, and `charIsAsciiIdentifierStart '_'` as `true`, while `charIsAsciiIdentifierStart '7'`, `charIsAsciiIdentifierStart '\''`, and `charIsAsciiIdentifierStart 'λ'` are `false` through the same layers. The explicit ASCII identifier-continuation `Char` classification helper, `charIsAsciiIdentifierContinue : Char -> Bool`, now matches the current parser continuation set by classifying `charIsAsciiIdentifierContinue 'a'`, `charIsAsciiIdentifierContinue 'A'`, `charIsAsciiIdentifierContinue '7'`, `charIsAsciiIdentifierContinue '_'`, and `charIsAsciiIdentifierContinue '\''` as `true`, while `charIsAsciiIdentifierContinue 'λ'` is `false` through the same layers. The explicit ASCII whitespace `Char` classification helper, `charIsAsciiWhitespace : Char -> Bool`, now classifies ASCII space, tab, newline, carriage return, form feed, and vertical tab as `true`, while `charIsAsciiWhitespace 'a'` and `charIsAsciiWhitespace 'λ'` are `false` through the same layers. The explicit ASCII punctuation `Char` classification helper, `charIsAsciiPunctuation : Char -> Bool`, now classifies ASCII punctuation ranges `0x21..0x2f`, `0x3a..0x40`, `0x5b..0x60`, and `0x7b..0x7e` as `true`, while `charIsAsciiPunctuation 'a'`, `charIsAsciiPunctuation '7'`, `charIsAsciiPunctuation ' '`, and `charIsAsciiPunctuation 'λ'` are `false` through the same layers. The explicit ASCII printable `Char` classification helper, `charIsAsciiPrintable : Char -> Bool`, now classifies ASCII scalar values `0x20..0x7e` as `true`, while `charIsAsciiPrintable '\t'`, `charIsAsciiPrintable '\n'`, and `charIsAsciiPrintable 'λ'` are `false` through the same layers. Formatting, full slicing coverage, broader String/List Char collection APIs, broader classification families, complete cursor APIs, broader substring index APIs, replacement-family completion beyond `stringReplace`, splitting, regex, Unicode normalization, locale behavior, and parser parity for `.mlfp` code remain open. | `needed-before-larger-compiler` | Per `docs/adr/2026-05-18-native-broad-string-library-before-parser-parity.md`, continue adding a Prelude-level broad Unicode-scalar string/`Char` library with native slicing/classification before replacing the symbolic lexer with a real source-text parser. |
...
- After that corpus exists, target native-capable broad string/`Char` substrate
  ...
  tracer, `stringContains` substring search tracer, `stringStartsWith` prefix
  search tracer, `stringEndsWith` suffix search tracer, `stringAppend`
  ...
  Unicode-scalar substring index search tracer, `stringFromChar`
  ...
  in-range cursor/index tracer, and
