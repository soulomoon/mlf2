...
|----------|---------------|----------------------------------|----------------|-------------|
| Text, characters, or strings | `SeedSource` models source as closed `SeedInputSymbol` values and symbolic `SourceSpan` labels. The lexer never inspects raw `String`, `Char`, substrings, or offsets. | Source checking knows `String` literals and the Prelude exposes string-valued IO, but there is no source-level `Char`, Unicode-scalar string operation, substring, search, formatting, or cursor API for `.mlfp` code. | `needed-before-larger-compiler` | Per `docs/adr/2026-05-18-native-broad-string-library-before-parser-parity.md`, add a Prelude-level broad Unicode-scalar string/`Char` library with native slicing/classification before replacing the symbolic lexer with a real source-text parser. |
...
- After that corpus exists, target native-capable broad string/`Char` substrate
  plus full canonical `.mlfp` parser parity. Start that later family by adding
  the Prelude-level Unicode-scalar string library required by
