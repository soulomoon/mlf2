...
- After that corpus exists, target native-capable broad string/`Char` substrate
  ...
  literal tracers plus the first `stringLength` operation tracer and first
  `stringIsEmpty` classification tracer are not the complete broad string
  library; continue adding the Prelude-level
  Unicode-scalar string library required by
