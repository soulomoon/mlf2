...
| Parser/lexer | The Haskell parser accepts the current `.mlfp`, eMLF, and xMLF syntax and rejects retired compatibility spellings. The `.mlfp` lexer seed tokenizes one bounded symbolic `def main = true` input and returns an unknown-symbol diagnostic for a negative symbolic input. The `.mlfp` parser seed consumes the seed token stream, returns a definition AST for the accepted token stream, and returns an expected-equals diagnostic for a malformed token stream. Parser parity tracers now consume fixed source-text fixtures through one shared parser-owned source/token/projection/lexer/parser library: the basic Bool module, a single `import Prelude exposing (Bool);` fixture with import module and exposing-item source-span evidence plus a malformed import semicolon diagnostic, an `import Prelude exposing (Int);` fixture with two `Int` value definitions plus malformed value-definition semicolon evidence, an `import Prelude exposing (Int);` fixture with one let/lambda/application definition plus malformed missing-`in` evidence, an `import Prelude exposing (Int);` fixture with typed let annotations, annotated lambda parameters, expression annotations, arrow source types, forall source types, and malformed annotation evidence, a `Nat(..)` export/data-declaration fixture with constructor declaration spans, `Nat -> Nat` source-type rendering, constructor application rendering, and malformed constructor-colon evidence, two case-expression fixtures with constructor, wildcard, and nested patterns plus malformed branch-arrow evidence, two typeclass/instance fixtures with class method signatures, deriving clauses, instance method definitions, and malformed instance-method-definition evidence, two higher-kinded/constrained class fixtures with kinded declaration parameters, variable-headed type applications, constructor type applications, superclass constraints, multi-parameter class heads, Unicode functional dependencies, empty instance bodies, and malformed functional-dependency arrow evidence, two closed type-family/type-level fixtures with optional module exports, kinded and plain family parameters, source kind variables, family equation lists, constructor and variable type-level patterns, type-level lambdas/applications, source type-family annotations, and malformed type-family-equation evidence, two recursive-ADT fixtures with parameterized data heads, GADT-style constructor results, constructor-local Unicode `∀`, nested constructor field/result types, related case patterns, and malformed constructor-local forall-dot evidence, two qualified import/reference fixtures with import aliases, alias-only imports, exposed classes/types/constructors/values/methods, qualified value/type/constructor/class/method references, and malformed import-alias evidence, two complete multi-module export/import fixtures with abstract exports/imports, recursive ADT constructor exports/imports, cross-module references, and malformed import-exposing separator evidence, the authoritative recursive-let fixture with a `Nat` declaration, typed local recursive let, and case-expression diagnostics, the authoritative cross-module let-polymorphism fixture with one-definition `Core`/`User` modules and malformed definition-semicolon evidence, four exact authoritative unified fixtures covering case analysis, importless let-polymorphism with no import rows, nullary overloaded `Monoid` methods, overloaded `Eq` deriving methods, and malformed importless let-polymorphism definition-semicolon evidence, the recursive-ADT plain Nat fixture with `NatPlain`, top-level annotated-lambda case definitions, nested constructor/application expressions, and malformed case-branch-arrow evidence, the recursive-list tail fixture with paired `Nat`/`List` declarations, a two-field `Cons` constructor, two-argument list case patterns, nested `isNil`/`tailOrNil`/`Cons` application, and malformed case-branch-arrow evidence, two recursive-tree fixtures with two-field `Branch` constructor syntax, two-argument tree case patterns, wildcard patterns, recursive `mirror`, nested constructor/application expressions, `data Tree ... deriving Eq`, and malformed case-branch-arrow evidence, the recursive ADT/typeclass integration fixture with an `Eq` class, recursive `Nat` declaration, explicit `Eq Nat` instance method with nested case expressions, a `same` wrapper, nested constructor-application `main`, and malformed nested case branch-arrow evidence, the abstract recursive ADT module-use fixture with abstract `Nat` exports/imports and imported destructor calls, the module-integrated recursive existential fixture with `Eq`, `Nat(..) deriving Eq`, `Expr(..)`, `SomeExpr(..)`, `peel`, `peelSome`, nested existential constructor applications, and malformed `SomeExpr` case branch-arrow evidence, and the complex recursive program fixture with `ComplexRecursiveProgram`, `Eq`, `Nat(..) deriving Eq`, `Tree(..)`, recursive `mirror`, `leftDepth`, `rightDepth`, nested `leftDepth`/`rightDepth`/`mirror`/`Branch` applications, and malformed Tree traversal branch-arrow evidence. | There is no broad `.mlfp` source-text lexer/parser implementation, no character or byte stream abstraction, and no parser bootstrap corpus beyond the seed plus these bounded parser-parity fixtures. |
...
| Fixture evidence | Static fixture files cover trivial file-as-package inputs, multi-file package roots, ordered search paths, runtime parity rows, backend emission over package mode, the compiler frontend seed contract fixture, the bounded lexer evidence fixture, the bounded parser/AST evidence fixture, the bounded parser-program projection artifacts for the basic Bool definition, import-exposing Bool, value-definition-list Int/reference, let/lambda/application Int, typed-annotation Int, data-declaration Nat, case-expression Nat, typeclass/instance declaration, higher-kinded/constrained class, closed type-family/type-level, GADT/existential constructor, qualified import/reference, multi-module export/import, authoritative recursive-let, authoritative cross-module let-polymorphism, exact authoritative unified case-analysis, authoritative let-polymorphism, authoritative nullary overloaded-method, authoritative overloaded-method, recursive-ADT plain Nat, recursive-list tail, recursive-tree first-order, recursive-tree deriving, recursive ADT/typeclass integration, abstract recursive ADT module-use, module-integrated recursive existential, and complex recursive program parser parity tracers, and bounded compiler-seed backend/native execution for the current package entrypoint. | Fixtures do not prove broad source-text lexing/parsing, separate compilation, stable ABI/linking, **Self-Boot Total Native Coverage**, or self-hosting. |

Round 339 extends the parser/lexer and fixture-evidence rows above with a
bounded parser-parity fixture copied byte-for-byte from the compiler-seed
frontend lexer source `SeedLexer.mlfp`. The evidence covers the selected lexer
module header, three imports, lexer result/evidence ADTs, repeated lexer-state
case functions, seven-argument lexer-state applications, nested token-stream
constructor applications, source span preservation through line 227, and one
malformed lexer-source case-branch diagnostic through the shared parser-owned
source-text library. It does not claim full parser parity,
resolver/checker/backend progress, package resolver behavior,
compiler-package progress, platform work, driver work, proof work, or
self-boot completion.

Round 338 extends the parser/lexer and fixture-evidence rows above with a
bounded parser-parity fixture copied byte-for-byte from the compiler-seed
frontend data-model sources `SeedSource.mlfp`, `SeedToken.mlfp`,
`SeedDiagnostic.mlfp`, and `SeedAst.mlfp`. The evidence covers stable
four-source package projection order, selected seed data declarations,
imports/exports, repeated five-branch case definitions, nested seed input
constructor applications, and malformed selected case-branch diagnostics
through the shared parser-owned source-text library. It does not claim full
parser parity, resolver/checker/backend progress, package resolver behavior,
compiler-package progress, platform work, driver work, proof work, or
self-boot completion.

Round 336 extends the parser/lexer and fixture-evidence rows above with
bounded parser-parity fixtures copied from the exact authoritative unified
corpus sources `authoritative-case-analysis.mlfp`,
`authoritative-let-polymorphism.mlfp`,
`authoritative-nullary-overloaded-method.mlfp`, and
`authoritative-overloaded-method.mlfp`, covering the selected case,
importless let/lambda/application, class/instance, and deriving syntax
families through the shared parser-owned source-text library, plus malformed
importless let-polymorphism definition-semicolon diagnostics. The evidence
does not claim full parser parity, resolver/checker/backend progress,
compiler-package progress, platform work, driver work, proof work, or
self-boot completion.

Round 335 extends the parser/lexer and fixture-evidence rows above with a
bounded parser-parity slice for the exact named recursive-ADT corpus source
modules `DerivingEq`, `RecursiveGadt`, and `RecursiveExistential`, covering
the selected deriving, GADT, and existential recursive-ADT syntax families
through the shared parser-owned source-text library, plus malformed named
recursive-ADT case branch-arrow diagnostics. The evidence removes the retired
`Main`-only static recursive-ADT fallback for these surfaces and does not
claim full parser parity, resolver/checker/backend progress, compiler-package
progress, platform work, driver work, proof work, or self-boot completion.

Round 334 extends the parser/lexer and fixture-evidence rows above with a
bounded parser-parity fixture for complex recursive program source text,
including `module ComplexRecursiveProgram export (Eq, Nat(..), Tree(..), eq,
mirror, leftDepth, rightDepth, main)`, an `Eq` class, recursive `Nat` with
`deriving Eq`, recursive binary `Tree`, recursive `mirror`, `leftDepth`, and
`rightDepth` traversals, nested `leftDepth`/`rightDepth`/`mirror`/`Branch`
applications in `main`, and malformed Tree traversal branch-arrow diagnostics.
The evidence continues to use the shared parser-owned source-text library and
does not claim full parser parity, resolver/checker/backend progress, platform
work, driver work, proof work, or self-boot completion.

Round 333 extends the parser/lexer and fixture-evidence rows above with a
bounded parser-parity fixture for module-integrated recursive existential
source text, including `module Core export (Eq, Nat(..), Expr(..),
SomeExpr(..), eq)`, an `Eq` class, recursive `Nat` with `deriving Eq`,
recursive `Expr a`, existential `SomeExpr`, `module User export (peelSome,
main)`, the four-item constructor/value import surface, `peel`, `peelSome`,
nested existential constructor applications in `main`, and malformed
`SomeExpr` case branch-arrow diagnostics. The evidence continues to use the
shared parser-owned source-text library and does not claim full parser parity,
resolver/checker/backend progress, platform work, driver work, proof work, or
self-boot completion.

Round 332 extends the parser/lexer and fixture-evidence rows above with a
bounded parser-parity fixture for abstract recursive ADT module-use source
text, including `module Core export (Nat, zero, succ, peel, isZero)`, an
abstractly exported recursive `Nat`, constructor-building and destructor
functions `zero`, `succ`, `peel`, and `isZero`, `module User export (main)`,
the five-item `import Core exposing (Nat, zero, succ, peel, isZero);` surface,
imported `main` as `isZero (peel (succ zero))`, and malformed destructor-case
branch-arrow diagnostics. The evidence continues to use the shared
parser-owned source-text library and does not claim full parser parity,
resolver/checker/backend progress, platform work, driver work, proof work, or
self-boot completion.

Round 331 extends the parser/lexer and fixture-evidence rows above with a
bounded parser-parity fixture for recursive ADT/typeclass integration source
text, including `module TypeclassIntegration export (Eq, Nat(..), eq, same,
main)`, an `Eq` class, recursive `Nat` declaration, explicit `Eq Nat`
instance method with nested `case left` / `case right` expressions, a `same`
wrapper, nested constructor-application `main`, and malformed nested-case
branch-arrow diagnostics. The evidence continues to use the shared
parser-owned source-text library and does not claim full parser parity,
resolver/checker/backend progress, platform work, driver work, proof work, or
self-boot completion.

Round 330 extends the parser/lexer and fixture-evidence rows above with two
bounded parser-parity fixtures for recursive tree source text, including
`recursive-tree-first-order` with `Tree`, `Leaf`, `Branch`, recursive `mirror`,
`isBranch`, two-argument constructor patterns, wildcard patterns, and nested
constructor/application expressions, plus `recursive-tree-deriving` with
`class Eq`, `data Tree ... deriving Eq`, and `eq (Branch Leaf Leaf) (Branch
Leaf Leaf)`. The evidence continues to use the shared parser-owned source-text
library and does not claim full parser parity, resolver/checker/backend
progress, platform work, driver work, proof work, or self-boot completion.

Round 329 extends the parser/lexer and fixture-evidence rows above with a
bounded parser-parity fixture for recursive list-tail source text, including
`module RecursiveList export (Nat(..), List(..), tailOrNil, isNil, main)`,
paired `Nat` and `List` data declarations, a two-field
`Cons : Nat -> List -> List` constructor, list case expressions with
`Cons _ rest` and `Cons _ _` patterns, nested
`isNil (tailOrNil (Cons Zero Nil))` application, and malformed missing
case-branch-arrow diagnostics. The evidence continues to use the shared
parser-owned source-text library and does not claim full parser parity,
resolver/checker/backend progress, platform work, driver work, proof work, or
self-boot completion.

Round 328 extends the parser/lexer and fixture-evidence rows above with a
bounded parser-parity fixture for recursive-ADT plain Nat source text,
including `module NatPlain export (Nat(..), isZero, peel, main)`, a recursive
`Nat` data declaration, top-level `isZero` and `peel` annotated-lambda case
expressions, nested constructor/application expressions in `main`, and
malformed missing case-branch-arrow diagnostics. The evidence continues to use
the shared parser-owned source-text library and does not claim full parser
parity, resolver/checker/backend progress, platform work, driver work, proof
work, or self-boot completion.

Round 327 extends the parser/lexer and fixture-evidence rows above with a
bounded parser-parity fixture for authoritative cross-module let-polymorphism
source text, including a one-definition `Core` module exporting `applyId`,
`let id = λx x in id 1`, a `User` import and reference to `applyId`, and
malformed missing definition-semicolon diagnostics. The evidence continues to
use the shared parser-owned source-text library and does not claim full parser
parity, resolver/checker/backend progress, platform work, driver work, proof
work, or self-boot completion.

Round 326 extends the parser/lexer and fixture-evidence rows above with a
bounded parser-parity fixture for authoritative recursive-let source text,
including a `Nat` data declaration, typed local recursive let, annotated-lambda
RHS case expression, outer case over constructor application, and malformed
missing case-branch-arrow diagnostics. The evidence continues to use the shared
parser-owned source-text library and does not claim full parser parity,
resolver/checker/backend progress, platform work, driver work, proof work, or
self-boot completion.

Round 325 extends the parser/lexer and fixture-evidence rows above with a
bounded parser-parity fixture for higher-order function fields through a
function-valued constructor field and a bounded constructor-pattern case branch
that applies the extracted function, plus malformed missing case-branch-arrow
diagnostics. The evidence continues to use the shared parser-owned source-text
library and does not claim full parser parity, resolver/checker/backend
progress, platform work, driver work, proof work, or self-boot completion.

Round 323 extends the parser/lexer and fixture-evidence rows above with a
bounded parser-parity fixture for higher-order returned functions through an
annotated lambda whose typed local let body returns another annotated lambda,
plus parenthesized function-valued callee application and malformed
missing-close-paren diagnostics. The evidence continues to use the shared
parser-owned source-text library and does not claim full parser parity,
resolver/checker/backend progress, platform work, driver work, proof work, or
self-boot completion.
...
atoms, and malformed missing-close-paren diagnostics. The evidence continues
to use the shared parser-owned source-text library and does not claim full
parser parity, resolver/checker/backend progress, platform work, driver work,
proof work, or self-boot completion.

Round 318 extends the parser/lexer and fixture-evidence rows above with
bounded parser-parity fixtures for complete programs containing `Core` and
`User` modules, selected abstract export/import items, recursive ADT
constructor export/import items, cross-module references, and malformed
import-exposing separator diagnostics. The evidence continues to use the
shared parser-owned source-text library and does not claim full parser parity,
resolver/checker/backend progress, platform work, driver work, proof work, or
self-boot completion.

Round 317 extends the parser/lexer and fixture-evidence rows above with
bounded parser-parity fixtures for qualified import aliases, alias-only
imports, exposed classes/types/constructors/values/methods, qualified
value/type/constructor/class/method references, and malformed import-alias
diagnostics. The evidence continues to use the shared parser-owned source-text
library and does not claim full parser parity, resolver/checker/backend
progress, platform work, driver work, proof work, or self-boot completion.

Round 316 extends the parser/lexer and fixture-evidence rows above with
bounded parser-parity fixtures for parameterized data declarations,
GADT-style constructor result heads, constructor-local Unicode `∀` signatures,
nested source-type applications in constructor fields/results, related
constructor/case patterns, and malformed constructor-local forall-dot
diagnostics. The evidence continues to use the shared parser-owned source-text
library and does not claim full parser parity, checker/resolver/backend
progress, platform work, driver work, proof work, or self-boot completion.

Round 315 extends the parser/lexer and fixture-evidence rows above with
bounded parser-parity fixtures for closed type-family declarations, kinded and
plain family parameters, source kind variables, type-level constructor and
variable equation patterns, type-level lambda/application RHS syntax, and
type-family-style source type annotations. The evidence continues to use the
shared parser-owned source-text library and does not claim full parser parity,
type-family checker or reducer support, checker/backend progress, platform
work, driver work, proof work, or self-boot completion.
...
Round 313 extends the same shared parser-owned source-text parser-parity
library with a bounded typeclass/instance declaration slice. It proves
committed parser-program projections for `class Eq a` with a method signature
and `deriving Eq`, plus `class Monoid a` with an `instance Monoid Nat` and
method definitions for `mempty` and `append`. It also records malformed
instance-method-definition evidence through `run-program`. Full parser parity,
checker/backend/platform/compiler package/driver/proof work, and self-boot
evidence remain open.

Round 314 extends the same shared parser-owned source-text parser-parity
library with a bounded higher-kinded and constrained class syntax slice. It
proves committed parser-program projections for a kinded `Functor` class and
`Higher` data declaration, plus a multi-parameter `Monad` class with a
superclass constraint, Unicode functional dependency, and empty instance body.
It also records malformed functional-dependency arrow evidence through
`run-program`. Full parser parity, type-family parity,
checker/backend/platform/compiler package/driver/proof work, and self-boot
evidence remain open.

Round 315 extends the same shared parser-owned source-text parser-parity
library with a bounded closed type-family and type-level syntax slice. It
proves committed parser-program projections for kinded and plain family
parameters, source kind variables, family equations, type-level
lambda/application syntax, and source type-family annotations, plus malformed
type-family-equation evidence through `run-program`. Full parser parity,
checker/backend/platform/compiler package/driver/proof work, and self-boot
evidence remain open.

Round 316 extends the same shared parser-owned source-text parser-parity
library with a bounded GADT/existential constructor syntax slice. It proves
committed parser-program projections for parameterized data heads, GADT-style
constructor result heads, constructor-local Unicode `∀`, nested source types,
related constructor/case patterns, and malformed constructor-local forall-dot
evidence through `run-program`. Full parser parity,
checker/backend/platform/compiler package/driver/proof work, and self-boot
evidence remain open.

Round 317 extends the same shared parser-owned source-text parser-parity
library with a bounded qualified import/reference syntax slice. It proves
committed parser-program projections for import aliases, alias-only imports,
exposed classes/types/constructors/values/methods, qualified references, and
malformed import-alias evidence through `run-program`. Full parser parity,
resolver/checker/backend/platform/compiler package/driver/proof work, and
self-boot evidence remain open.

Round 318 extends the same shared parser-owned source-text parser-parity
library with a bounded complete-program multi-module export/import slice. It
proves committed parser-program projections for a `Core` module and `User`
module pair with selected abstract export/import items, recursive ADT
constructor export/import items, cross-module references, and malformed
import-exposing separator evidence through `run-program`. Full parser parity,
resolver/checker/backend/platform/compiler package/driver/proof work, and
self-boot evidence remain open.
...
- The parser parity tracer at
  ...
  evidence through `run-program`.
- The parser parity tracers at
  `test/programs/compiler-parser-parity/typeclass-deriving-method/` and
  `test/programs/compiler-parser-parity/typeclass-instance-nullary-method/`
  print the same parser-program projection and source-span evidence as the
  Haskell canonical parser for bounded class declarations, method signatures,
  `deriving`, instance declarations, and instance method definitions, and
  record malformed instance-method-definition evidence through `run-program`.
- The parser parity tracers at
  `test/programs/compiler-parser-parity/multi-module-abstract-export-import/`
  and
  `test/programs/compiler-parser-parity/multi-module-recursive-adt-export-import/`
  print the same parser-program projection and source-span evidence as the
  Haskell canonical parser for the selected complete-program multi-module
  export/import surfaces, and record malformed import-exposing separator
  evidence through `run-program`.
