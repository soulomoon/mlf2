...
`"λ"` (rendered as `"\955"`). `stringReplaceChar` is the first character
...
`stringFromNat (Succ (Succ Zero))` as `"2"`. `stringFromUnit`
is the first `Unit` to `String` conversion operation; the current
native-capable tracer is a pure Prelude definition and formats
`stringFromUnit Unit` as `"Unit"`. `stringFromList`
...
`charIsAsciiAlpha '7'` and `charIsAsciiAlpha 'λ'` are `false`.
`charIsAsciiAlphaNum` is an explicitly ASCII alphanumeric `Char`
classification tracer; it classifies ASCII uppercase, lowercase, or decimal
digit code points, so `charIsAsciiAlphaNum 'a'`, `charIsAsciiAlphaNum 'A'`,
and `charIsAsciiAlphaNum '7'` are `true` while `charIsAsciiAlphaNum '_'` and
`charIsAsciiAlphaNum 'λ'` are `false`. `charIsAsciiIdentifierStart` is an
explicitly ASCII identifier-start `Char` classification tracer matching the
current parser start set; it classifies ASCII lowercase, ASCII uppercase, or
underscore code points, so `charIsAsciiIdentifierStart 'a'`,
...
`false`. `charIsAsciiPunctuation` is an explicitly ASCII punctuation `Char`
classification tracer; it classifies exactly the ASCII ranges `0x21..0x2f`,
`0x3a..0x40`, `0x5b..0x60`, and `0x7b..0x7e`, so
...
`Zero`/`Succ` values. The first `Unit` to `String` conversion operation is
`stringFromUnit`, with native coverage through a pure Prelude definition that
returns the `"Unit"` string literal for `Unit`. The first `List Char` to `String`
...
scalar preserving list output. `stringDrop` is the first drop
...
Unicode scalar examples. The next explicit ASCII helper is
`charIsAsciiAlphaNum`, with native coverage for lowercase ASCII, uppercase
ASCII, ASCII digit, ASCII underscore, and non-ASCII Unicode scalar examples.
The next explicit ASCII helper is `charIsAsciiIdentifierStart`, with native
...
The next explicit ASCII helper is `charIsAsciiPunctuation`, with native
