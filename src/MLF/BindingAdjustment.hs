{- |
Module      : MLF.BindingAdjustment
Description : Binding-edge harmonization for unification
Copyright   : (c) 2024
License     : BSD-3-Clause

This module implements binding-edge harmonization, which generalizes the
legacy TyVar-only rank adjustment to work with the paper-style binding tree.
...
This is a generalization of the TyVar-only rank adjustment: instead of
adjusting `tnVarLevel` fields, we update the binding-edge structure directly.
-}
module MLF.BindingAdjustment (
    -- * Main API
...
import MLF.Types
import MLF.Binding (lookupBindParent, bindingLCA, isBindingRoot)
import MLF.GraphOps (applyRaiseStep, isInstantiable)
    ...
    case (mbParent1, mbParent2) of
        ...
        (Just (p1, _), Just (p2, _)) ->
            case bindingLCA c0 p1 p2 of
                ...
                Left (InvalidBindingTree msg)
                    -- Multi-root constraint graphs can yield disjoint binding trees.
                    -- In that case, there is no meaningful LCA to raise to; we
                    -- defer reconciliation to UF canonicalization (which
                    -- deterministically chooses one parent).
...
harmonizeBindParents n1 n2 c =
    case harmonizeBindParentsWithTrace n1 n2 c of
        ...
        Left (InvalidBindingTree msg)
            | msg == "No common ancestor found" ->
                -- Multi-root graphs can produce disjoint binding trees; treat
                -- this as “no harmonization possible” and let later UF
                -- canonicalization pick a parent.
                c
        Left err ->
            ...
            Just (parent, _) ->
                if parent == target
                    then return (constraint, reverse trace)
                        ...
                        instantiable <- isInstantiable constraint nid
                        if not instantiable
