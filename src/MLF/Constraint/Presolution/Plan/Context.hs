{- Note [Generalization planning context]
...
\* GeneralizeCtx — the resolved per-call-site context produced by
  ...
  type root, owning gen node, merged binding parents, and presolution view
  for reification.

'resolveContext' is the single entry point that canonicalizes roots and
targets (Phase 1), discovers the scope's owning gen node (Phase 2), and
merges base + solved binding parents into a unified view (Phase 3).  The
result feeds into 'buildSchemeRootsPlan' (SchemeRoots) and eventually into
the generalization-plan construction in Plan.Generalize.

Related thesis sections:
  - Section 7.6.2 — Generalized unification and gen-node scoping
  - Section 15.2.3.2 — Weaken inert-locked nodes and scope discovery
-}
...
validateCrossGenMapping ::
  GenNodeId ->
  -- | firstGenAncestor for the base tree
  (NodeRef -> Maybe GenNodeId) ->
  -- | base binding parents
  BindParents ->
  -- | findSolvedKey
  (Int -> Maybe Int) ->
  Either ElabError ()
