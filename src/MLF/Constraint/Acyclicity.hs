          ...
          if c' == c
            then
              Left
                cycleErr
                  { ceMessage =
                      "Cycle rewrite made no progress: "
                        ++ ceMessage cycleErr
                  }
            else go c'
...
mkAcyclicityResult c depGraph sortedIds =
  let edgeMap =
        IntMap.fromList
          [(getEdgeId (instEdgeId e), e) | e <- cInstEdges c]
      sortedEdges =
        mapMaybe
...
data CycleRewriteState = CycleRewriteState
  { crsNextNodeKey :: !Int,
    crsNodes :: !(NodeMap TyNode),
    crsBindParents :: !BindParents,
    crsCloneMap :: !(IntMap NodeId)
  }
...
breakSingleCycle c cycleIds = do
  pivotEdge <-
    maybe
      (Left (invalidCycleError cycleIds "pivot edge not found in constraint"))
      Right
      (lookupInstEdge c pivotId)
  cycleEdges <-
    traverse
      ( \eid ->
          maybe
            (Left (invalidCycleError cycleIds ("cycle edge not found: " ++ show (getEdgeId eid))))
            Right
  ...
  let recursiveNodes =
        IntSet.unions
          [ collectReachableNodes (cNodes c) (instLeft edge)
          | edge <- cycleEdges
          ]
      initialState =
        CycleRewriteState
          { crsNextNodeKey = maxNodeIdKeyOr0 c + 1,
            crsNodes = cNodes c,
            crsBindParents = cBindParents c,
            crsCloneMap = IntMap.empty
          }
      (muRoot, finalState) = runCycleRewrite c pivotEdge recursiveNodes initialState
      rewrittenEdges =
        map
          ( \edge ->
              if instEdgeId edge == instEdgeId pivotEdge
                then edge {instRight = muRoot}
                else edge
          )
          (cInstEdges c)
  pure
    c
      { cNodes = crsNodes finalState,
        cBindParents = crsBindParents finalState,
        cInstEdges = rewrittenEdges
      }
  where
    pivotId = minimum cycleIds

invalidCycleError :: [EdgeId] -> String -> CycleError
invalidCycleError cycleIds msg =
  CycleError
    { ceEdgesInCycle = cycleIds,
      ceMessage = msg
    }
...
runCycleRewrite ::
  ...
  IntSet ->
  CycleRewriteState ->
  (NodeId, CycleRewriteState)
runCycleRewrite c pivotEdge recursiveNodes =
  runRewrite
  where
    runRewrite =
      let originalNodes = cNodes c
          originalRhs = instRight pivotEdge
...
cloneWithSubstitution ::
  ...
  NodeId ->
  NodeId ->
  CycleRewriteM NodeId
cloneWithSubstitution originalNodes recursiveNodes binderId nid
  | IntSet.member (getNodeId nid) recursiveNodes = pure binderId
  | otherwise = do
      clones <- gets crsCloneMap
      case IntMap.lookup (getNodeId nid) clones of
        Just cloneId -> pure cloneId
        Nothing ->
          case lookupNodeIn originalNodes nid of
            Nothing -> pure nid
            Just node -> do
              cloneId <- freshNodeIdCR
              rememberClone nid cloneId
              cloneNode <- case node of
                TyVar {tnBound = mbBound} ->
                  pure
                    TyVar
                      { tnId = cloneId,
                        tnBound = mbBound
                ...
                TyArrow {tnDom = dom, tnCod = cod} -> do
                  dom' <- cloneWithSubstitution originalNodes recursiveNodes binderId dom
                  cod' <- cloneWithSubstitution originalNodes recursiveNodes binderId cod
                  pure
                    TyArrow
                      { tnId = cloneId,
                        tnDom = dom',
...
translateParent :: IntMap NodeId -> NodeRef -> NodeRef
translateParent clones parentRef =
  case parentRef of
    TypeRef parentId ->
      case IntMap.lookup (getNodeId parentId) clones of
        Just cloneId -> typeRef cloneId
        Nothing -> parentRef
    GenRef _ -> parentRef
...
freshNodeIdCR = do
  nextKey <- gets crsNextNodeKey
  modify' $ \st -> st {crsNextNodeKey = nextKey + 1}
  pure (NodeId nextKey)

insertNodeCR :: TyNode -> CycleRewriteM ()
insertNodeCR node =
  modify' $ \st ->
    st
      { crsNodes = insertNode (tnId node) node (crsNodes st)
      }

rememberClone :: NodeId -> NodeId -> CycleRewriteM ()
rememberClone originalId cloneId =
  modify' $ \st ->
    st
      { crsCloneMap =
          IntMap.insert (getNodeId originalId) cloneId (crsCloneMap st)
      }

setBindParentCR :: NodeRef -> (NodeRef, BindFlag) -> CycleRewriteM ()
setBindParentCR child parentInfo =
  modify' $ \st ->
    st
      { crsBindParents =
          IntMap.insert (nodeRefKey child) parentInfo (crsBindParents st)
      }
