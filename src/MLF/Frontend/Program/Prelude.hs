...
preludeSource =
  unlines
    [ "module Prelude export (Unit(..), IO, Nat(..), Option(..), List(..), Eq, Show, Functor, Applicative, Monad, eq, show, map, pure, ap, bind, putStrLn, getLine, putStr, readFile, writeFile, appendFile, exitWith, newIORef, readIORef, writeIORef, getArgs, stringLength, stringIsEmpty, stringContainsChar, stringContains, stringEquals, stringStartsWith, stringEndsWith, stringAppend, stringReplaceChar, stringReplace, stringIndexOfChar, stringIndexOf, stringSplit, stringJoin, stringSplitChar, stringCompare, stringFromChar, stringFromInt, stringFromBool, stringFromNat, stringFromUnit, stringFromList, stringToList, stringDrop, stringTake, stringSlice, stringCharAt, stringCharAtOption, charIsDigit, charIsAsciiLower, charIsAsciiUpper, charIsAsciiAlpha, charIsAsciiAlphaNum, charIsAsciiIdentifierStart, charIsAsciiIdentifierContinue, charIsAsciiWhitespace, charIsAsciiPunctuation, charIsAsciiPrintable, charIsAsciiHexDigit, charIsAsciiLineBreak, charIsAsciiControl, charToAsciiLower, charToAsciiUpper, stringToAsciiLower, stringToAsciiUpper, and, id) {",
      "  class Eq a {",
      "    eq : a -> a -> Bool;",
      "  }",
      "",
      ...
      "  def stringJoin : String -> List String -> String = __string_join;",
      "  def stringSplitChar : String -> Char -> List String = __string_split_char;",
      "  def stringCompare : String -> String -> Int = __string_compare;",
      ...
      "  def stringFromUnit : Unit -> String = λx \"Unit\";",
      "  def stringFromList : List Char -> String = __string_from_list;",
      "  def stringToList : String -> List Char = __string_to_list;",
      ...
      "  def charIsAsciiIdentifierContinue : Char -> Bool = __char_is_ascii_identifier_continue;",
      "  def charIsAsciiWhitespace : Char -> Bool = __char_is_ascii_whitespace;",
      "  def charIsAsciiPunctuation : Char -> Bool = __char_is_ascii_punctuation;",
      "  def charIsAsciiPrintable : Char -> Bool = __char_is_ascii_printable;",
      "  def charIsAsciiHexDigit : Char -> Bool = __char_is_ascii_hex_digit;",
      "  def charIsAsciiLineBreak : Char -> Bool = __char_is_ascii_line_break;",
      "  def charIsAsciiControl : Char -> Bool = __char_is_ascii_control;",
      "  def charToAsciiLower : Char -> Char = __char_to_ascii_lower;",
      "  def charToAsciiUpper : Char -> Char = __char_to_ascii_upper;",
      "  def stringToAsciiLower : String -> String = __string_to_ascii_lower;",
      "  def stringToAsciiUpper : String -> String = __string_to_ascii_upper;",
