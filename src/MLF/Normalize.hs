...
The paper describes several "local transformations" (§4, p.8):
  ...
  5. Push/pull G-nodes: ensure G-nodes appear only at the top.
     (§4, "push/pull binding nodes to the top" - structural in our repr.)
...
normalize c = nsConstraint finalState
  where
    initialState = NormalizeState
        { nsNextNodeId = findMaxNodeId c + 1
        , nsUnionFind = IntMap.empty
        , nsConstraint = c
        }
    finalState = execState normalizeLoop initialState

-- | Find the maximum NodeId in use so we can allocate fresh ones.
findMaxNodeId :: Constraint -> Int
findMaxNodeId c =
    maximum (0 : IntMap.keys (cNodes c))
...
partitionGraftable edges nodes = do
    ...
    let isGraftable edge =
            let leftId = findRoot uf (instLeft edge)
                ...
                leftNode = IntMap.lookup (getNodeIdInt leftId) nodes
                rightNode = IntMap.lookup (getNodeIdInt rightId) nodes
...
──────────────────────────────────────────────
  Var ≤ Arrow    → Graft: create fresh (α₁ → α₂), unify var with it,
                   then unify α₁, α₂ with the arrow's components
                   (This is the core "grafting" operation from the paper.)
    ...
    case (leftNode, rightNode) of
        ...
        (Just (TyVar { tnVarLevel = level }), Just (TyArrow { tnDom = rDom, tnCod = rCod }))
          ...
          | otherwise -> do
            ...
            arrowNode <- freshArrow domVar codVar
            -- Return unification edges
            pure $ Just [ UnifyEdge leftId arrowNode
...
insertNode node = modify' $ \s ->
    let c = nsConstraint s
        nodes' = IntMap.insert (getNodeIdInt (tnId node)) node (cNodes c)
        ...
        if left == right
            then pure acc  -- Already unified
                ...
                case (leftNode, rightNode) of
                    ...
                    (Just (TyArrow { tnDom = lDom, tnCod = lCod }),
                     Just (TyArrow { tnDom = rDom, tnCod = rCod })) -> do
                        unionNodes left right
                        -- Add new unification edges for components
                        let newEdges = [ UnifyEdge lDom rDom, UnifyEdge lCod rCod ]
                        pure (acc ++ newEdges)

                    -- Base = Base: check compatibility
                    (Just (TyBase { tnBase = lBase }),
                     Just (TyBase { tnBase = rBase }))
                        | lBase == rBase -> do
                            unionNodes left right
                            pure acc
                        | otherwise ->
...
-- | Apply union-find to an instantiation edge.
applyToInstEdge :: IntMap NodeId -> InstEdge -> InstEdge
applyToInstEdge uf edge = edge
    { instLeft = findRoot uf (instLeft edge)
    , instRight = findRoot uf (instRight edge)
    }

-- | Apply union-find to a unification edge.
applyToUnifyEdge :: IntMap NodeId -> UnifyEdge -> UnifyEdge
applyToUnifyEdge uf edge = edge
    { uniLeft = findRoot uf (uniLeft edge)
    , uniRight = findRoot uf (uniRight edge)
    }
