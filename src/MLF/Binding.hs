...
-- | Ordered binders for a TyForall node (leftmost-lowermost, paper ≺).
--
-- Binders are filtered to those reachable from the forall body.
orderedBinders
    :: (NodeId -> NodeId)
    -> Constraint
    -> NodeId
...
orderedBinders canonical c0 binder0 = do
    let binderC = canonical binder0
    body0 <- case IntMap.lookup (getNodeId binderC) (cNodes c0) of
        Nothing ->
            Left $
                InvalidBindingTree $
                    "orderedBinders: binder " ++ show (getNodeId binderC) ++ " not in cNodes"
        Just TyForall{ tnBody = body } -> Right body
        Just _ ->
            Left $
                InvalidBindingTree $
                    "orderedBinders: binder " ++ show (getNodeId binderC) ++ " is not TyForall"
    let bodyC = canonical body0
        reachable = structuralReachableFrom canonical (cNodes c0) bodyC
    binders <- boundFlexChildrenUnder canonical c0 binderC
    let bindersReachable =
            filter (\nid -> IntSet.member (getNodeId nid) reachable) binders
        orderKeys = orderKeysFromRootWith canonical (cNodes c0) bodyC
    pure (sortBy (compareNodesByOrderKey orderKeys) bindersReachable)
...
structuralReachableFrom :: (NodeId -> NodeId) -> IntMap.IntMap TyNode -> NodeId -> IntSet
structuralReachableFrom canonical nodes root0 = go IntSet.empty [canonical root0]
  where
    go visited [] = visited
    go visited (nid : rest)
...
orderKeysFromRootWith :: (NodeId -> NodeId) -> IntMap.IntMap TyNode -> NodeId -> IntMap.IntMap OrderKey
orderKeysFromRootWith canonical nodes root0 =
    go IntMap.empty [(canonical root0, 0, [])]
  where
    better :: OrderKey -> OrderKey -> Bool
    better new old = compareOrderKey new old == LT

    go :: IntMap.IntMap OrderKey -> [(NodeId, Int, [Int])] -> IntMap.IntMap OrderKey
    go m [] = m
...
compareNodesByOrderKey :: IntMap.IntMap OrderKey -> NodeId -> NodeId -> Ordering
...
bindingLCA c n1 n2 = do
    ...
    case common of
        [] -> Left $ InvalidBindingTree "No common ancestor found"
        (lca:_) -> Right lca
...
checkBindingTreeUnder canonical c0 = do
    let nodes0 = cNodes c0
        ...
        structuralChildren :: TyNode -> [NodeId]
        structuralChildren node = case node of
            TyVar {} -> []
            TyBase {} -> []
        ...
        incomingRoots :: IntSet
        incomingRoots = IntSet.unions (IntMap.elems structEdges)

        termDagRoots :: IntSet
        termDagRoots = IntSet.difference allRoots incomingRoots
...
computeTermDagRoots c =
    let nodes = cNodes c
        ...
        referencedNodes = IntSet.fromList $ concatMap getStructuralChildren (IntMap.elems nodes)
        -- Term-DAG roots are nodes that are NOT referenced by any structure edge
    in IntSet.difference allNodeIds' referencedNodes
  where
    getStructuralChildren :: TyNode -> [Int]
    getStructuralChildren (TyVar {}) = []
    getStructuralChildren (TyBase {}) = []
    getStructuralChildren (TyArrow _ (NodeId dom) (NodeId cod)) = [dom, cod]
    getStructuralChildren (TyForall _ _ _ (NodeId body)) = [body]
    getStructuralChildren (TyExp _ _ (NodeId body)) = [body]
...
isUpper c parent child
  ...
  where
    ...
    -- Get the structural children of a node in the term-DAG
    structuralChildren :: TyNode -> [NodeId]
    structuralChildren (TyVar {}) = []
    structuralChildren (TyBase {}) = []
    structuralChildren (TyArrow _ dom cod) = [dom, cod]
    structuralChildren (TyForall _ _ _ body) = [body]
    structuralChildren (TyExp _ _ body) = [body]
