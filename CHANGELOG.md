...
### Changed
- Added the M7 explicit-only recursive-annotation path: normalized `STMu` annotations now lower into an acyclic graph `TyMu`, the graph/reifier/result-type pipeline preserves those explicit binders end-to-end, and focused coverage confirms explicit `μ` annotations succeed without enabling automatic recursive-type inference or cyclic/equi-recursive solver behavior.
