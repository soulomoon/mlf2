...
3. **Constraint core:** `MLF.Constraint.Types.Graph`, `MLF.Constraint.Types.Witness`, and `MLF.Constraint.Types.Presolution` hold the split core data types.
...
8. **Audit/test surfaces:** `test/PipelineSpec.hs`, `test/Presolution/ExpansionSpec.hs`, `test/Presolution/WitnessSpec.hs`, `test/Presolution/EnforcementSpec.hs`, `test/TypeCheckSpec.hs`, `test/ReduceSpec.hs`, `test/TypeSoundnessSpec.hs`, `docs/paper-map.md`, `docs/thesis-obligations.yaml`, `docs/thesis-claims.yaml`, and `docs/thesis-deviations.yaml` are the main regression/audit anchors for keeping the roadmap aligned with the implementation.
