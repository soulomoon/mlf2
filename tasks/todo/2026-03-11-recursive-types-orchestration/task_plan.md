...
| Fresh Round 9 authority-audit dispatch on `gpt-5.2` errored before inspection because that model/runtime combination did not support the hidden `tool_search` request path | 1 | Relaunched the same read-only authority-audit role as a fresh `gpt-5.5` subagent and captured the required JSON authority result without changing packet state |
